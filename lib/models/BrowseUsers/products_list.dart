...
class ProductsListingModel extends ProductsListingBase{
  Map<String, dynamic> json;
  String productID = '';
  bool isLogin = false;
  ProductsListingModel(Map<String, dynamic> this.json):super(json);
 // @override
  Widget viewItem2 (ItemProductsModel item,ItemProductsModel item1, String search, BuildContext context,bool account, String id , String title, String userid, bool isMax, int max, int index, ChatBloc cb) {
    ShapeBorder? shape;
    double? height = 0;
    productID = id;
    isLogin = account;
    ...
    return Visibility (
        ...
        child:  ItemUserProductCard(destination :item, search : search, shape : shape, account: account, height: height, idHash: userid, title : title, idSeller: id, cb : cb )
    );
...
class ItemUserProductCard extends StatelessWidget {
  const ItemUserProductCard({ Key? key, @required this.destination, this.search, this.shape, this.height, this.account, this.idHash, this.title, this.idSeller, this.cb})
    ...
    return SafeArea(
        ...
        child: Padding(
          ...
          Card(
            ...
            ItemUserProductContent(destination: destination, account : account, idHash: this.idHash, title: this.title, idSeller: this.idSeller, cb : cb),
...
class ItemUserProductContent extends StatelessWidget {
  const ItemUserProductContent({ Key? key, @required this.destination, this.account, this.idHash, this.title, this.idSeller, this.cb})
      : assert(destination != null),
        super(key: key);
    ...
    final List<Widget> children = <Widget>[
              ...
              children:<Widget>[
                ...
                GestureDetector(
                  onTap: () {
                    AppProvider.getRouter(context)!.navigateTo(
                        context,
                        urlToRoute('/public/browse_users/view/$idHash/zzzz'));
                  },
                  child:
                  Padding(
                    ...
                    GestureDetector(
                      onTap:(){
                        if(account!){
                          AppProvider.getRouter(context)!.navigateTo(
                              context,
                              urlToRoute('/public/browse_products/add_to_cart/${destination!.item.product_id}/zzzzz'));
                        }else{
                          AppProvider.getRouter(context)!.navigateTo(
                              context,
                              '/login/1');
      ...
      GestureDetector(
        onTap: () {
          AppProvider.getRouter(context)!.navigateTo(
              context,
              urlToRoute('/public/browse_products/view/${destination!.item.product_id}/123' ));
        },
        child:
        Padding(
                ...
                child: new Material(
                    ...
                    child: new InkWell(
                      ...
                      onTap: () {
                        AppProvider.getRouter(context)!.navigateTo(
                            context,
                            urlToRoute('/public/browse_products/view/${destination!.item.product_id}/${destination!.item.title}'));
                      },
                    )
                )
            ),
