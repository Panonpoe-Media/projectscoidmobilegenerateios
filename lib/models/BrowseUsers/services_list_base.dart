...
import '../photo.dart';
import '../paging.dart';
part 'services_list_base.g.dart';


class ServicesListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ServicesListingBase(this.json){
		items = ServicesListingItems.fromJson(this.json);
		tools = ServicesListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemServicesModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 4 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemServicesCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemServicesModel item,ItemServicesModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonServicesWidget(tools.buttons[i],context));
    }
...
class ServicesListingItems{
  List<ItemServicesModel?>? items;
  ServicesListingItems(
      this.items,
      );

  factory ServicesListingItems.fromJson(Map<String, dynamic> json) {
  	return ServicesListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemServicesModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ServicesListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemServices?>? items;
	...
	factory ServicesListingTools.fromJson(Map<String, dynamic> json) => _$ServicesListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ServicesListingToolsToJson(this);
...
class ItemServicesCard extends StatelessWidget {
  const ItemServicesCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemServicesContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemServicesContent extends StatelessWidget {
  const ItemServicesContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemServicesButton(ItemServicesModel destination, int? index, BuildContext context){
...
class ItemServicesWidget extends StatelessWidget {
	...
	final ItemServicesModel? item;
	final String? search;
	const  ItemServicesWidget({Key? key, @required this.item, this.search}) : super(key: key);
