...
import 'as_project_owner_index_item.dart';
import 'as_project_owner_index_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'as_project_owner_index_base.g.dart';


class AsProjectOwnerIndexBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	AsProjectOwnerIndexBase(this.json){
		items = AsProjectOwnerIndexItems.fromJson(this.json);
		tools = AsProjectOwnerIndexTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemIndexAsProjectOwnerModel item,String? search) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemIndexAsProjectOwnerCard(destination :item, search : search, shape : shape, height : height)
    );
   ...
   List<SpeedDialChild>	 listButton(BuildContext context){
    ...
    {
      buttonChildren.add(IndexButtonAsProjectOwnerWidget(tools.buttons[i],context));
    }
...
class AsProjectOwnerIndexItems{
  List<ItemIndexAsProjectOwnerModel?>? items;
  AsProjectOwnerIndexItems(
      this.items,
      );

  factory AsProjectOwnerIndexItems.fromJson(Map<String, dynamic> json) {
  	return AsProjectOwnerIndexItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemIndexAsProjectOwnerModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class AsProjectOwnerIndexTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemIndexAsProjectOwner?>? items;
	...
	factory AsProjectOwnerIndexTools.fromJson(Map<String, dynamic> json) => _$AsProjectOwnerIndexToolsFromJson(json);

	Map<String, dynamic> toJson() => _$AsProjectOwnerIndexToolsToJson(this);
...
class ItemIndexAsProjectOwnerCard extends StatelessWidget {
  const ItemIndexAsProjectOwnerCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemIndexAsProjectOwnerModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            SizedBox(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemIndexAsProjectOwnerContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemIndexAsProjectOwnerContent extends StatelessWidget {
  const ItemIndexAsProjectOwnerContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemAsProjectOwnerButton(ItemIndexAsProjectOwnerModel destination, int index, BuildContext context){
...
class ItemIndexAsProjectOwnerWidget extends StatelessWidget {

	final ItemIndexAsProjectOwnerModel?  item;
	final String? search;
	const  ItemIndexAsProjectOwnerWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild IndexButtonAsProjectOwnerWidget(Button button, BuildContext context){
