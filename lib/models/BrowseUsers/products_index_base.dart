...
import 'products_index_item.dart';
import 'products_index_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'products_index_base.g.dart';


class ProductsIndexBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ProductsIndexBase(this.json){
		items = ProductsIndexItems.fromJson(this.json);
		tools = ProductsIndexTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemIndexProductsModel item,String? search) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 4 ;
	  height = totalHeight * 50;
        ...
        child:  ItemIndexProductsCard(destination :item, search : search, shape : shape, height : height)
    );
   ...
   List<SpeedDialChild>	 listButton(BuildContext context){
    ...
    {
      buttonChildren.add(IndexButtonProductsWidget(tools.buttons[i],context));
    }
...
class ProductsIndexItems{
  List<ItemIndexProductsModel?>? items;
  ProductsIndexItems(
      this.items,
      );

  factory ProductsIndexItems.fromJson(Map<String, dynamic> json) {
  	return ProductsIndexItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemIndexProductsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ProductsIndexTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemIndexProducts?>? items;
	ProductsIndexTools(
			this.id,
			this.buttons,
			this.meta,
	...
	factory ProductsIndexTools.fromJson(Map<String, dynamic> json) => _$ProductsIndexToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ProductsIndexToolsToJson(this);
...
class ItemIndexProductsCard extends StatelessWidget {
  const ItemIndexProductsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemIndexProductsModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            SizedBox(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemIndexProductsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemIndexProductsContent extends StatelessWidget {
  const ItemIndexProductsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemProductsButton(ItemIndexProductsModel destination, int index, BuildContext context){
...
class ItemIndexProductsWidget extends StatelessWidget {

	final ItemIndexProductsModel?  item;
	final String? search;
	const  ItemIndexProductsWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild IndexButtonProductsWidget(Button button, BuildContext context){
