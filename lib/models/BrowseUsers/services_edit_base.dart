...
part 'services_edit_base.g.dart';


Widget RButtonActionServicesWidget(Button button, BuildContext context,var formKey, ScrollController controller,  SubModelController services,
...
class EditModelServices {
  ...
  String? special_requirements;
    ...
    factory EditModelServices.fromJson(Map<String, dynamic> json) => _$EditModelServicesFromJson(json);

  Map<String, dynamic> toJson() => _$EditModelServicesToJson(this);
...
class EditServicesBase{
	var model;
	Map<String, dynamic> json;
	EditServicesBase(this.json){
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, SubModelController services,
  ...
  var formData = convertFormDataEdit(model);  
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionServicesWidget(model.buttons[i], context,formKey, controller,services, postServicesResult, state, sendPath, id,  title, formData));
      }
    }
       return(
	        buttonChildren 
  ...
  var postServicesResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonEditServicesWidget(model.buttons[i], context,formKey, controller,services, postServicesResult, state, sendPath, id,  title, formData ));
       ...
       Widget	 RButtons(BuildContext context, bool? visible, var formKey, ScrollController controller, SubModelController services,
  var postServicesResult, State state, String? sendPath, String? id,  String? title ){
    //  final size =MediaQuery.of(context).size;
    double? width = 400;
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,services, postServicesResult, state, sendPath, id,  title )
