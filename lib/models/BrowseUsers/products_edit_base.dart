...
part 'products_edit_base.g.dart';


Widget RButtonActionProductsWidget(Button button, BuildContext context,var formKey, ScrollController controller,  SubModelController products,
...
class EditModelProducts {
  ...
  String? rating_div_str;
    ...
    factory EditModelProducts.fromJson(Map<String, dynamic> json) => _$EditModelProductsFromJson(json);

  Map<String, dynamic> toJson() => _$EditModelProductsToJson(this);
...
class EditProductsBase{
	var model;
	Map<String, dynamic> json;
	EditProductsBase(this.json){
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, SubModelController products,
  ...
  var formData = convertFormDataEdit(model);  
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionProductsWidget(model.buttons[i], context,formKey, controller,products, postProductsResult, state, sendPath, id,  title, formData));
      }
    }
       return(
	        buttonChildren 
  ...
  var postProductsResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonEditProductsWidget(model.buttons[i], context,formKey, controller,products, postProductsResult, state, sendPath, id,  title, formData ));
       ...
       Widget	 RButtons(BuildContext context, bool? visible, var formKey, ScrollController controller, SubModelController products,
  var postProductsResult, State state, String? sendPath, String? id,  String? title ){
    //  final size =MediaQuery.of(context).size;
    double? width = 400;
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,products, postProductsResult, state, sendPath, id,  title )
