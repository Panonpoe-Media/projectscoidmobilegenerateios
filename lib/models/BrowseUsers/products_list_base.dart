...
import 'products_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'products_list_base.g.dart';


class ProductsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ProductsListingBase(this.json){
		items = ProductsListingItems.fromJson(this.json);
		tools = ProductsListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemProductsModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 4 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemProductsCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemProductsModel item,ItemProductsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonProductsWidget(tools.buttons[i],context));
    }
...
class ProductsListingItems{
  List<ItemProductsModel?>? items;
  ProductsListingItems(
      this.items,
      );

  factory ProductsListingItems.fromJson(Map<String, dynamic> json) {
  	return ProductsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemProductsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
class ProductsListingTools{
	...
	factory ProductsListingTools.fromJson(Map<String, dynamic> json) => _$ProductsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ProductsListingToolsToJson(this);
...
class ItemProductsCard extends StatelessWidget {
  const ItemProductsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemProductsModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemProductsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemProductsContent extends StatelessWidget {
  const ItemProductsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemProductsButton(destination!,i , context));
		}
    }
    ...
    final List<Widget> children = [
	  ...
	  */
	   ...
	   children.add(destination!.viewRanking(context)); 
...
Widget itemProductsButton(ItemProductsModel destination, int? index, BuildContext context){
...
class ItemProductsWidget extends StatelessWidget {
	...
	final ItemProductsModel? item;
	final String? search;
	const  ItemProductsWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonProductsWidget(Button button, BuildContext context){
