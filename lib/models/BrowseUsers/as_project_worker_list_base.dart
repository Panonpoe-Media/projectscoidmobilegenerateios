...
import '../photo.dart';
import '../paging.dart';
part 'as_project_worker_list_base.g.dart';
/** AUTOGENERATE OFF **/

class AsProjectWorkerListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	AsProjectWorkerListingBase(this.json){
		items = AsProjectWorkerListingItems.fromJson(this.json);
		tools = AsProjectWorkerListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemAsProjectWorkerModel item,String search, bool account, String id) {
	ShapeBorder? shape;
    double? height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemAsProjectWorkerCard(destination :item, search : search, shape : shape, height: height)
    );
  ...
  Widget viewItem1 (ItemAsProjectWorkerModel item,ItemAsProjectWorkerModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonAsProjectWorkerWidget(tools.buttons[i],context));
    }
...
class AsProjectWorkerListingItems{
  List<ItemAsProjectWorkerModel?>? items;
  AsProjectWorkerListingItems(
      this.items,
      );

  factory AsProjectWorkerListingItems.fromJson(Map<String, dynamic> json) {
  	return AsProjectWorkerListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemAsProjectWorkerModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class AsProjectWorkerListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemAsProjectWorkerRev?>? items;
	...
	factory AsProjectWorkerListingTools.fromJson(Map<String, dynamic> json) => _$AsProjectWorkerListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$AsProjectWorkerListingToolsToJson(this);
...
class ItemAsProjectWorkerCard extends StatelessWidget {
  const ItemAsProjectWorkerCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemAsProjectWorkerContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemAsProjectWorkerContent extends StatelessWidget {
  const ItemAsProjectWorkerContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
class ItemAsProjectWorkerWidget extends StatelessWidget {

	final ItemAsProjectWorkerModel? item;
	final String? search;
	const  ItemAsProjectWorkerWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonAsProjectWorkerWidget(Button button, BuildContext context){
