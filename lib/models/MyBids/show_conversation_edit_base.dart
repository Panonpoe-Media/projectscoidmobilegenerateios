...
part 'show_conversation_edit_base.g.dart';
...
Widget RButtonActionShowConversationWidget(Button button, BuildContext context,var formKey, ScrollController controller,  SubModelController show_conversation,
 var postShowConversationResult, State state, String? sendPath, String? id,  String? title,  var formData){
  var cl;
  var ic;
		 ...
		 children: <Widget>[
             ...
             // color : Color(0xFF037f51),
              onPressed: () async  {
                        ...
                        show_conversation =      SubModelController(AppProvider.getApplication(context),
                        sendPath,
                        formData);
...
SpeedDialChild ButtonEditShowConversationWidget(Button button, BuildContext context,var formKey, ScrollController controller, SubModelController show_conversation,
 var postShowConversationResult, State state, String? sendPath, String? id,  String? title, var formData){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
...
class EditModelShowConversation {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
  ...
  bool? read_by_worker;
    ...
    factory EditModelShowConversation.fromJson(Map<String, dynamic> json) => _$EditModelShowConversationFromJson(json);

  Map<String, dynamic> toJson() => _$EditModelShowConversationToJson(this);
...
class ShowConversationSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  EditModelShowConversation? model;
  ShowConversationSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ShowConversationSuperBase.fromJson(Map<String, dynamic> json) => _$ShowConversationSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ShowConversationSuperBaseToJson(this);
...
class EditShowConversationBase{
	var model;
	Map<String, dynamic> json;
	EditShowConversationBase(this.json){
		model = ShowConversationSuperBase.fromJson(this.json);
	}
	
   Map<String, dynamic> convertFormDataEdit(var data){
      var formData = {
      "bid_conversation_for_worker[_trigger_]":"",
  "bid_conversation_for_worker[user_id]": "${model.model.user_id }",
  "bid_conversation_for_worker[project_id]": "${model.model.project_id }",
  "bid_conversation_for_worker[thread_id]": "${model.model.thread_id }",
   "bid_conversation_for_worker[post_type_id]": "${model.model.post_type_id}",
  "bid_conversation_for_worker[post_date]": "${model.model.post_date}",
  "bid_conversation_for_worker[bid_amount]": "${model.model.bid_amount}",
  "bid_conversation_for_worker[message]": "${model.model.message }",
  "bid_conversation_for_worker[attachments]": '[{"name":"${model.model.attachments[0].name}","size":${model.model.attachments[0].size.toString()},"created":${model.model.attachments[0].date.toString()},"modified":${model.model.attachments[0].date.toString()},"temp":"${model.model.attachments[0].temp}","remote":"","dir":"temp"}]',
  "bid_conversation_for_worker[read_by_owner]": "${model.model.read_by_owner}",
  "bid_conversation_for_worker[read_by_worker]": "${model.model.read_by_worker}",
  ...
  var formData = convertFormDataEdit(model);  
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionShowConversationWidget(model.buttons[i], context,formKey, controller,show_conversation, postShowConversationResult, state, sendPath, id,  title, formData));
      }
    }
       return(
	        buttonChildren 
  ...
  var postShowConversationResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonEditShowConversationWidget(model.buttons[i], context,formKey, controller,show_conversation, postShowConversationResult, state, sendPath, id,  title, formData ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,show_conversation, postShowConversationResult, state, sendPath, id,  title )
  ...
  var postShowConversationResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,show_conversation, postShowConversationResult, state, sendPath, id,  title )
	    )
	 );
   } 	
