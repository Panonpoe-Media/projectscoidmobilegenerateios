...
import 'existing_user_item.dart';
import 'existing_user_item_base.dart';
import 'photo.dart';
import 'paging.dart';
import 'package:horizontal_data_table/horizontal_data_table.dart';
import 'package:flutter_widget_from_html_core/flutter_widget_from_html_core.dart';
import 'package:url_launcher/url_launcher.dart';
import 'package:projectscoid/models/ExistingUser/action.dart';
import 'package:projectscoid/views/route.dart' as rt;
part 'existing_user_base.g.dart';

/** AUTOGENERATE OFF **/

@JsonSerializable()

class ModelActionForgotPasswordExistingUser {
  int?    age;
  int?    cnt;
  int?    page;
  String?   id;
...
String?   user_id;
  ...
  String?   captcha;
	  ...
	  ModelActionForgotPasswordExistingUser(
            this.id,
			this.user_id,
			this.your_email,
			this.captcha,
    ...
    factory ModelActionForgotPasswordExistingUser.fromJson(Map<String, dynamic> json) => _$ModelActionForgotPasswordExistingUserFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionForgotPasswordExistingUserToJson(this);

}



class ForgotPasswordExistingUserBase{
	var model;
	Map<String, dynamic> json;
	ForgotPasswordExistingUserBase(this.json){
		model = ForgotPasswordExistingUserSuperBase.fromJson(this.json);
	}



Widget RButtonActionExistingUserWidget(Button button, BuildContext context,var formKey, ScrollController controller, ExistingUserController existing_user,
 var postExistingUserResult, State state, String?   sendPath, String?   id,  String?   title){
  var cl;
  var ic;
           ...
           overlayColor: MaterialStateProperty.all<Color>(const Color(0x33ffcc5c)),
             ...
             // color : Color(0xFF037f51),
              onPressed: () async{
                      ...
                      var formData = await convertFormDataForgotPassword(button.text!, model
                     					  );
                      ...
                      final future = existing_user.postForgotPasswordExistingUserWithID();
                                  future.then((value) {
                                  state.setState(() {
                                  postExistingUserResult = value;
                                  });
                                  }).catchError((Error){
                       // AppProvider.getRouter(context)!.pop(context);	
							 Navigator.pushAndRemoveUntil(
								context,
								MaterialPageRoute(builder: (context) => rt.ForgotLinkView(id :  id!, title: 'Password Reminder Sent',)),
									(Route<dynamic> route) => false,
							  );
                      });
                      }else{
                      final future = existing_user.postForgotPasswordExistingUser();
                                  future.then((value) {
                                  state.setState(() {
                                  postExistingUserResult = value;
                                  });
...
SpeedDialChild  ButtonActionExistingUserWidget(Button button, BuildContext context,var formKey, ScrollController controller, ExistingUserController existing_user,
 var postExistingUserResult, State state, String?   sendPath, String?   id,  String?   title){
  var cl;
  var ic;
  if (button.color == 'green'){
  ...
  if(button.type == 'custom_filter'){
   ...
   bool? isEdit = action.contains(RegExp(r'edit', caseSensitive: false));    
      ...
      var formData = {
      "user[_trigger_]":"$tgr",
  "user[your_email]": "${model.model.your_email }",
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, ExistingUserController existing_user,
  var postForgotPasswordResult, State state, String?   sendPath, String?   id,  String?   title){
    final List<Widget> buttonChildren = <Widget>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(RButtonActionExistingUserWidget(model.buttons[i], context,formKey, controller,existing_user, postForgotPasswordResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
   ...
   List<SpeedDialChild> listButton(BuildContext context,var formKey, ScrollController controller, ExistingUserController existing_user,
  var postForgotPasswordResult, State state, String?   sendPath, String?   id,  String?   title){
    ...
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(ButtonActionExistingUserWidget(model.buttons[i], context,formKey, controller,existing_user, postForgotPasswordResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  Widget injectAction(BuildContext context, bool? visible, var formKey, ScrollController controller, ExistingUserController existing_user,
  var postForgotPasswordResult, State state, String?   sendPath, String?   id,  String?   title, bool? isLoading ) {
     return(Container(height: 0.0, width: 0.0)
	 );
  }
   
    Widget	 RButtons(BuildContext context, bool? visible, var formKey, ScrollController controller, ExistingUserController existing_user,
  var postForgotPasswordResult, State state, String?   sendPath, String?   id,  String?   title ){
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,existing_user, postForgotPasswordResult, state, sendPath, id,  title )
 ...
 SpeedDial	 Buttons(BuildContext context, bool? visible, var formKey, ScrollController controller, ExistingUserController existing_user,
  var postForgotPasswordResult, State state, String?   sendPath, String?   id,  String?   title ){
     return(
	 SpeedDial(
	childMargin: const EdgeInsets.symmetric(horizontal: 18, vertical: 20),
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children: listButton(context, formKey,controller,existing_user, postForgotPasswordResult, state, sendPath, id,  title )
	    )
	 );
   } 
...
@JsonSerializable()
class ForgotPasswordExistingUserSuperBase {

  String?   id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionForgotPasswordExistingUser? model;
  ForgotPasswordExistingUserSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ForgotPasswordExistingUserSuperBase.fromJson(Map<String, dynamic> json) => _$ForgotPasswordExistingUserSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ForgotPasswordExistingUserSuperBaseToJson(this);
...
class ModelActionResetPasswordExistingUser {
  int?    age;
  int?    cnt;
  int?    page;
  String?   id;
...
String?   user_id;
  String?   password;
	  ...
	  ModelActionResetPasswordExistingUser(
            this.id,
			this.user_id,
			this.password,

  );
    factory ModelActionResetPasswordExistingUser.fromJson(Map<String, dynamic> json) => _$ModelActionResetPasswordExistingUserFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionResetPasswordExistingUserToJson(this);

}



class ResetPasswordExistingUserBase{
	var model;
	Map<String, dynamic> json;
	ResetPasswordExistingUserBase(this.json){
		model = ResetPasswordExistingUserSuperBase.fromJson(this.json);
             ...
             // color : Color(0xFF037f51),
              onPressed: () async{
                                controller.animateTo(controller.position.minScrollExtent,
                                  ...
                                  //Map<String, dynamic> res = model.toJson();
                                  //print('json result == $res');
                      var formData = await convertFormDataResetPassword(button.text!, model
                     					  );
                                  existing_user = ExistingUserController(AppProvider.getApplication(context),
                                  sendPath,
                                  AppAction.post,
                      ...
                      false);
                      if(sendPath!.contains('%s')){
                      final future = existing_user.postResetPasswordExistingUserWithID();
                                  future.then((value) {
                                  state.setState(() {
                                  postExistingUserResult = value;
                                  });
                 ...
                 MaterialPageRoute(builder: (context) => rt.ForgotLinkView(id :  id!, title: 'Password Reminder Sent',)),
                      ...
                      final future = existing_user.postResetPasswordExistingUser();
                                  future.then((value) {
                                  state.setState(() {
                                  postExistingUserResult = value;
                                  });
...
SpeedDialChild  ButtonActionExistingUserWidget(Button button, BuildContext context,var formKey, ScrollController controller, ExistingUserController existing_user,
 var postExistingUserResult, State state, String?   sendPath, String?   id,  String?   title){
           ...
           SpeedDialChild(
                      ...
                      final future = existing_user.postResetPasswordExistingUserWithID();
                        ...
                        //AppProvider.getRouter(context)!.pop(context);	
						    Navigator.pushAndRemoveUntil(
								context,
								MaterialPageRoute(builder: (context) => rt.PublicExistingUserListing(id :  id!)),
									(Route<dynamic> route) => false,
							  );
                      });
                      }else{
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, ExistingUserController existing_user,
  var postResetPasswordResult, State state, String?   sendPath, String?   id,  String?   title){
    final List<Widget> buttonChildren = <Widget>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(RButtonActionExistingUserWidget(model.buttons[i], context,formKey, controller,existing_user, postResetPasswordResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postResetPasswordResult, State state, String?   sendPath, String?   id,  String?   title){
    ...
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(ButtonActionExistingUserWidget(model.buttons[i], context,formKey, controller,existing_user, postResetPasswordResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  Widget injectAction(BuildContext context, bool? visible, var formKey, ScrollController controller, ExistingUserController existing_user,
  var postResetPasswordResult, State state, String?   sendPath, String?   id,  String?   title, bool? isLoading ) {
     return(Container(height: 0.0, width: 0.0)
	 );
  }
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,existing_user, postResetPasswordResult, state, sendPath, id,  title )
 ...
 SpeedDial	 Buttons(BuildContext context, bool? visible, var formKey, ScrollController controller, ExistingUserController existing_user,
  var postResetPasswordResult, State state, String?   sendPath, String?   id,  String?   title ){
	...
	childMargin: const EdgeInsets.symmetric(horizontal: 18, vertical: 20),
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children: listButton(context, formKey,controller,existing_user, postResetPasswordResult, state, sendPath, id,  title )
	    )
	 );
   } 
...
@JsonSerializable()
class ResetPasswordExistingUserSuperBase {

  String?   id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionResetPasswordExistingUser? model;
  ResetPasswordExistingUserSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ResetPasswordExistingUserSuperBase.fromJson(Map<String, dynamic> json) => _$ResetPasswordExistingUserSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ResetPasswordExistingUserSuperBaseToJson(this);
...
class ModelExistingUser {
  ...
  String?   coupon;
    ...
    factory ModelExistingUser.fromJson(Map<String, dynamic> json) => _$ModelExistingUserFromJson(json);

  Map<String, dynamic> toJson() => _$ModelExistingUserToJson(this);
...
class ExistingUserBase{
	var model;
	Map<String, dynamic> json;
	ExistingUserBase(this.json){
		model = ExistingUserSuperBase.fromJson(this.json);
			model.model.password = '';
   ...
   Map<String, dynamic> convertFormDataEdit(var data){
	...
	if (model.model.photo!= null) {
      ...
      var formData = {
      "existing_user[_trigger_]":"",
   "existing_user[user_id]": "${model.model.user_id }",
  "existing_user[user_name]": "${model.model.user_name }",
  "existing_user[group_id]": "${model.model.group_id }",
  "existing_user[email]": "${model.model.email }",
  "existing_user[password]": "${model.model.password }",
  "existing_user[password_confirmation]": "${model.model.password }",
  "existing_user[auth]": "${model.model.auth }",
   "existing_user[account_status_id]": "${model.model.account_status_id}",
   "existing_user[account_type_id]": "${model.model.account_type_id}",
  "existing_user[date_registered]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.date_registered)}",
  "existing_user[last_login]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.last_login)}",
  "existing_user[last_seen]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.last_seen)}",
  "existing_user[online]": "${model.model.online ? 1 : 0}",
  "existing_user[last_online_mark]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.last_online_mark)}",
  "existing_user[online_hours]": "${model.model.online_hours}",
  "existing_user[nomor_ktp]": "${model.model.nomor_ktp }",
  "existing_user[scan_ktp]": '$scan_ktp',
  "existing_user[scan_ktp_lastval]": '{"name":"${model.model.scan_ktp.name}","dir":"${model.model.scan_ktp.dir}","file":"${model.model.scan_ktp.file}","thumb":"${model.model.scan_ktp.thumb}"}',
  "existing_user[nomor_siup]": "${model.model.nomor_siup }",
  "existing_user[scan_siup]": '$scan_siup',
  "existing_user[scan_siup_lastval]": '{"name":"${model.model.scan_siup.name}","dir":"${model.model.scan_siup.dir}","file":"${model.model.scan_siup.file}","thumb":"${model.model.scan_siup.thumb}"}',
  "existing_user[npwp]": "${model.model.npwp }",
   "existing_user[status_marital_id]": "${model.model.status_marital_id}",
  "existing_user[jumlah_tanggungan]": "${model.model.jumlah_tanggungan }",
  "existing_user[personal_data]": "${model.model.personal_data }",
  "existing_user[photo]": '$photo',
  "existing_user[photo_lastval]": '{"name":"${model.model.photo.name}","dir":"${model.model.photo.dir}","file":"${model.model.photo.file}","thumb":"${model.model.photo.thumb}"}',
  "existing_user[display_name]": "${model.model.display_name }",
  "existing_user[birth_date]": "${DateFormat('dd/MM/yyyy').format(model.model.birth_date)}",
  "existing_user[birth_code]": "${model.model.birth_code }",
   "existing_user[jenis_kelamin_id]": "${model.model.jenis_kelamin_id}",
  "existing_user[address]": "${model.model.address }",
  "existing_user[country_id]": "${model.model.country_id }",
  "existing_user[provinsi_id]": "${model.model.provinsi_id }",
  "existing_user[kabupaten_id]": "${model.model.kabupaten_id }",
  "existing_user[kecamatan_id]": "${model.model.kecamatan_id }",
  "existing_user[kelurahan_id]": "${model.model.kelurahan_id }",
  "existing_user[post_code]": "${model.model.post_code }",
  "existing_user[handphone]": "${model.model.handphone }",
  "existing_user[self_introduction]": "${model.model.self_introduction }",
  "existing_user[website]": "${model.model.website }",
  "existing_user[language_id]": "${model.model.language_id }",
  "existing_user[full_name]": "${model.model.full_name }",
  "existing_user[captcha]": "${model.model.captcha }",
  "existing_user[secure_code]": "${model.model.secure_code }",
  "existing_user[old_password]": "${model.model.old_password }",
  "existing_user[old_password_confirmation]": "${model.model.old_password }",
  "existing_user[new_email]": "${model.model.new_email }",
  "existing_user[your_email]": "${model.model.your_email }",
  "existing_user[invitation_message]": "${model.model.invitation_message }",
  "existing_user[financial_data]": "${model.model.financial_data }",
  "existing_user[bank_id]": "${model.model.bank_id }",
  "existing_user[branch]": "${model.model.branch }",
  "existing_user[city]": "${model.model.city }",
  "existing_user[account_number]": "${model.model.account_number }",
  "existing_user[account_name]": "${model.model.account_name }",
   "existing_user[cutoff_period_id]": "${model.model.cutoff_period_id}",
  "existing_user[cutoff_amount]": "${model.model.cutoff_amount}",
  "existing_user[deposit_amount]": "${model.model.deposit_amount}",
  "existing_user[statistics]": "${model.model.statistics }",
  "existing_user[money_earned]": "${model.model.money_earned}",
  "existing_user[money_spent]": "${model.model.money_spent}",
  "existing_user[available_balance]": "${model.model.available_balance}",
  "existing_user[withheld_balance]": "${model.model.withheld_balance}",
  "existing_user[admin_notes]": "${model.model.admin_notes }",
  "existing_user[pesta_point]": "${model.model.pesta_point }",
  "existing_user[worker_rating]": "${model.model.worker_rating }",
  "existing_user[worker_ranking]": "${model.model.worker_ranking }",
  "existing_user[worker_point]": "${model.model.worker_point }",
  "existing_user[worker_rating_num]": "${model.model.worker_rating_num}",
  "existing_user[worker_rating_sum]": "${model.model.worker_rating_sum }",
  "existing_user[worker_rating_div]": "${model.model.worker_rating_div }",
  "existing_user[owner_rating]": "${model.model.owner_rating }",
  "existing_user[owner_ranking]": "${model.model.owner_ranking }",
  "existing_user[owner_point]": "${model.model.owner_point }",
  "existing_user[owner_rating_num]": "${model.model.owner_rating_num}",
  "existing_user[owner_rating_sum]": "${model.model.owner_rating_sum }",
  "existing_user[owner_rating_div]": "${model.model.owner_rating_div }",
  "existing_user[seller_rating]": "${model.model.seller_rating }",
  "existing_user[seller_ranking]": "${model.model.seller_ranking }",
  "existing_user[seller_point]": "${model.model.seller_point }",
  "existing_user[seller_rating_num]": "${model.model.seller_rating_num}",
  "existing_user[seller_rating_sum]": "${model.model.seller_rating_sum }",
  "existing_user[seller_rating_div]": "${model.model.seller_rating_div }",
  "existing_user[buyer_rating]": "${model.model.buyer_rating }",
  "existing_user[buyer_ranking]": "${model.model.buyer_ranking }",
  "existing_user[buyer_point]": "${model.model.buyer_point }",
  "existing_user[buyer_rating_num]": "${model.model.buyer_rating_num}",
  "existing_user[buyer_rating_sum]": "${model.model.buyer_rating_sum }",
  "existing_user[buyer_rating_div]": "${model.model.buyer_rating_div }",
  "existing_user[affiliate_rating]": "${model.model.affiliate_rating }",
  "existing_user[affiliate_ranking]": "${model.model.affiliate_ranking }",
  "existing_user[affiliate_point]": "${model.model.affiliate_point }",
  "existing_user[affiliate_rating_num]": "${model.model.affiliate_rating_num}",
  "existing_user[affiliate_rating_sum]": "${model.model.affiliate_rating_sum }",
  "existing_user[affiliate_rating_div]": "${model.model.affiliate_rating_div }",
  "existing_user[projects_won]": "${model.model.projects_won }",
  "existing_user[projects_won_cumulative]": "${model.model.projects_won_cumulative}",
  "existing_user[projects_owned]": "${model.model.projects_owned }",
  "existing_user[projects_owned_cumulative]": "${model.model.projects_owned_cumulative}",
  "existing_user[projects_noaction]": "${model.model.projects_noaction }",
  "existing_user[projects_canceled]": "${model.model.projects_canceled }",
  "existing_user[missed_payment]": "${model.model.missed_payment }",
  "existing_user[projects_completed]": "${model.model.projects_completed }",
  "existing_user[projects_arbitrated]": "${model.model.projects_arbitrated }",
  "existing_user[projects_arbit_won]": "${model.model.projects_arbit_won }",
  "existing_user[projects_arbit_lost]": "${model.model.projects_arbit_lost }",
  "existing_user[current_projects]": "${model.model.current_projects }",
  "existing_user[missed_weekly_report]": "${model.model.missed_weekly_report }",
  "existing_user[product_sold]": "${model.model.product_sold }",
  "existing_user[product_sold_cumulative]": "${model.model.product_sold_cumulative}",
  "existing_user[product_bought]": "${model.model.product_bought }",
  "existing_user[product_bought_cumulative]": "${model.model.product_bought_cumulative}",
  "existing_user[bid_count]": "${model.model.bid_count }",
  "existing_user[post_count]": "${model.model.post_count }",
  "existing_user[work_quality]": "${model.model.work_quality }",
  "existing_user[work_quality_num]": "${model.model.work_quality_num}",
  "existing_user[work_quality_sum]": "${model.model.work_quality_sum }",
  "existing_user[work_quality_div]": "${model.model.work_quality_div }",
  "existing_user[expertise]": "${model.model.expertise }",
  "existing_user[expertise_num]": "${model.model.expertise_num}",
  "existing_user[expertise_sum]": "${model.model.expertise_sum }",
  "existing_user[expertise_div]": "${model.model.expertise_div }",
  "existing_user[worker_communication]": "${model.model.worker_communication }",
  "existing_user[worker_communication_num]": "${model.model.worker_communication_num}",
  "existing_user[worker_communication_sum]": "${model.model.worker_communication_sum }",
  "existing_user[worker_communication_div]": "${model.model.worker_communication_div }",
  "existing_user[worker_professionalism]": "${model.model.worker_professionalism }",
  "existing_user[worker_professionalism_num]": "${model.model.worker_professionalism_num}",
  "existing_user[worker_professionalism_sum]": "${model.model.worker_professionalism_sum }",
  "existing_user[worker_professionalism_div]": "${model.model.worker_professionalism_div }",
  "existing_user[clarity]": "${model.model.clarity }",
  "existing_user[clarity_num]": "${model.model.clarity_num}",
  "existing_user[clarity_sum]": "${model.model.clarity_sum }",
  "existing_user[clarity_div]": "${model.model.clarity_div }",
  "existing_user[friendliness]": "${model.model.friendliness }",
  "existing_user[friendliness_num]": "${model.model.friendliness_num}",
  "existing_user[friendliness_sum]": "${model.model.friendliness_sum }",
  "existing_user[friendliness_div]": "${model.model.friendliness_div }",
  "existing_user[owner_communication]": "${model.model.owner_communication }",
  "existing_user[owner_communication_num]": "${model.model.owner_communication_num}",
  "existing_user[owner_communication_sum]": "${model.model.owner_communication_sum }",
  "existing_user[owner_communication_div]": "${model.model.owner_communication_div }",
  "existing_user[owner_professionalism]": "${model.model.owner_professionalism }",
  "existing_user[owner_professionalism_num]": "${model.model.owner_professionalism_num}",
  "existing_user[owner_professionalism_sum]": "${model.model.owner_professionalism_sum }",
  "existing_user[owner_professionalism_div]": "${model.model.owner_professionalism_div }",
  "existing_user[product_quality]": "${model.model.product_quality }",
  "existing_user[product_quality_num]": "${model.model.product_quality_num}",
  "existing_user[product_quality_sum]": "${model.model.product_quality_sum }",
  "existing_user[product_quality_div]": "${model.model.product_quality_div }",
  "existing_user[service_quality]": "${model.model.service_quality }",
  "existing_user[service_quality_num]": "${model.model.service_quality_num}",
  "existing_user[service_quality_sum]": "${model.model.service_quality_sum }",
  "existing_user[service_quality_div]": "${model.model.service_quality_div }",
  "existing_user[seller_communication]": "${model.model.seller_communication }",
  "existing_user[seller_communication_num]": "${model.model.seller_communication_num}",
  "existing_user[seller_communication_sum]": "${model.model.seller_communication_sum }",
  "existing_user[seller_communication_div]": "${model.model.seller_communication_div }",
  "existing_user[seller_professionalism]": "${model.model.seller_professionalism }",
  "existing_user[seller_professionalism_num]": "${model.model.seller_professionalism_num}",
  "existing_user[seller_professionalism_sum]": "${model.model.seller_professionalism_sum }",
  "existing_user[seller_professionalism_div]": "${model.model.seller_professionalism_div }",
  "existing_user[referer_id]": "${model.model.referer_id }",
  "existing_user[referer_expires]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.referer_expires)}",
  "existing_user[referal_this_month]": "${model.model.referal_this_month }",
  "existing_user[cookies_this_month]": "${model.model.cookies_this_month }",
  "existing_user[premium_affiliate]": "${model.model.premium_affiliate ? 1 : 0}",
  "existing_user[refered_users]": "${model.model.refered_users }",
  "existing_user[referal_earnings]": "${model.model.referal_earnings}",
  "existing_user[affiliate_id]": "${model.model.affiliate_id }",
  "existing_user[last_announced_project]": "${model.model.last_announced_project }",
  "existing_user[notif_count]": "${model.model.notif_count }",
  "existing_user[notify_projects]": "${model.model.notify_projects ? 1 : 0}",
  "existing_user[lost_arbitration_recently]": "${model.model.lost_arbitration_recently ? 1 : 0}",
  "existing_user[coupon]": "${model.model.coupon }",
	  
  };  
      int count = 0;
      for(var tag in model.model.skills_id){
        formData.addAll({"existing_user[inline_tags][selection][$count]": "$tag"});
        count++;
      }
  ...
  var postExistingUserResult, State state, String?   sendPath, String?   id,  String?   title){
    ...
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(ButtonEditExistingUserWidget(model.buttons[i], context,formKey, controller,existing_user, postExistingUserResult, state, sendPath, id,  title ));
      }  
    }
       return(
	        buttonChildren 
  ...
  var postExistingUserResult, State state, String?   sendPath, String?   id,  String?   title ){
	...
	childMargin: const EdgeInsets.symmetric(horizontal: 18, vertical: 20),
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,existing_user, postExistingUserResult, state, sendPath, id,  title )
	    )
	 );
   } 

SpeedDialChild ButtonEditExistingUserWidget(Button button, BuildContext context,var formKey, ScrollController controller, ExistingUserController existing_user,
 var postExistingUserResult, State state, String?   sendPath, String?   id,  String?   title){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
	...
	return(	SpeedDialChild(
		  ...
		  onTap: ()async{
		  {
                        ...
                        formData,
						false);
                        final future = existing_user.postExistingUser();
                        future.then((value) {
                        state.setState(() {
                        postExistingUserResult = value;
						//AppProvider.getRouter(context)!.pop(context);	
...
class ExistingUserSuperBase {

  String?   id;
  Meta? meta;
  List<Button?>? buttons;
  ModelExistingUser? model;
  ExistingUserSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ExistingUserSuperBase.fromJson(Map<String, dynamic> json) => _$ExistingUserSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ExistingUserSuperBaseToJson(this);
...
class ExistingUserListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ExistingUserListingBase(this.json){
		items = ExistingUserListingItems.fromJson(this.json);
		tools = ExistingUserListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemExistingUserModel? item,String?   search, bool? account) {
	ShapeBorder? shape;
    double?  height = 0;
	...
	totalHeight = totalHeight + 4 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemExistingUserCard(destination :item, search : search, shape : shape, height : height)
    );
  ...
  Widget viewItemId(ItemExistingUserModel? item,String?   search, int? index, bool? account, String?   id) {
	ShapeBorder? shape;
    double?  height = 0;
	double?  totalHeight = 0;
	   totalHeight = totalHeight + 2 ;
	...
	totalHeight = totalHeight + 4 ;
	  ...
	  Widget viewItemIndex (ItemExistingUserModel? item,String?   search, int? index, bool? account) {
	ShapeBorder? shape;
    double?  height = 0;
	double?  totalHeight = 0;
	   totalHeight = totalHeight + 2 ;
...
Widget viewItem1 (ItemExistingUserModel? item,ItemExistingUserModel? item1, String?   search, BuildContext context,bool? account, String?   id , bool? isMax, int? max, int? index) {
	ShapeBorder? shape;
    double?  height = 0;
	double?  totalHeight = 0;
	    return Visibility (
	...
	Widget viewItemState (ItemExistingUserModel? item,String?   search, int? index, BuildContext context, bool? account, State st, VoidCallback onrefresh) {
	ShapeBorder? shape;
    double?  height = 0;
	double?  totalHeight = 0;
	   totalHeight = totalHeight + 2 ;
   ...
   // if(tools.buttons[i].text != "Table View"){
      ...
      buttonChildren.add(ListButtonExistingUserWidget(tools.buttons[i],context, account));
   // }  
    }
...
class ExistingUserListingItems{
  List<ItemExistingUserModel?>? items;
  ExistingUserListingItems(
      this.items,
      );

  factory ExistingUserListingItems.fromJson(Map<String, dynamic> json) {
  	return ExistingUserListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemExistingUserModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ExistingUserListingTools{

	String?   id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemExistingUser?>? items;
	ExistingUserListingTools(
			this.id,
			this.buttons,
			this.meta,
			this.paging,
	...
	factory ExistingUserListingTools.fromJson(Map<String, dynamic> json) => _$ExistingUserListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ExistingUserListingToolsToJson(this);
...
class ItemExistingUserCard extends StatelessWidget {
  const ItemExistingUserCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double?  height ;
  final ItemExistingUserModel? destination;
  final String?   search;
            ...
            Container(
             // height: height,
              child: Card(
                ...
                shape: shape,
                child: ItemExistingUserContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemExistingUserContent extends StatelessWidget {
  const ItemExistingUserContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> buttonChildren = <Widget>[
	  ...
	  if (destination!.item.buttons != null) {
		...
		{
		  buttonChildren.add(itemExistingUserButton(destination,i , context));
		}
    }
...
Widget itemExistingUserButton(ItemExistingUserModel? destination, int? index, BuildContext context){
        ...
        style: ButtonStyle(
			  ...
			  onPressed: () {
		  {
                       ...
                       // duration: Duration(milliseconds: 1000), curve: Curves.easeInOutCirc);
                        ...
                        null,
						false);
                        final future = existing_user.downloadFile();
                        future.then((value) {
                       // state.setState(() {
                       // postExistingUserResult = value;
						//AppProvider.getRouter(context)!.pop(context);	
...
class ItemExistingUserWidget extends StatelessWidget {

	final ItemExistingUserModel? item;
	final String?   search;
	const  ItemExistingUserWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonExistingUserWidget(Button button, BuildContext context, bool? account){
    var cl;
	var ic;
  	if (button.color == 'green'){
  		cl = Colors.green;
