...
import 'product_comments_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'product_comments_list_base.g.dart';


class ProductCommentsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ProductCommentsListingBase(this.json){
		items = ProductCommentsListingItems.fromJson(this.json);
		tools = ProductCommentsListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemProductCommentsModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  height = totalHeight * 50;
        ...
        child:  ItemProductCommentsCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemProductCommentsModel item,ItemProductCommentsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonProductCommentsWidget(tools.buttons[i],context));
    }
...
class ProductCommentsListingItems{
  List<ItemProductCommentsModel?>? items;
  ProductCommentsListingItems(
      this.items,
      );

  factory ProductCommentsListingItems.fromJson(Map<String, dynamic> json) {
  	return ProductCommentsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemProductCommentsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ProductCommentsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemProductComments?>? items;
	...
	factory ProductCommentsListingTools.fromJson(Map<String, dynamic> json) => _$ProductCommentsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ProductCommentsListingToolsToJson(this);
...
class ItemProductCommentsCard extends StatelessWidget {
  const ItemProductCommentsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemProductCommentsModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemProductCommentsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemProductCommentsContent extends StatelessWidget {
  const ItemProductCommentsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemProductCommentsButton(ItemProductCommentsModel destination, int? index, BuildContext context){
...
class ItemProductCommentsWidget extends StatelessWidget {

	final ItemProductCommentsModel? item;
	final String? search;
	const  ItemProductCommentsWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonProductCommentsWidget(Button button, BuildContext context){
