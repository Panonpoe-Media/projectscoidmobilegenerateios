...
import '../photo.dart';
import '../paging.dart';
part 'product_reviews_list_base.g.dart';


class ProductReviewsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ProductReviewsListingBase(this.json){
		items = ProductReviewsListingItems.fromJson(this.json);
		tools = ProductReviewsListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemProductReviewsModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemProductReviewsCard(destination :item, search : search, shape : shape, height: height)
    );
  ...
  Widget viewItem1 (ItemProductReviewsModel item,ItemProductReviewsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonProductReviewsWidget(tools.buttons[i],context));
    }
...
class ProductReviewsListingItems{
  List<ItemProductReviewsModel?>? items;
  ProductReviewsListingItems(
      this.items,
      );

  factory ProductReviewsListingItems.fromJson(Map<String, dynamic> json) {
  	return ProductReviewsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemProductReviewsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ProductReviewsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemProductReviews?>? items;
	...
	factory ProductReviewsListingTools.fromJson(Map<String, dynamic> json) => _$ProductReviewsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ProductReviewsListingToolsToJson(this);
...
class ItemProductReviewsCard extends StatelessWidget {
  const ItemProductReviewsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemProductReviewsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemProductReviewsContent extends StatelessWidget {
  const ItemProductReviewsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemProductReviewsButton(ItemProductReviewsModel destination, int? index, BuildContext context){
...
class ItemProductReviewsWidget extends StatelessWidget {

	final ItemProductReviewsModel? item;
	final String? search;
	const  ItemProductReviewsWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonProductReviewsWidget(Button button, BuildContext context){
