...
part 'product_comments_edit_base.g.dart';
...
Widget RButtonActionProductCommentsWidget(Button button, BuildContext context,var formKey, ScrollController controller,  SubModelController product_comments,
 var postProductCommentsResult, State state, String? sendPath, String? id,  String? title,  var formData){
  var cl;
  var ic;
		 ...
		 children: <Widget>[
             ...
             // color : Color(0xFF037f51),
              onPressed: () async  {
                        ...
                        product_comments =      SubModelController(AppProvider.getApplication(context),
                        sendPath,
                        formData);
...
SpeedDialChild ButtonEditProductCommentsWidget(Button button, BuildContext context,var formKey, ScrollController controller, SubModelController product_comments,
 var postProductCommentsResult, State state, String? sendPath, String? id,  String? title, var formData){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
...
class EditModelProductComments {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
  ...
  String? captcha;
    ...
    factory EditModelProductComments.fromJson(Map<String, dynamic> json) => _$EditModelProductCommentsFromJson(json);

  Map<String, dynamic> toJson() => _$EditModelProductCommentsToJson(this);
...
class ProductCommentsSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  EditModelProductComments? model;
  ...
  factory ProductCommentsSuperBase.fromJson(Map<String, dynamic> json) => _$ProductCommentsSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ProductCommentsSuperBaseToJson(this);
...
class EditProductCommentsBase{
	var model;
	Map<String, dynamic> json;
	EditProductCommentsBase(this.json){
		model = ProductCommentsSuperBase.fromJson(this.json);
	}
	
   Map<String, dynamic> convertFormDataEdit(var data){
      var formData = {
      "product_comments[_trigger_]":"",
  "product_comments[user_id]": "${model.model.user_id }",
  "product_comments[product_id]": "${model.model.product_id }",
  "product_comments[thread_id]": "${model.model.thread_id }",
   "product_comments[post_type_id]": "${model.model.post_type_id}",
  "product_comments[post_date]": "${model.model.post_date}",
  "product_comments[message]": "${model.model.message }",
  "product_comments[attachments]": '[{"name":"${model.model.attachments[0].name}","size":${model.model.attachments[0].size.toString()},"created":${model.model.attachments[0].date.toString()},"modified":${model.model.attachments[0].date.toString()},"temp":"${model.model.attachments[0].temp}","remote":"","dir":"temp"}]',
  "product_comments[read_by_seller]": "${model.model.read_by_seller}",
  "product_comments[read_by_buyer]": "${model.model.read_by_buyer}",
  "product_comments[captcha]": "${model.model.captcha }",
  ...
  var formData = convertFormDataEdit(model);  
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionProductCommentsWidget(model.buttons[i], context,formKey, controller,product_comments, postProductCommentsResult, state, sendPath, id,  title, formData));
      }
    }
       return(
	        buttonChildren 
  ...
  var postProductCommentsResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonEditProductCommentsWidget(model.buttons[i], context,formKey, controller,product_comments, postProductCommentsResult, state, sendPath, id,  title, formData ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,product_comments, postProductCommentsResult, state, sendPath, id,  title )
  ...
  var postProductCommentsResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,product_comments, postProductCommentsResult, state, sendPath, id,  title )
	    )
	 );
   } 	
