...
import 'cart_items_index_item.dart';
import 'cart_items_index_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'cart_items_index_base.g.dart';


class CartItemsIndexBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	CartItemsIndexBase(this.json){
		items = CartItemsIndexItems.fromJson(this.json);
		tools = CartItemsIndexTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemIndexCartItemsModel item,String? search) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemIndexCartItemsCard(destination :item, search : search, shape : shape, height : height)
    );
   ...
   List<SpeedDialChild>	 listButton(BuildContext context){
    ...
    {
      buttonChildren.add(IndexButtonCartItemsWidget(tools.buttons[i],context));
    }
...
class CartItemsIndexItems{
  List<ItemIndexCartItemsModel?>? items;
  CartItemsIndexItems(
      this.items,
      );

  factory CartItemsIndexItems.fromJson(Map<String, dynamic> json) {
  	return CartItemsIndexItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemIndexCartItemsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class CartItemsIndexTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemIndexCartItems?>? items;
	CartItemsIndexTools(
			this.id,
			this.buttons,
			this.meta,
	...
	factory CartItemsIndexTools.fromJson(Map<String, dynamic> json) => _$CartItemsIndexToolsFromJson(json);

	Map<String, dynamic> toJson() => _$CartItemsIndexToolsToJson(this);
...
class ItemIndexCartItemsCard extends StatelessWidget {
  const ItemIndexCartItemsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemIndexCartItemsModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            SizedBox(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemIndexCartItemsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemIndexCartItemsContent extends StatelessWidget {
  const ItemIndexCartItemsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemCartItemsButton(ItemIndexCartItemsModel destination, int index, BuildContext context){
...
class ItemIndexCartItemsWidget extends StatelessWidget {

	final ItemIndexCartItemsModel?  item;
	final String? search;
	const  ItemIndexCartItemsWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild IndexButtonCartItemsWidget(Button button, BuildContext context){
