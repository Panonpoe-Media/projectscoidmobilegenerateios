...
import 'checkout_items_list_item.dart';
import 'checkout_items_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'checkout_items_list_base.g.dart';


class CheckoutItemsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	CheckoutItemsListingBase(this.json){
		items = CheckoutItemsListingItems.fromJson(this.json);
		tools = CheckoutItemsListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemCheckoutItemsModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemCheckoutItemsCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemCheckoutItemsModel item,ItemCheckoutItemsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonCheckoutItemsWidget(tools.buttons[i],context));
    }
...
class CheckoutItemsListingItems{
  List<ItemCheckoutItemsModel?>? items;
  CheckoutItemsListingItems(
      this.items,
      );

  factory CheckoutItemsListingItems.fromJson(Map<String, dynamic> json) {
  	return CheckoutItemsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemCheckoutItemsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class CheckoutItemsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemCheckoutItems?>? items;
	...
	factory CheckoutItemsListingTools.fromJson(Map<String, dynamic> json) => _$CheckoutItemsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$CheckoutItemsListingToolsToJson(this);
...
class ItemCheckoutItemsCard extends StatelessWidget {
  const ItemCheckoutItemsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemCheckoutItemsModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemCheckoutItemsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemCheckoutItemsContent extends StatelessWidget {
  const ItemCheckoutItemsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemCheckoutItemsButton(destination!,i , context));
		}
    }
...
Widget itemCheckoutItemsButton(ItemCheckoutItemsModel destination, int? index, BuildContext context){
...
class ItemCheckoutItemsWidget extends StatelessWidget {

	final ItemCheckoutItemsModel? item;
	final String? search;
	const  ItemCheckoutItemsWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonCheckoutItemsWidget(Button button, BuildContext context){
