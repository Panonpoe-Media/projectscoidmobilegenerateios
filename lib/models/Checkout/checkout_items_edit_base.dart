...
part 'checkout_items_edit_base.g.dart';


Widget RButtonActionCheckoutItemsWidget(Button button, BuildContext context,var formKey, ScrollController controller,  SubModelController checkout_items,
 var postCheckoutItemsResult, State state, String? sendPath, String? id,  String? title,  var formData){
  var cl;
  var ic;
		 ...
		 children: <Widget>[
             ...
             // color : Color(0xFF037f51),
              onPressed: () async  {
                        ...
                        checkout_items =      SubModelController(AppProvider.getApplication(context),
                        sendPath,
                        formData);
                        final future = checkout_items.sendData();
                        future.then((value) {
                        state.setState(() {
                        postCheckoutItemsResult = value;
                        });
                        });
...
SpeedDialChild ButtonEditCheckoutItemsWidget(Button button, BuildContext context,var formKey, ScrollController controller, SubModelController checkout_items,
 var postCheckoutItemsResult, State state, String? sendPath, String? id,  String? title, var formData){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
...
class EditModelCheckoutItems {
  ...
  String? total_price_str;
    ...
    factory EditModelCheckoutItems.fromJson(Map<String, dynamic> json) => _$EditModelCheckoutItemsFromJson(json);

  Map<String, dynamic> toJson() => _$EditModelCheckoutItemsToJson(this);
...
class CheckoutItemsSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  EditModelCheckoutItems? model;
  CheckoutItemsSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory CheckoutItemsSuperBase.fromJson(Map<String, dynamic> json) => _$CheckoutItemsSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$CheckoutItemsSuperBaseToJson(this);
...
class EditCheckoutItemsBase{
	var model;
	Map<String, dynamic> json;
	EditCheckoutItemsBase(this.json){
		model = CheckoutItemsSuperBase.fromJson(this.json);
	}
	
   Map<String, dynamic> convertFormDataEdit(var data){
      var formData = {
      "checkout_items[_trigger_]":"",
  "checkout_items[user_id]": "${model.model.user_id }",
   "checkout_items[type_id]": "${model.model.type_id}",
  "checkout_items[title]": "${model.model.title }",
  "checkout_items[product_id]": "${model.model.product_id }",
  "checkout_items[project_id]": "${model.model.project_id }",
  "checkout_items[service_id]": "${model.model.service_id }",
  "checkout_items[bid_id]": "${model.model.bid_id }",
  "checkout_items[iklan_id]": "${model.model.iklan_id }",
  "checkout_items[seller_id]": "${model.model.seller_id }",
  "checkout_items[item_price]": "${model.model.item_price}",
  "checkout_items[quantity]": "${model.model.quantity }",
  "checkout_items[total_price]": "${model.model.total_price}",
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, SubModelController checkout_items,
  var postCheckoutItemsResult, State state, String? sendPath, String? id,  String? title){
  var formData = convertFormDataEdit(model);  
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionCheckoutItemsWidget(model.buttons[i], context,formKey, controller,checkout_items, postCheckoutItemsResult, state, sendPath, id,  title, formData));
      }
    }
       return(
	        buttonChildren 
  ...
  List<SpeedDialChild> listButton(BuildContext context,var formKey, ScrollController controller, SubModelController checkout_items,
  var postCheckoutItemsResult, State state, String? sendPath, String? id,  String? title){
	var formData = convertFormDataEdit(model);   
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonEditCheckoutItemsWidget(model.buttons[i], context,formKey, controller,checkout_items, postCheckoutItemsResult, state, sendPath, id,  title, formData ));
    }
       return(
	        buttonChildren 
	   );
       ...
       Widget	 RButtons(BuildContext context, bool? visible, var formKey, ScrollController controller, SubModelController checkout_items,
  var postCheckoutItemsResult, State state, String? sendPath, String? id,  String? title ){
    //  final size =MediaQuery.of(context).size;
    double? width = 400;
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,checkout_items, postCheckoutItemsResult, state, sendPath, id,  title )
    ...
    SpeedDial	 Buttons(BuildContext context, bool? visible, var formKey, ScrollController controller, SubModelController checkout_items,
  var postCheckoutItemsResult, State state, String? sendPath, String? id,  String? title ){
     return(
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,checkout_items, postCheckoutItemsResult, state, sendPath, id,  title )
	    )
	 );
   } 	
