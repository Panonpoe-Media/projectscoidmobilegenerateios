...
import 'referals_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'referals_list_base.g.dart';


class ReferalsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ReferalsListingBase(this.json){
		items = ReferalsListingItems.fromJson(this.json);
		tools = ReferalsListingTools.fromJson(this.json);
	}
  ...
  Widget viewItem (ItemReferalsModel item,String? search, bool? account, String? id) {
	...
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemReferalsCard(destination :item, search : search, shape : shape, height: height)
    );
  ...
  Widget viewItem1 (ItemReferalsModel item,ItemReferalsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonReferalsWidget(tools.buttons[i],context));
    }
...
class ReferalsListingItems{
  List<ItemReferalsModel?>? items;
  ReferalsListingItems(
      this.items,
      );

  factory ReferalsListingItems.fromJson(Map<String, dynamic> json) {
  	return ReferalsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemReferalsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ReferalsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemReferals?>? items;
	ReferalsListingTools(
			this.id,
			this.buttons,
			this.meta,
			this.paging,
	...
	factory ReferalsListingTools.fromJson(Map<String, dynamic> json) => _$ReferalsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ReferalsListingToolsToJson(this);
...
class ItemReferalsCard extends StatelessWidget {
  const ItemReferalsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemReferalsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemReferalsContent extends StatelessWidget {
  const ItemReferalsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemReferalsButton(ItemReferalsModel destination, int? index, BuildContext context){
...
class ItemReferalsWidget extends StatelessWidget {

	final ItemReferalsModel? item;
	final String? search;
	const  ItemReferalsWidget({Key? key, @required this.item, this.search}) : super(key: key);
