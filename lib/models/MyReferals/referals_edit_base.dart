...
part 'referals_edit_base.g.dart';
...
Widget RButtonActionReferalsWidget(Button button, BuildContext context,var formKey, ScrollController controller,  SubModelController referals,
...
class EditModelReferals {
  ...
  String? coupon;
    ...
    factory EditModelReferals.fromJson(Map<String, dynamic> json) => _$EditModelReferalsFromJson(json);

  Map<String, dynamic> toJson() => _$EditModelReferalsToJson(this);
...
class EditReferalsBase{
	var model;
	Map<String, dynamic> json;
	EditReferalsBase(this.json){
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, SubModelController referals,
  ...
  var formData = convertFormDataEdit(model);  
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionReferalsWidget(model.buttons[i], context,formKey, controller,referals, postReferalsResult, state, sendPath, id,  title, formData));
      }
    }
       return(
	        buttonChildren 
  ...
  var postReferalsResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonEditReferalsWidget(model.buttons[i], context,formKey, controller,referals, postReferalsResult, state, sendPath, id,  title, formData ));
       ...
       Widget	 RButtons(BuildContext context, bool? visible, var formKey, ScrollController controller, SubModelController referals,
  var postReferalsResult, State state, String? sendPath, String? id,  String? title ){
    //  final size =MediaQuery.of(context).size;
    double? width = 400;
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,referals, postReferalsResult, state, sendPath, id,  title )
