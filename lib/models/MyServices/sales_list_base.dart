...
import 'sales_list_item.dart';
import 'sales_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'sales_list_base.g.dart';


class SalesListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	SalesListingBase(this.json){
		items = SalesListingItems.fromJson(this.json);
		tools = SalesListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemSalesModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  height = totalHeight * 50;
        ...
        child:  ItemSalesCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemSalesModel item,ItemSalesModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonSalesWidget(tools.buttons[i],context));
    }
...
class SalesListingItems{
  List<ItemSalesModel?>? items;
  SalesListingItems(
      this.items,
      );

  factory SalesListingItems.fromJson(Map<String, dynamic> json) {
  	return SalesListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemSalesModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
class SalesListingTools{
	...
	factory SalesListingTools.fromJson(Map<String, dynamic> json) => _$SalesListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$SalesListingToolsToJson(this);
...
class ItemSalesCard extends StatelessWidget {
  const ItemSalesCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemSalesModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemSalesContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemSalesContent extends StatelessWidget {
  const ItemSalesContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemSalesButton(ItemSalesModel destination, int? index, BuildContext context){
...
class ItemSalesWidget extends StatelessWidget {
	...
	final ItemSalesModel? item;
	final String? search;
	const  ItemSalesWidget({Key? key, @required this.item, this.search}) : super(key: key);
