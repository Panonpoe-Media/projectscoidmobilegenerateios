...
part 'screenshots_edit_image_base.g.dart';



@JsonSerializable()

class ModelActionEditImageScreenshots {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? service_images_id;
  ...
  String? description;
    ...
    factory ModelActionEditImageScreenshots.fromJson(Map<String, dynamic> json) => _$ModelActionEditImageScreenshotsFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionEditImageScreenshotsToJson(this);

}

@JsonSerializable()
class EditImageScreenshotsSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionEditImageScreenshots? model;
  ...
  factory EditImageScreenshotsSuperBase.fromJson(Map<String, dynamic> json) => _$EditImageScreenshotsSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$EditImageScreenshotsSuperBaseToJson(this);
  
 
}

class EditImageScreenshotsBase{
	var model;
	Map<String, dynamic> json;
            var imagelast ;
	EditImageScreenshotsBase(this.json){
		model = EditImageScreenshotsSuperBase.fromJson(this.json);
...
SpeedDialChild ButtonActionScreenshotsWidget(Button button, BuildContext context,var formKey, ScrollController controller, SubModelController screenshots,
 var postScreenshotsResult, State state, String? sendPath, String? id,  String? title){
  ...
  var postEditImageResult, State state, String? sendPath, String? id,  String? title){
    final List<Widget> buttonChildren = <Widget>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(RButtonActionScreenshotsWidget(model.buttons[i], context,formKey, controller,screenshots, postEditImageResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postEditImageResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionScreenshotsWidget(model.buttons[i], context,formKey, controller,screenshots, postEditImageResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,screenshots, postEditImageResult, state, sendPath, id,  title )
  ...
  var postEditImageResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,screenshots, postEditImageResult, state, sendPath, id,  title )
	    )
	 );
   } 
		...
		return(  ModelView(
			...
			nameenum: model.model.service_list_str ?? model.model.service_str ,
		));} 
