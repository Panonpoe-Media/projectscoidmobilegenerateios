...
import 'screenshots_list_item.dart';
import 'screenshots_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'screenshots_list_base.g.dart';


class ScreenshotsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ScreenshotsListingBase(this.json){
		items = ScreenshotsListingItems.fromJson(this.json);
		tools = ScreenshotsListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemScreenshotsModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 8 ;
	  height = totalHeight * 50;
        ...
        child:  ItemScreenshotsCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemScreenshotsModel item,ItemScreenshotsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonScreenshotsWidget(tools.buttons[i],context));
    }
...
class ScreenshotsListingItems{
  List<ItemScreenshotsModel?>? items;
  ScreenshotsListingItems(
      this.items,
      );

  factory ScreenshotsListingItems.fromJson(Map<String, dynamic> json) {
  	return ScreenshotsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemScreenshotsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ScreenshotsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemScreenshots?>? items;
	...
	factory ScreenshotsListingTools.fromJson(Map<String, dynamic> json) => _$ScreenshotsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ScreenshotsListingToolsToJson(this);
...
class ItemScreenshotsCard extends StatelessWidget {
  const ItemScreenshotsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemScreenshotsModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemScreenshotsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemScreenshotsContent extends StatelessWidget {
  const ItemScreenshotsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemScreenshotsButton(destination!,i , context));
		}
    }
...
Widget itemScreenshotsButton(ItemScreenshotsModel destination, int? index, BuildContext context){
...
class ItemScreenshotsWidget extends StatelessWidget {

	final ItemScreenshotsModel? item;
	final String? search;
	const  ItemScreenshotsWidget({Key? key, @required this.item, this.search}) : super(key: key);
