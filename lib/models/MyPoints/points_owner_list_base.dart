...
import 'points_owner_list_item.dart';
import 'points_owner_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'points_owner_list_base.g.dart';


class PointsOwnerListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	PointsOwnerListingBase(this.json){
		items = PointsOwnerListingItems.fromJson(this.json);
		tools = PointsOwnerListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemPointsOwnerModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemPointsOwnerCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemPointsOwnerModel item,ItemPointsOwnerModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonPointsOwnerWidget(tools.buttons[i],context));
    }
...
class PointsOwnerListingItems{
  List<ItemPointsOwnerModel?>? items;
  PointsOwnerListingItems(
      this.items,
      );

  factory PointsOwnerListingItems.fromJson(Map<String, dynamic> json) {
  	return PointsOwnerListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemPointsOwnerModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class PointsOwnerListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemPointsOwner?>? items;
	PointsOwnerListingTools(
			this.id,
			this.buttons,
			this.meta,
			this.paging,
	...
	factory PointsOwnerListingTools.fromJson(Map<String, dynamic> json) => _$PointsOwnerListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$PointsOwnerListingToolsToJson(this);
...
class ItemPointsOwnerCard extends StatelessWidget {
  const ItemPointsOwnerCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemPointsOwnerModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemPointsOwnerContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemPointsOwnerContent extends StatelessWidget {
  const ItemPointsOwnerContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemPointsOwnerButton(destination!,i , context));
		}
    }
...
Widget itemPointsOwnerButton(ItemPointsOwnerModel destination, int? index, BuildContext context){
...
class ItemPointsOwnerWidget extends StatelessWidget {

	final ItemPointsOwnerModel? item;
	final String? search;
	const  ItemPointsOwnerWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonPointsOwnerWidget(Button button, BuildContext context){
