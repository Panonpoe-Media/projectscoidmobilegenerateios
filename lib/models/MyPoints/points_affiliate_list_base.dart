...
import 'points_affiliate_list_item.dart';
import 'points_affiliate_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'points_affiliate_list_base.g.dart';


class PointsAffiliateListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	PointsAffiliateListingBase(this.json){
		items = PointsAffiliateListingItems.fromJson(this.json);
		tools = PointsAffiliateListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemPointsAffiliateModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemPointsAffiliateCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemPointsAffiliateModel item,ItemPointsAffiliateModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonPointsAffiliateWidget(tools.buttons[i],context));
    }
...
class PointsAffiliateListingItems{
  List<ItemPointsAffiliateModel?>? items;
  PointsAffiliateListingItems(
      this.items,
      );

  factory PointsAffiliateListingItems.fromJson(Map<String, dynamic> json) {
  	return PointsAffiliateListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemPointsAffiliateModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class PointsAffiliateListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemPointsAffiliate?>? items;
	...
	factory PointsAffiliateListingTools.fromJson(Map<String, dynamic> json) => _$PointsAffiliateListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$PointsAffiliateListingToolsToJson(this);
...
class ItemPointsAffiliateCard extends StatelessWidget {
  const ItemPointsAffiliateCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemPointsAffiliateModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemPointsAffiliateContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemPointsAffiliateContent extends StatelessWidget {
  const ItemPointsAffiliateContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemPointsAffiliateButton(destination!,i , context));
		}
    }
...
Widget itemPointsAffiliateButton(ItemPointsAffiliateModel destination, int? index, BuildContext context){
...
class ItemPointsAffiliateWidget extends StatelessWidget {

	final ItemPointsAffiliateModel? item;
	final String? search;
	const  ItemPointsAffiliateWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonPointsAffiliateWidget(Button button, BuildContext context){
