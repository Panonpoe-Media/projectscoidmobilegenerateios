...
import 'points_buyer_list_item.dart';
import 'points_buyer_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'points_buyer_list_base.g.dart';


class PointsBuyerListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	PointsBuyerListingBase(this.json){
		items = PointsBuyerListingItems.fromJson(this.json);
		tools = PointsBuyerListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemPointsBuyerModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemPointsBuyerCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemPointsBuyerModel item,ItemPointsBuyerModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonPointsBuyerWidget(tools.buttons[i],context));
    }
...
class PointsBuyerListingItems{
  List<ItemPointsBuyerModel?>? items;
  PointsBuyerListingItems(
      this.items,
      );

  factory PointsBuyerListingItems.fromJson(Map<String, dynamic> json) {
  	return PointsBuyerListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemPointsBuyerModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class PointsBuyerListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemPointsBuyer?>? items;
	...
	factory PointsBuyerListingTools.fromJson(Map<String, dynamic> json) => _$PointsBuyerListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$PointsBuyerListingToolsToJson(this);
...
class ItemPointsBuyerCard extends StatelessWidget {
  const ItemPointsBuyerCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemPointsBuyerModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemPointsBuyerContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemPointsBuyerContent extends StatelessWidget {
  const ItemPointsBuyerContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemPointsBuyerButton(destination!,i , context));
		}
    }
...
Widget itemPointsBuyerButton(ItemPointsBuyerModel destination, int? index, BuildContext context){
...
class ItemPointsBuyerWidget extends StatelessWidget {

	final ItemPointsBuyerModel? item;
	final String? search;
	const  ItemPointsBuyerWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonPointsBuyerWidget(Button button, BuildContext context){
