...
import 'points_seller_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'points_seller_list_base.g.dart';


class PointsSellerListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	PointsSellerListingBase(this.json){
		items = PointsSellerListingItems.fromJson(this.json);
		tools = PointsSellerListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemPointsSellerModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemPointsSellerCard(destination :item, search : search, shape : shape, height: height)
    );
  ...
  Widget viewItem1 (ItemPointsSellerModel item,ItemPointsSellerModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonPointsSellerWidget(tools.buttons[i],context));
    }
...
class PointsSellerListingItems{
  List<ItemPointsSellerModel?>? items;
  PointsSellerListingItems(
      this.items,
      );

  factory PointsSellerListingItems.fromJson(Map<String, dynamic> json) {
  	return PointsSellerListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemPointsSellerModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class PointsSellerListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemPointsSeller?>? items;
	...
	factory PointsSellerListingTools.fromJson(Map<String, dynamic> json) => _$PointsSellerListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$PointsSellerListingToolsToJson(this);
...
class ItemPointsSellerCard extends StatelessWidget {
  const ItemPointsSellerCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemPointsSellerContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemPointsSellerContent extends StatelessWidget {
  const ItemPointsSellerContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemPointsSellerButton(ItemPointsSellerModel destination, int? index, BuildContext context){
...
class ItemPointsSellerWidget extends StatelessWidget {

	final ItemPointsSellerModel? item;
	final String? search;
	const  ItemPointsSellerWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonPointsSellerWidget(Button button, BuildContext context){
