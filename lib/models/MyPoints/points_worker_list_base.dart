...
import 'points_worker_list_item.dart';
import 'points_worker_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'points_worker_list_base.g.dart';


class PointsWorkerListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	PointsWorkerListingBase(this.json){
		items = PointsWorkerListingItems.fromJson(this.json);
		tools = PointsWorkerListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemPointsWorkerModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemPointsWorkerCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemPointsWorkerModel item,ItemPointsWorkerModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonPointsWorkerWidget(tools.buttons[i],context));
    }
...
class PointsWorkerListingItems{
  List<ItemPointsWorkerModel?>? items;
  PointsWorkerListingItems(
      this.items,
      );

  factory PointsWorkerListingItems.fromJson(Map<String, dynamic> json) {
  	return PointsWorkerListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemPointsWorkerModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class PointsWorkerListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemPointsWorker?>? items;
	...
	factory PointsWorkerListingTools.fromJson(Map<String, dynamic> json) => _$PointsWorkerListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$PointsWorkerListingToolsToJson(this);
...
class ItemPointsWorkerCard extends StatelessWidget {
  const ItemPointsWorkerCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemPointsWorkerModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemPointsWorkerContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemPointsWorkerContent extends StatelessWidget {
  const ItemPointsWorkerContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemPointsWorkerButton(destination!,i , context));
		}
    }
...
Widget itemPointsWorkerButton(ItemPointsWorkerModel destination, int? index, BuildContext context){
...
class ItemPointsWorkerWidget extends StatelessWidget {

	final ItemPointsWorkerModel? item;
	final String? search;
	const  ItemPointsWorkerWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonPointsWorkerWidget(Button button, BuildContext context){
