...
import '../photo.dart';
import '../paging.dart';
part 'user_bids_list_base.g.dart';


class UserBidsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	UserBidsListingBase(this.json){
		items = UserBidsListingItems.fromJson(this.json);
		tools = UserBidsListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemUserBidsModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemUserBidsCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemUserBidsModel item,ItemUserBidsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonUserBidsWidget(tools.buttons[i],context));
    }
...
class UserBidsListingItems{
  List<ItemUserBidsModel?>? items;
  UserBidsListingItems(
      this.items,
      );

  factory UserBidsListingItems.fromJson(Map<String, dynamic> json) {
  	return UserBidsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemUserBidsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class UserBidsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemUserBids?>? items;
	...
	factory UserBidsListingTools.fromJson(Map<String, dynamic> json) => _$UserBidsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$UserBidsListingToolsToJson(this);
...
class ItemUserBidsCard extends StatelessWidget {
  const ItemUserBidsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemUserBidsModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemUserBidsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemUserBidsContent extends StatelessWidget {
  const ItemUserBidsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemUserBidsButton(ItemUserBidsModel destination, int? index, BuildContext context){
...
class ItemUserBidsWidget extends StatelessWidget {

	final ItemUserBidsModel? item;
	final String? search;
	const  ItemUserBidsWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonUserBidsWidget(Button button, BuildContext context){
