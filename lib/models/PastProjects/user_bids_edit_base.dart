...
part 'user_bids_edit_base.g.dart';


Widget RButtonActionUserBidsWidget(Button button, BuildContext context,var formKey, ScrollController controller,  SubModelController user_bids,
 var postUserBidsResult, State state, String? sendPath, String? id,  String? title,  var formData){
  var cl;
  var ic;
		 ...
		 children: <Widget>[
             ...
             // color : Color(0xFF037f51),
              onPressed: () async  {
                        ...
                        user_bids =      SubModelController(AppProvider.getApplication(context),
                        sendPath,
                        formData);
                        final future = user_bids.sendData();
                        future.then((value) {
                        state.setState(() {
                        postUserBidsResult = value;
                        });
                        });
...
SpeedDialChild ButtonEditUserBidsWidget(Button button, BuildContext context,var formKey, ScrollController controller, SubModelController user_bids,
 var postUserBidsResult, State state, String? sendPath, String? id,  String? title, var formData){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
...
class EditModelUserBids {
  ...
  String? published_budget;
    ...
    factory EditModelUserBids.fromJson(Map<String, dynamic> json) => _$EditModelUserBidsFromJson(json);

  Map<String, dynamic> toJson() => _$EditModelUserBidsToJson(this);
...
class UserBidsSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  EditModelUserBids? model;
  UserBidsSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
  ...
  factory UserBidsSuperBase.fromJson(Map<String, dynamic> json) => _$UserBidsSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$UserBidsSuperBaseToJson(this);
...
class EditUserBidsBase{
	var model;
	Map<String, dynamic> json;
	EditUserBidsBase(this.json){
		model = UserBidsSuperBase.fromJson(this.json);
	}
  ...
  "user_bids[rating]": "${model.model.rating }",
  "user_bids[matching_skills]": "${model.model.matching_skills }",
  "user_bids[amount]": "${model.model.amount}",
  "user_bids[message]": "${model.model.message }",
  "user_bids[attachments]": '[{"name":"${model.model.attachments[0].name}","size":${model.model.attachments[0].size.toString()},"created":${model.model.attachments[0].date.toString()},"modified":${model.model.attachments[0].date.toString()},"temp":"${model.model.attachments[0].temp}","remote":"","dir":"temp"}]',
  "user_bids[read_by_owner]": "${model.model.read_by_owner}",
  "user_bids[shortlisted]": "${model.model.shortlisted}",
  "user_bids[captcha]": "${model.model.captcha }",
  "user_bids[project_title]": "${model.model.project_title }",
  "user_bids[project_owner]": "${model.model.project_owner }",
  "user_bids[description]": "${model.model.description }",
  "user_bids[published_budget]": "${model.model.published_budget }",
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, SubModelController user_bids,
  var postUserBidsResult, State state, String? sendPath, String? id,  String? title){
  var formData = convertFormDataEdit(model);  
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionUserBidsWidget(model.buttons[i], context,formKey, controller,user_bids, postUserBidsResult, state, sendPath, id,  title, formData));
      }
    }
       return(
	        buttonChildren 
  ...
  List<SpeedDialChild> listButton(BuildContext context,var formKey, ScrollController controller, SubModelController user_bids,
  var postUserBidsResult, State state, String? sendPath, String? id,  String? title){
	var formData = convertFormDataEdit(model);   
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonEditUserBidsWidget(model.buttons[i], context,formKey, controller,user_bids, postUserBidsResult, state, sendPath, id,  title, formData ));
    }
       return(
	        buttonChildren 
	   );
       ...
       Widget	 RButtons(BuildContext context, bool? visible, var formKey, ScrollController controller, SubModelController user_bids,
  var postUserBidsResult, State state, String? sendPath, String? id,  String? title ){
    //  final size =MediaQuery.of(context).size;
    double? width = 400;
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,user_bids, postUserBidsResult, state, sendPath, id,  title )
    ...
    SpeedDial	 Buttons(BuildContext context, bool? visible, var formKey, ScrollController controller, SubModelController user_bids,
  var postUserBidsResult, State state, String? sendPath, String? id,  String? title ){
     return(
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,user_bids, postUserBidsResult, state, sendPath, id,  title )
	    )
	 );
   } 	
