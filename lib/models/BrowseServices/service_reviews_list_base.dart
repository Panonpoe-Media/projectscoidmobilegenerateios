...
import '../photo.dart';
import '../paging.dart';
part 'service_reviews_list_base.g.dart';


class ServiceReviewsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ServiceReviewsListingBase(this.json){
		items = ServiceReviewsListingItems.fromJson(this.json);
		tools = ServiceReviewsListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemServiceReviewsModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemServiceReviewsCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemServiceReviewsModel item,ItemServiceReviewsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonServiceReviewsWidget(tools.buttons[i],context));
    }
...
class ServiceReviewsListingItems{
  List<ItemServiceReviewsModel?>? items;
  ServiceReviewsListingItems(
      this.items,
      );

  factory ServiceReviewsListingItems.fromJson(Map<String, dynamic> json) {
  	return ServiceReviewsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemServiceReviewsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ServiceReviewsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemServiceReviews?>? items;
	ServiceReviewsListingTools(
			this.id,
			this.buttons,
			this.meta,
			this.paging,
	...
	factory ServiceReviewsListingTools.fromJson(Map<String, dynamic> json) => _$ServiceReviewsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ServiceReviewsListingToolsToJson(this);
...
class ItemServiceReviewsCard extends StatelessWidget {
  const ItemServiceReviewsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemServiceReviewsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemServiceReviewsContent extends StatelessWidget {
  const ItemServiceReviewsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemServiceReviewsButton(ItemServiceReviewsModel destination, int? index, BuildContext context){
...
class ItemServiceReviewsWidget extends StatelessWidget {

	final ItemServiceReviewsModel? item;
	final String? search;
	const  ItemServiceReviewsWidget({Key? key, @required this.item, this.search}) : super(key: key);
