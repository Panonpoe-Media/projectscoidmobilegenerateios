...
import '../photo.dart';
import '../paging.dart';
part 'service_comments_list_base.g.dart';


class ServiceCommentsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ServiceCommentsListingBase(this.json){
		items = ServiceCommentsListingItems.fromJson(this.json);
		tools = ServiceCommentsListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemServiceCommentsModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  height = totalHeight * 50;
        ...
        child:  ItemServiceCommentsCard(destination :item, search : search, shape : shape, height: height)
    );
  ...
  Widget viewItem1 (ItemServiceCommentsModel item,ItemServiceCommentsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonServiceCommentsWidget(tools.buttons[i],context));
    }
...
class ServiceCommentsListingItems{
  List<ItemServiceCommentsModel?>? items;
  ServiceCommentsListingItems(
      this.items,
      );

  factory ServiceCommentsListingItems.fromJson(Map<String, dynamic> json) {
  	return ServiceCommentsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemServiceCommentsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ServiceCommentsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemServiceComments?>? items;
	ServiceCommentsListingTools(
			this.id,
			this.buttons,
			this.meta,
			this.paging,
	...
	factory ServiceCommentsListingTools.fromJson(Map<String, dynamic> json) => _$ServiceCommentsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ServiceCommentsListingToolsToJson(this);
...
class ItemServiceCommentsCard extends StatelessWidget {
  const ItemServiceCommentsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemServiceCommentsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemServiceCommentsContent extends StatelessWidget {
  const ItemServiceCommentsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemServiceCommentsButton(ItemServiceCommentsModel destination, int? index, BuildContext context){
...
class ItemServiceCommentsWidget extends StatelessWidget {

	final ItemServiceCommentsModel? item;
	final String? search;
	const  ItemServiceCommentsWidget({Key? key, @required this.item, this.search}) : super(key: key);
