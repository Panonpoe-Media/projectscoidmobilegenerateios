...
import 'my_profile_item_base.dart';
import 'photo.dart';
import 'paging.dart';
import 'package:horizontal_data_table/horizontal_data_table.dart';
import 'package:flutter_widget_from_html_core/flutter_widget_from_html_core.dart';
import 'package:url_launcher/url_launcher.dart';
import 'package:projectscoid/models/MyProfile/action.dart';
...
class ModelActionEditProfileMyProfile {
  int?    age;
  int?    cnt;
  int?    page;
  String?   id;
...
String?   user_id;
  ...
  List<String?>?  skills_list_str;
	  ...
	  ModelActionEditProfileMyProfile(
            this.id,
			this.user_id,
        this.user_name,
        this.email,
    ...
    factory ModelActionEditProfileMyProfile.fromJson(Map<String, dynamic> json) => _$ModelActionEditProfileMyProfileFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionEditProfileMyProfileToJson(this);

}



class EditProfileMyProfileBase{
	var model;
	Map<String, dynamic> json;
            var photolast ;
	EditProfileMyProfileBase(this.json){
		model = EditProfileMyProfileSuperBase.fromJson(this.json);
     if (model.model.photo!= null) {
            photolast = Photo(model.model.photo.name, model.model.photo.status, model.model.photo.dir, model.model.photo.file, model.model.photo.thumb, model.model.photo.temp);
     }
	}



Widget RButtonActionMyProfileWidget(InterstitialAd? _interstitialAd,Button button, BuildContext context,var formKey, ScrollController controller, MyProfileController my_profile,
		 ...
		 children: <Widget>[
             ...
             // color : Color(0xFF037f51),
              onPressed: () async{
                      ...
                      var formData = await convertFormDataEditProfile(button.text!, model
											, photolast 
                     					  );
                      ...
                      false);
					  	if([ 'PlaceNewBid'].contains('EditProfile')){
										my_profile.deleteAllEditProfileMyProfile(this.model.model.project_title);
										}else if(['AskOwner'].contains('EditProfile')){
										my_profile.deleteAllEditProfileMyProfile(this.model.model.project_id.toString());
									    }else if(['HireMe'].contains('EditProfile')){
										my_profile.deleteAllEditProfileMyProfile(this.model.model.private_worker_id.toString());
										}else if(['InviteToBid'].contains('EditProfile')){
										my_profile.deleteAllEditProfileMyProfile(this.model.model.user_id);
										}else{
										my_profile.deleteAllEditProfileMyProfile('');
										}
                      if(sendPath!.contains('%s')){
                      final future = my_profile.postEditProfileMyProfileWithID();
                                  future.then((value) {
                                  state.setState(() {
                      ...
                      final future = my_profile.postEditProfileMyProfile();
                                  future.then((value) {
                                  state.setState(() {
                                  postMyProfileResult = value;
                                  });
...
SpeedDialChild  ButtonActionMyProfileWidget(Button button, BuildContext context,var formKey, ScrollController controller, MyProfileController my_profile,
 var postMyProfileResult, State state, String?   sendPath, String?   id,  String?   title){
  var cl;
  var ic;
  if (button.color == 'green'){
  ...
  if(isEdit && photolast != null){
	...
	if (model.model.photo!= null) {
      ...
      "user[_trigger_]":"$tgr",
  "user[photo]": '$photo',
  "user[photo_lastval]": '$photo_last',
  "user[display_name]": "${model.model.display_name }",
  "user[birth_date]": "${DateFormat('dd/MM/yyyy').format(model.model.birth_date)}",
   "user[jenis_kelamin_id]": "${model.model.jenis_kelamin_id_id }",
  "user[address]": "${model.model.address }",
  "user[country_id]": "${model.model.country_id_id }",
  "user[provinsi_id]": "${model.model.provinsi_id_id }",
  "user[kabupaten_id]": "${model.model.kabupaten_id_id }",
  "user[kecamatan_id]": "${model.model.kecamatan_id_id }",
  "user[kelurahan_id]": "${model.model.kelurahan_id_id }",
  "user[post_code]": "${model.model.post_code }",
  "user[self_introduction]": "${model.model.self_introduction }",
  "user[website]": "${model.model.website }",
  "user[language_id]": "${model.model.language_id_id }",
  "user[notify_projects]": "${model.model.notify_projects ? 1 : 0}",
  ...
  List<Widget> RlistButton(InterstitialAd? _interstitialAd,BuildContext context,var formKey, ScrollController controller, MyProfileController my_profile,
  var postEditProfileResult, State state, String?   sendPath, String?   id,  String?   title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionMyProfileWidget( _interstitialAd,model.buttons[i], context,formKey, controller,my_profile, postEditProfileResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
   ...
   List<SpeedDialChild> listButton(BuildContext context,var formKey, ScrollController controller, MyProfileController my_profile,
  var postEditProfileResult, State state, String?   sendPath, String?   id,  String?   title){
    ...
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(ButtonActionMyProfileWidget(model.buttons[i], context,formKey, controller,my_profile, postEditProfileResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postEditProfileResult, State state, String?   sendPath, String?   id,  String?   title, bool? isLoading ) {
     return(Container(height: 0.0, width: 0.0)
	 );
  }
   
    Widget	 RButtons(InterstitialAd? _interstitialAd, BuildContext context, bool? visible, var formKey, ScrollController controller, MyProfileController my_profile,
  var postEditProfileResult, State state, String?   sendPath, String?   id,  String?   title ){
     // final size =MediaQuery.of(context).size;
    double?   width = 400;
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(_interstitialAd,context, formKey,controller,my_profile, postEditProfileResult, state, sendPath, id,  title )
 ...
 SpeedDial	 Buttons(BuildContext context, bool? visible, var formKey, ScrollController controller, MyProfileController my_profile,
  var postEditProfileResult, State state, String?   sendPath, String?   id,  String?   title ){
     return(
	 SpeedDial(
	childMargin: const EdgeInsets.symmetric(horizontal: 18, vertical: 20),
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children: listButton(context, formKey,controller,my_profile, postEditProfileResult, state, sendPath, id,  title )
	    )
	 );
   } 
							  ...
							  //ImageWidget edit& FileWidget editexperiment validation non from validation.
								...
								ArticleWidget(
								 ...
								 WebsiteWidget(
								  ...
								  hint: 'pilih Model',
								  required: false,
								  idenum: model.model.language_list,
								  nameenum: model.model.language_list_str,
								  getValue: (int? val) {
									state.setState(() {
...
@JsonSerializable()
class EditProfileMyProfileSuperBase {

  String?   id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionEditProfileMyProfile? model;
  EditProfileMyProfileSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory EditProfileMyProfileSuperBase.fromJson(Map<String, dynamic> json) => _$EditProfileMyProfileSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$EditProfileMyProfileSuperBaseToJson(this);
...
class ModelActionChangePasswordMyProfile {
  int?    age;
  int?    cnt;
  int?    page;
  String?   id;
...
String?   user_id;
	  String?   user_name;
  String?   old_password;
  String?   old_password_confirmation;
  String?   password;
  String?   password_confirmation;

	
	  ModelActionChangePasswordMyProfile(
            this.id,
			this.user_id,
        this.user_name,
			this.old_password,
			this.old_password_confirmation,
			this.password,
      this.password_confirmation,

  );
    factory ModelActionChangePasswordMyProfile.fromJson(Map<String, dynamic> json) => _$ModelActionChangePasswordMyProfileFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionChangePasswordMyProfileToJson(this);

}



class ChangePasswordMyProfileBase{
	var model;
	Map<String, dynamic> json;
	ChangePasswordMyProfileBase(this.json){
		model = ChangePasswordMyProfileSuperBase.fromJson(this.json);
             ...
             // color : Color(0xFF037f51),
              onPressed: () async{
                                controller.animateTo(controller.position.minScrollExtent,
                                  ...
                                  //Map<String, dynamic> res = model.toJson();
                                  //print('json result == $res');
                      var formData = await convertFormDataChangePassword(button.text!, model
                     					  );
                                  my_profile = MyProfileController(AppProvider.getApplication(context),
                                  sendPath,
                                  AppAction.post,
                      ...
                      false);
					  	if([ 'PlaceNewBid'].contains('ChangePassword')){
										my_profile.deleteAllChangePasswordMyProfile(this.model.model.project_title);
										}else if(['AskOwner'].contains('ChangePassword')){
										my_profile.deleteAllChangePasswordMyProfile(this.model.model.project_id.toString());
									    }else if(['HireMe'].contains('ChangePassword')){
										my_profile.deleteAllChangePasswordMyProfile(this.model.model.private_worker_id.toString());
										}else if(['InviteToBid'].contains('ChangePassword')){
										my_profile.deleteAllChangePasswordMyProfile(this.model.model.user_id);
										}else{
										my_profile.deleteAllChangePasswordMyProfile('');
										}
                      if(sendPath!.contains('%s')){
                      final future = my_profile.postChangePasswordMyProfileWithID();
                                  future.then((value) {
                                  state.setState(() {
                                  postMyProfileResult = value;
                                  });
                       ...
                       // AppProvider.getRouter(context)!.pop(context);	
                                    ...
                                    if(Error.toString().contains('302')){
                                     // print(Error);
                                      var accountController = AccountController(AppProvider.getApplication(context), AppAction.listing);
                                      List listAccount = [];
                                      ...
                                      future.then((val){
                                        ...
                                        LoginItem login;
                                        //  for(var map in val){
                                        login =  LoginItem('OK',val.asMap()[0]['id'],val.asMap()[0]['user_name'], val.asMap()[0]['user_display'],  val.asMap()[0]['user_hash'], val.asMap()[0]['notif_count'], val.asMap()[0]['user_photo'], );
                                        APIRepository? apiRepProvider = AppProvider.getApplication(context).projectsAPIRepository;
                                        apiRepProvider?.saveOrUpdateAccount(login, this.model.model.password);

                                        //  }

                                      });
                                      setPasswordPref(this.model.model.password);


                                      print('error ${Error.toString()}');
                                    //  print('error2 ${Error.toString().split('[')[1].replaceAll(']', '')}');
                                     // print('error code ${Error.statusCode.toString()}');
                                     // print('error code1 ${Error.statusCode}');
                                      Navigator.pushAndRemoveUntil(
                                        context,
                                        MaterialPageRoute(builder: (context) => rt.UserMyProfileView()),
                                            (Route<dynamic> route) => false,
                      ...
                      final future = my_profile.postChangePasswordMyProfile();
                                  future.then((value) {
                                 // state.setState(() {
                                  postMyProfileResult = value;
                                 // });
 ...
 var postMyProfileResult, State state, String?   sendPath, String?   id,  String?   title){
  ...
  if(button.type == 'custom_filter'){
   ...
   bool? isEdit = action.contains(RegExp(r'edit', caseSensitive: false));    
      ...
      var formData = {
      "user[_trigger_]":"$tgr",
  "user[old_password]": "${model.model.old_password }",
  "user[password]": "${model.model.password }",
  ...
  var postChangePasswordResult, State state, String?   sendPath, String?   id,  String?   title){
    final List<Widget> buttonChildren = <Widget>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(RButtonActionMyProfileWidget(model.buttons[i], context,formKey, controller,my_profile, postChangePasswordResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postChangePasswordResult, State state, String?   sendPath, String?   id,  String?   title){
    ...
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(ButtonActionMyProfileWidget(model.buttons[i], context,formKey, controller,my_profile, postChangePasswordResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  Widget injectAction(BuildContext context, bool? visible, var formKey, ScrollController controller, MyProfileController my_profile,
  var postChangePasswordResult, State state, String?   sendPath, String?   id,  String?   title, bool? isLoading ) {
     return(Container(height: 0.0, width: 0.0)
	 );
  }
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,my_profile, postChangePasswordResult, state, sendPath, id,  title )
 ...
 SpeedDial	 Buttons(BuildContext context, bool? visible, var formKey, ScrollController controller, MyProfileController my_profile,
  var postChangePasswordResult, State state, String?   sendPath, String?   id,  String?   title ){
	...
	childMargin: const EdgeInsets.symmetric(horizontal: 18, vertical: 20),
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children: listButton(context, formKey,controller,my_profile, postChangePasswordResult, state, sendPath, id,  title )
	    )
	 );
   } 
...
@JsonSerializable()
class ChangePasswordMyProfileSuperBase {

  String?   id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionChangePasswordMyProfile? model;
  ChangePasswordMyProfileSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ChangePasswordMyProfileSuperBase.fromJson(Map<String, dynamic> json) => _$ChangePasswordMyProfileSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ChangePasswordMyProfileSuperBaseToJson(this);
...
class ModelActionChangeEmailMyProfile {
  int?    age;
  int?    cnt;
  int?    page;
  String?   id;
...
String?   user_id;
  ...
  String?   secure_code;
	  ...
	  ModelActionChangeEmailMyProfile(
            this.id,
			this.user_id,
        this.user_name,
			this.new_email,
    ...
    factory ModelActionChangeEmailMyProfile.fromJson(Map<String, dynamic> json) => _$ModelActionChangeEmailMyProfileFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionChangeEmailMyProfileToJson(this);

}



class ChangeEmailMyProfileBase{
	var model;
	Map<String, dynamic> json;
	ChangeEmailMyProfileBase(this.json){
		model = ChangeEmailMyProfileSuperBase.fromJson(this.json);
	}
             ...
             // color : Color(0xFF037f51),
              onPressed: () async{
                                controller.animateTo(controller.position.minScrollExtent,
                                  ...
                                  //Map<String, dynamic> res = model.toJson();
                                  //print('json result == $res');
                      var formData = await convertFormDataChangeEmail(button.text!, model
                     					  );
                                  my_profile = MyProfileController(AppProvider.getApplication(context),
                                  sendPath,
                                  AppAction.post,
                      ...
                      false);
					  	if([ 'PlaceNewBid'].contains('ChangeEmail')){
										my_profile.deleteAllChangeEmailMyProfile(this.model.model.project_title);
										}else if(['AskOwner'].contains('ChangeEmail')){
										my_profile.deleteAllChangeEmailMyProfile(this.model.model.project_id.toString());
									    }else if(['HireMe'].contains('ChangeEmail')){
										my_profile.deleteAllChangeEmailMyProfile(this.model.model.private_worker_id.toString());
										}else if(['InviteToBid'].contains('ChangeEmail')){
										my_profile.deleteAllChangeEmailMyProfile(this.model.model.user_id);
										}else{
										my_profile.deleteAllChangeEmailMyProfile('');
										}
                      if(sendPath!.contains('%s')){
                      final future = my_profile.postChangeEmailMyProfileWithID();
                                  future.then((value) {
                                  state.setState(() {
                                  postMyProfileResult = value;
                                  });
                      ...
                      final future = my_profile.postChangeEmailMyProfile();
                                  future.then((value) {
                                  state.setState(() {
                                  postMyProfileResult = value;
                                  });
 ...
 var postMyProfileResult, State state, String?   sendPath, String?   id,  String?   title){
  ...
  if(button.type == 'custom_filter'){
   ...
   bool? isEdit = action.contains(RegExp(r'edit', caseSensitive: false));    
      ...
      var formData = {
      "user[_trigger_]":"$tgr",
  "user[new_email]": "${model.model.new_email }",
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, MyProfileController my_profile,
  var postChangeEmailResult, State state, String?   sendPath, String?   id,  String?   title){
    final List<Widget> buttonChildren = <Widget>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(RButtonActionMyProfileWidget(model.buttons[i], context,formKey, controller,my_profile, postChangeEmailResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postChangeEmailResult, State state, String?   sendPath, String?   id,  String?   title){
    ...
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(ButtonActionMyProfileWidget(model.buttons[i], context,formKey, controller,my_profile, postChangeEmailResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,my_profile, postChangeEmailResult, state, sendPath, id,  title )
 ...
 SpeedDial	 Buttons(BuildContext context, bool? visible, var formKey, ScrollController controller, MyProfileController my_profile,
  var postChangeEmailResult, State state, String?   sendPath, String?   id,  String?   title ){
	...
	childMargin: const EdgeInsets.symmetric(horizontal: 18, vertical: 20),
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children: listButton(context, formKey,controller,my_profile, postChangeEmailResult, state, sendPath, id,  title )
	    )
	 );
   } 
...
@JsonSerializable()
class ChangeEmailMyProfileSuperBase {

  String?   id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionChangeEmailMyProfile? model;
  ChangeEmailMyProfileSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ChangeEmailMyProfileSuperBase.fromJson(Map<String, dynamic> json) => _$ChangeEmailMyProfileSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ChangeEmailMyProfileSuperBaseToJson(this);
...
class ModelActionChangeHandphoneMyProfile {
  int?    age;
  int?    cnt;
  int?    page;
  String?   id;
...
String?   user_id;
  ...
  String?   secure_code;
	  ...
	  ModelActionChangeHandphoneMyProfile(
            this.id,
			this.user_id,
        this.user_name,
			this.handphone,
    ...
    factory ModelActionChangeHandphoneMyProfile.fromJson(Map<String, dynamic> json) => _$ModelActionChangeHandphoneMyProfileFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionChangeHandphoneMyProfileToJson(this);

}



class ChangeHandphoneMyProfileBase{
	var model;
	Map<String, dynamic> json;
	ChangeHandphoneMyProfileBase(this.json){
		model = ChangeHandphoneMyProfileSuperBase.fromJson(this.json);
	}
             ...
             // color : Color(0xFF037f51),
              onPressed: () async{
                                controller.animateTo(controller.position.minScrollExtent,
                                  ...
                                  //Map<String, dynamic> res = model.toJson();
                                  //print('json result == $res');
                      var formData = await convertFormDataChangeHandphone(button.text!, model
                     					  );
                                  my_profile = MyProfileController(AppProvider.getApplication(context),
                                  sendPath,
                                  AppAction.post,
                      ...
                      false);
					  	if([ 'PlaceNewBid'].contains('ChangeHandphone')){
										my_profile.deleteAllChangeHandphoneMyProfile(this.model.model.project_title);
										}else if(['AskOwner'].contains('ChangeHandphone')){
										my_profile.deleteAllChangeHandphoneMyProfile(this.model.model.project_id.toString());
									    }else if(['HireMe'].contains('ChangeHandphone')){
										my_profile.deleteAllChangeHandphoneMyProfile(this.model.model.private_worker_id.toString());
										}else if(['InviteToBid'].contains('ChangeHandphone')){
										my_profile.deleteAllChangeHandphoneMyProfile(this.model.model.user_id);
										}else{
										my_profile.deleteAllChangeHandphoneMyProfile('');
										}
                      if(sendPath!.contains('%s')){
                      final future = my_profile.postChangeHandphoneMyProfileWithID();
                                  future.then((value) {
                                  state.setState(() {
                                  postMyProfileResult = value;
                                  });
                      ...
                      final future = my_profile.postChangeHandphoneMyProfile();
                                  future.then((value) {
                                  state.setState(() {
                                  postMyProfileResult = value;
                                  });
...
SpeedDialChild  ButtonActionMyProfileWidget(Button button, BuildContext context,var formKey, ScrollController controller, MyProfileController my_profile,
 var postMyProfileResult, State state, String?   sendPath, String?   id,  String?   title){
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, MyProfileController my_profile,
  var postChangeHandphoneResult, State state, String?   sendPath, String?   id,  String?   title){
    final List<Widget> buttonChildren = <Widget>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(RButtonActionMyProfileWidget(model.buttons[i], context,formKey, controller,my_profile, postChangeHandphoneResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postChangeHandphoneResult, State state, String?   sendPath, String?   id,  String?   title){
    ...
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(ButtonActionMyProfileWidget(model.buttons[i], context,formKey, controller,my_profile, postChangeHandphoneResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,my_profile, postChangeHandphoneResult, state, sendPath, id,  title )
 ...
 SpeedDial	 Buttons(BuildContext context, bool? visible, var formKey, ScrollController controller, MyProfileController my_profile,
  var postChangeHandphoneResult, State state, String?   sendPath, String?   id,  String?   title ){
	...
	childMargin: const EdgeInsets.symmetric(horizontal: 18, vertical: 20),
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children: listButton(context, formKey,controller,my_profile, postChangeHandphoneResult, state, sendPath, id,  title )
	    )
	 );
   } 
...
@JsonSerializable()
class ChangeHandphoneMyProfileSuperBase {

  String?   id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionChangeHandphoneMyProfile? model;
  ChangeHandphoneMyProfileSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ChangeHandphoneMyProfileSuperBase.fromJson(Map<String, dynamic> json) => _$ChangeHandphoneMyProfileSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ChangeHandphoneMyProfileSuperBaseToJson(this);
...
class ModelMyProfile {
  ...
  String?   coupon;
    ...
    factory ModelMyProfile.fromJson(Map<String, dynamic> json) => _$ModelMyProfileFromJson(json);

  Map<String, dynamic> toJson() => _$ModelMyProfileToJson(this);
...
class MyProfileBase{
	var model;
	Map<String, dynamic> json;
	MyProfileBase(this.json){
		model = MyProfileSuperBase.fromJson(this.json);
			model.model.password = '';
			model.model.old_password = '';
   ...
   Map<String, dynamic> convertFormDataEdit(var data){
	...
	if (model.model.photo!= null) {
      ...
      var formData = {
      "my_profile[_trigger_]":"",
   "my_profile[user_id]": "${model.model.user_id }",
  "my_profile[user_name]": "${model.model.user_name }",
  "my_profile[group_id]": "${model.model.group_id }",
  "my_profile[email]": "${model.model.email }",
  "my_profile[password]": "${model.model.password }",
  "my_profile[password_confirmation]": "${model.model.password }",
  "my_profile[auth]": "${model.model.auth }",
   "my_profile[account_status_id]": "${model.model.account_status_id}",
   "my_profile[account_type_id]": "${model.model.account_type_id}",
  "my_profile[date_registered]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.date_registered)}",
  "my_profile[last_login]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.last_login)}",
  "my_profile[last_seen]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.last_seen)}",
  "my_profile[online]": "${model.model.online ? 1 : 0}",
  "my_profile[last_online_mark]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.last_online_mark)}",
  "my_profile[online_hours]": "${model.model.online_hours}",
  "my_profile[nomor_ktp]": "${model.model.nomor_ktp }",
  "my_profile[scan_ktp]": '$scan_ktp',
  "my_profile[scan_ktp_lastval]": '{"name":"${model.model.scan_ktp.name}","dir":"${model.model.scan_ktp.dir}","file":"${model.model.scan_ktp.file}","thumb":"${model.model.scan_ktp.thumb}"}',
  "my_profile[nomor_siup]": "${model.model.nomor_siup }",
  "my_profile[scan_siup]": '$scan_siup',
  "my_profile[scan_siup_lastval]": '{"name":"${model.model.scan_siup.name}","dir":"${model.model.scan_siup.dir}","file":"${model.model.scan_siup.file}","thumb":"${model.model.scan_siup.thumb}"}',
  "my_profile[npwp]": "${model.model.npwp }",
   "my_profile[status_marital_id]": "${model.model.status_marital_id}",
  "my_profile[jumlah_tanggungan]": "${model.model.jumlah_tanggungan }",
  "my_profile[personal_data]": "${model.model.personal_data }",
  "my_profile[photo]": '$photo',
  "my_profile[photo_lastval]": '{"name":"${model.model.photo.name}","dir":"${model.model.photo.dir}","file":"${model.model.photo.file}","thumb":"${model.model.photo.thumb}"}',
  "my_profile[display_name]": "${model.model.display_name }",
  "my_profile[birth_date]": "${DateFormat('dd/MM/yyyy').format(model.model.birth_date)}",
  "my_profile[birth_code]": "${model.model.birth_code }",
   "my_profile[jenis_kelamin_id]": "${model.model.jenis_kelamin_id}",
  "my_profile[address]": "${model.model.address }",
  "my_profile[country_id]": "${model.model.country_id }",
  "my_profile[provinsi_id]": "${model.model.provinsi_id }",
  "my_profile[kabupaten_id]": "${model.model.kabupaten_id }",
  "my_profile[kecamatan_id]": "${model.model.kecamatan_id }",
  "my_profile[kelurahan_id]": "${model.model.kelurahan_id }",
  "my_profile[post_code]": "${model.model.post_code }",
  "my_profile[handphone]": "${model.model.handphone }",
  "my_profile[self_introduction]": "${model.model.self_introduction }",
  "my_profile[website]": "${model.model.website }",
  "my_profile[language_id]": "${model.model.language_id }",
  "my_profile[full_name]": "${model.model.full_name }",
  "my_profile[captcha]": "${model.model.captcha }",
  "my_profile[secure_code]": "${model.model.secure_code }",
  "my_profile[old_password]": "${model.model.old_password }",
  "my_profile[old_password_confirmation]": "${model.model.old_password }",
  "my_profile[new_email]": "${model.model.new_email }",
  "my_profile[your_email]": "${model.model.your_email }",
  "my_profile[invitation_message]": "${model.model.invitation_message }",
  "my_profile[financial_data]": "${model.model.financial_data }",
  "my_profile[bank_id]": "${model.model.bank_id }",
  "my_profile[branch]": "${model.model.branch }",
  "my_profile[city]": "${model.model.city }",
  "my_profile[account_number]": "${model.model.account_number }",
  "my_profile[account_name]": "${model.model.account_name }",
   "my_profile[cutoff_period_id]": "${model.model.cutoff_period_id}",
  "my_profile[cutoff_amount]": "${model.model.cutoff_amount}",
  "my_profile[deposit_amount]": "${model.model.deposit_amount}",
  "my_profile[statistics]": "${model.model.statistics }",
  "my_profile[money_earned]": "${model.model.money_earned}",
  "my_profile[money_spent]": "${model.model.money_spent}",
  "my_profile[available_balance]": "${model.model.available_balance}",
  "my_profile[withheld_balance]": "${model.model.withheld_balance}",
  "my_profile[admin_notes]": "${model.model.admin_notes }",
  "my_profile[pesta_point]": "${model.model.pesta_point }",
  "my_profile[worker_rating]": "${model.model.worker_rating }",
  "my_profile[worker_ranking]": "${model.model.worker_ranking }",
  "my_profile[worker_point]": "${model.model.worker_point }",
  "my_profile[worker_rating_num]": "${model.model.worker_rating_num}",
  "my_profile[worker_rating_sum]": "${model.model.worker_rating_sum }",
  "my_profile[worker_rating_div]": "${model.model.worker_rating_div }",
  "my_profile[owner_rating]": "${model.model.owner_rating }",
  "my_profile[owner_ranking]": "${model.model.owner_ranking }",
  "my_profile[owner_point]": "${model.model.owner_point }",
  "my_profile[owner_rating_num]": "${model.model.owner_rating_num}",
  "my_profile[owner_rating_sum]": "${model.model.owner_rating_sum }",
  "my_profile[owner_rating_div]": "${model.model.owner_rating_div }",
  "my_profile[seller_rating]": "${model.model.seller_rating }",
  "my_profile[seller_ranking]": "${model.model.seller_ranking }",
  "my_profile[seller_point]": "${model.model.seller_point }",
  "my_profile[seller_rating_num]": "${model.model.seller_rating_num}",
  "my_profile[seller_rating_sum]": "${model.model.seller_rating_sum }",
  "my_profile[seller_rating_div]": "${model.model.seller_rating_div }",
  "my_profile[buyer_rating]": "${model.model.buyer_rating }",
  "my_profile[buyer_ranking]": "${model.model.buyer_ranking }",
  "my_profile[buyer_point]": "${model.model.buyer_point }",
  "my_profile[buyer_rating_num]": "${model.model.buyer_rating_num}",
  "my_profile[buyer_rating_sum]": "${model.model.buyer_rating_sum }",
  "my_profile[buyer_rating_div]": "${model.model.buyer_rating_div }",
  "my_profile[affiliate_rating]": "${model.model.affiliate_rating }",
  "my_profile[affiliate_ranking]": "${model.model.affiliate_ranking }",
  "my_profile[affiliate_point]": "${model.model.affiliate_point }",
  "my_profile[affiliate_rating_num]": "${model.model.affiliate_rating_num}",
  "my_profile[affiliate_rating_sum]": "${model.model.affiliate_rating_sum }",
  "my_profile[affiliate_rating_div]": "${model.model.affiliate_rating_div }",
  "my_profile[projects_won]": "${model.model.projects_won }",
  "my_profile[projects_won_cumulative]": "${model.model.projects_won_cumulative}",
  "my_profile[projects_owned]": "${model.model.projects_owned }",
  "my_profile[projects_owned_cumulative]": "${model.model.projects_owned_cumulative}",
  "my_profile[projects_noaction]": "${model.model.projects_noaction }",
  "my_profile[projects_canceled]": "${model.model.projects_canceled }",
  "my_profile[missed_payment]": "${model.model.missed_payment }",
  "my_profile[projects_completed]": "${model.model.projects_completed }",
  "my_profile[projects_arbitrated]": "${model.model.projects_arbitrated }",
  "my_profile[projects_arbit_won]": "${model.model.projects_arbit_won }",
  "my_profile[projects_arbit_lost]": "${model.model.projects_arbit_lost }",
  "my_profile[current_projects]": "${model.model.current_projects }",
  "my_profile[missed_weekly_report]": "${model.model.missed_weekly_report }",
  "my_profile[product_sold]": "${model.model.product_sold }",
  "my_profile[product_sold_cumulative]": "${model.model.product_sold_cumulative}",
  "my_profile[product_bought]": "${model.model.product_bought }",
  "my_profile[product_bought_cumulative]": "${model.model.product_bought_cumulative}",
  "my_profile[bid_count]": "${model.model.bid_count }",
  "my_profile[post_count]": "${model.model.post_count }",
  "my_profile[work_quality]": "${model.model.work_quality }",
  "my_profile[work_quality_num]": "${model.model.work_quality_num}",
  "my_profile[work_quality_sum]": "${model.model.work_quality_sum }",
  "my_profile[work_quality_div]": "${model.model.work_quality_div }",
  "my_profile[expertise]": "${model.model.expertise }",
  "my_profile[expertise_num]": "${model.model.expertise_num}",
  "my_profile[expertise_sum]": "${model.model.expertise_sum }",
  "my_profile[expertise_div]": "${model.model.expertise_div }",
  "my_profile[worker_communication]": "${model.model.worker_communication }",
  "my_profile[worker_communication_num]": "${model.model.worker_communication_num}",
  "my_profile[worker_communication_sum]": "${model.model.worker_communication_sum }",
  "my_profile[worker_communication_div]": "${model.model.worker_communication_div }",
  "my_profile[worker_professionalism]": "${model.model.worker_professionalism }",
  "my_profile[worker_professionalism_num]": "${model.model.worker_professionalism_num}",
  "my_profile[worker_professionalism_sum]": "${model.model.worker_professionalism_sum }",
  "my_profile[worker_professionalism_div]": "${model.model.worker_professionalism_div }",
  "my_profile[clarity]": "${model.model.clarity }",
  "my_profile[clarity_num]": "${model.model.clarity_num}",
  "my_profile[clarity_sum]": "${model.model.clarity_sum }",
  "my_profile[clarity_div]": "${model.model.clarity_div }",
  "my_profile[friendliness]": "${model.model.friendliness }",
  "my_profile[friendliness_num]": "${model.model.friendliness_num}",
  "my_profile[friendliness_sum]": "${model.model.friendliness_sum }",
  "my_profile[friendliness_div]": "${model.model.friendliness_div }",
  "my_profile[owner_communication]": "${model.model.owner_communication }",
  "my_profile[owner_communication_num]": "${model.model.owner_communication_num}",
  "my_profile[owner_communication_sum]": "${model.model.owner_communication_sum }",
  "my_profile[owner_communication_div]": "${model.model.owner_communication_div }",
  "my_profile[owner_professionalism]": "${model.model.owner_professionalism }",
  "my_profile[owner_professionalism_num]": "${model.model.owner_professionalism_num}",
  "my_profile[owner_professionalism_sum]": "${model.model.owner_professionalism_sum }",
  "my_profile[owner_professionalism_div]": "${model.model.owner_professionalism_div }",
  "my_profile[product_quality]": "${model.model.product_quality }",
  "my_profile[product_quality_num]": "${model.model.product_quality_num}",
  "my_profile[product_quality_sum]": "${model.model.product_quality_sum }",
  "my_profile[product_quality_div]": "${model.model.product_quality_div }",
  "my_profile[service_quality]": "${model.model.service_quality }",
  "my_profile[service_quality_num]": "${model.model.service_quality_num}",
  "my_profile[service_quality_sum]": "${model.model.service_quality_sum }",
  "my_profile[service_quality_div]": "${model.model.service_quality_div }",
  "my_profile[seller_communication]": "${model.model.seller_communication }",
  "my_profile[seller_communication_num]": "${model.model.seller_communication_num}",
  "my_profile[seller_communication_sum]": "${model.model.seller_communication_sum }",
  "my_profile[seller_communication_div]": "${model.model.seller_communication_div }",
  "my_profile[seller_professionalism]": "${model.model.seller_professionalism }",
  "my_profile[seller_professionalism_num]": "${model.model.seller_professionalism_num}",
  "my_profile[seller_professionalism_sum]": "${model.model.seller_professionalism_sum }",
  "my_profile[seller_professionalism_div]": "${model.model.seller_professionalism_div }",
  "my_profile[referer_id]": "${model.model.referer_id }",
  "my_profile[referer_expires]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.referer_expires)}",
  "my_profile[referal_this_month]": "${model.model.referal_this_month }",
  "my_profile[cookies_this_month]": "${model.model.cookies_this_month }",
  "my_profile[premium_affiliate]": "${model.model.premium_affiliate ? 1 : 0}",
  "my_profile[refered_users]": "${model.model.refered_users }",
  "my_profile[referal_earnings]": "${model.model.referal_earnings}",
  "my_profile[affiliate_id]": "${model.model.affiliate_id }",
  "my_profile[last_announced_project]": "${model.model.last_announced_project }",
  "my_profile[notif_count]": "${model.model.notif_count }",
  "my_profile[notify_projects]": "${model.model.notify_projects ? 1 : 0}",
  "my_profile[lost_arbitration_recently]": "${model.model.lost_arbitration_recently ? 1 : 0}",
  "my_profile[coupon]": "${model.model.coupon }",
	  
  };  
      int count = 0;
      for(var tag in model.model.skills_id){
        formData.addAll({"my_profile[inline_tags][selection][$count]": "$tag"});
        count++;
      }
  ...
  var postMyProfileResult, State state, String?   sendPath, String?   id,  String?   title){
    ...
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(ButtonEditMyProfileWidget(model.buttons[i], context,formKey, controller,my_profile, postMyProfileResult, state, sendPath, id,  title ));
      }  
    }
       return(
	        buttonChildren 
  ...
  var postMyProfileResult, State state, String?   sendPath, String?   id,  String?   title ){
	...
	childMargin: const EdgeInsets.symmetric(horizontal: 18, vertical: 20),
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,my_profile, postMyProfileResult, state, sendPath, id,  title )
	    )
	 );
   } 

SpeedDialChild ButtonEditMyProfileWidget(Button button, BuildContext context,var formKey, ScrollController controller, MyProfileController my_profile,
 var postMyProfileResult, State state, String?   sendPath, String?   id,  String?   title){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
...
class MyProfileSuperBase {

  String?   id;
  Meta? meta;
  List<Button?>? buttons;
  ModelMyProfile? model;
  MyProfileSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
  ...
  factory MyProfileSuperBase.fromJson(Map<String, dynamic> json) => _$MyProfileSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$MyProfileSuperBaseToJson(this);
...
class ViewModelMyProfile {
  ...
  List<String?>?  skills_url;
   ...
   factory ViewModelMyProfile.fromJson(Map<String, dynamic> json) => _$ViewModelMyProfileFromJson(json);

   Map<String, dynamic> toJson() => _$ViewModelMyProfileToJson(this); 
  
}


class MyProfileViewBase{
	var model;
	Map<String, dynamic> json;
	final List<Widget> viewChildren = <Widget>[
	];
	MyProfileViewBase(this.json);
	
	
   List<SpeedDialChild>	 listButton(BuildContext context, bool? visible, var formKey, ScrollController controller, MyProfileController my_profile,
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(ButtonViewMyProfileWidget(model.buttons[i],context,  formKey, controller, my_profile,
 ...
 SpeedDialChild ButtonViewMyProfileWidget(Button button, BuildContext context,var formKey, ScrollController controller, MyProfileController my_profile,
  State state, String?   sendPath, String?   id,  String?   title, bool? account){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
	...
	return(	SpeedDialChild(
		  ...
		  onTap: ()async{
		  {
                        ...
                        null,
						false);
                        final future = my_profile.downloadFile();
                        future.then((value) {
                        state.setState(() {
                       // postMyProfileResult = value;
						//AppProvider.getRouter(context)!.pop(context);	
	...
	childMargin: const EdgeInsets.symmetric(horizontal: 18, vertical: 20),
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, visible, formKey, controller, my_profile,
                                    state, sendPath, id,  title, account )
	    )
	 );
   } 	
	...
	Widget view (BuildContext context, ScrollController controller, bool? account) {
	  ...
	  if(this.model.meta.before_content != null){ viewChildren.add(Padding(
	   ...
	   viewChildren.add(viewBirthDate(context)); 
	   viewChildren.add(viewJenisKelamin(context)); 
	   ...
	   viewChildren.add(viewKelurahan(context)); 
	   viewChildren.add(viewPostCode(context)); 
	   viewChildren.add(viewHandphone(context)); 
	   viewChildren.add(viewSelfIntroduction(context)); 
	   viewChildren.add(viewWebsite(context)); 
	   viewChildren.add(viewLanguage(context)); 
	   viewChildren.add(viewNotifyMeNewProjects(context)); 
	   viewChildren.add(viewSkills(context)); 

	 //
	if(model.meta.after_content != null){ viewChildren.add(Padding(
		padding: const EdgeInsets.fromLTRB(
			8.0, 14.0, 8.0, 2.0),
   ...
   return(  SingleChildScrollView(
    ...
    if(model.model.birth_date != null){
		return(  DateView(
		value: model.model.birth_date,
		caption: 'Birth Date',
		));
	    ...
	    Widget viewJenisKelamin (BuildContext context) {
		 ...
		 EnumView(
			...
			caption: 'Jenis Kelamin',
			idenum: model.model.jenis_kelamin_list,
			nameenum: model.model.jenis_kelamin_str,
		));} 
	...
	return(  AddressView(
		...
		return(  ModelView(
			...
			caption: 'Country',
			idenum: model.model.country_list,
			nameenum: model.model.country_str,
		));} 
		...
		return(  ModelView(
			...
			caption: 'Kelurahan',
			idenum: model.model.kelurahan_list,
			nameenum: model.model.kelurahan_str,
		));} 
	...
	return(  WebsiteView(
		...
		return(  ModelView(
			...
			caption: 'Language',
			idenum: model.model.language_list,
			nameenum: model.model.language_str,
		));} 
		    Widget viewNotifyMeNewProjects (BuildContext context) {  

	if(model.model.notify_projects != null){
		return(  BooleanView(
		value: model.model.notify_projects,
		caption: 'Notify Me New Projects',
	));
...
@JsonSerializable()
class MyProfileViewSuperBase {

  String?   id;
  Meta? meta;
  List<Button?>? buttons;
  ViewModelMyProfile? model;
  MyProfileViewSuperBase(
      this.id,
      this.buttons,
      this.meta,
      this.model,
      ...
      factory MyProfileViewSuperBase.fromJson(Map<String, dynamic> json) => _$MyProfileViewSuperBaseFromJson(json);
      Map<String, dynamic> toJson() => _$MyProfileViewSuperBaseToJson(this);
...
class MyProfileListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	MyProfileListingBase(this.json){
		items = MyProfileListingItems.fromJson(this.json);
		tools = MyProfileListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemMyProfileModel? item,String?   search, bool? account) {
	ShapeBorder? shape;
    double?   height = 0;
	...
	totalHeight = totalHeight + 4 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemMyProfileCard(destination :item, search : search, shape : shape, height : height)
    );
  ...
  Widget viewItemId(ItemMyProfileModel? item,String?   search, int? index, bool? account, String?   id) {
	ShapeBorder? shape;
    double?   height = 0;
	double?   totalHeight = 0;
	   totalHeight = totalHeight + 2 ;
	...
	totalHeight = totalHeight + 4 ;
	  ...
	  Widget viewItemIndex (ItemMyProfileModel? item,String?   search, int? index, bool? account) {
	ShapeBorder? shape;
    double?   height = 0;
	double?   totalHeight = 0;
	   totalHeight = totalHeight + 2 ;
...
Widget viewItem1 (ItemMyProfileModel? item,ItemMyProfileModel? item1, String?   search, BuildContext context,bool? account, String?   id , bool? isMax, int? max, int? index) {
	ShapeBorder? shape;
    double?   height = 0;
	double?   totalHeight = 0;
	    return Visibility (
	...
	Widget viewItemState (ItemMyProfileModel? item,String?   search, int? index, BuildContext context, bool? account, State st, VoidCallback onrefresh) {
	ShapeBorder? shape;
    double?   height = 0;
	double?   totalHeight = 0;
	   totalHeight = totalHeight + 2 ;
   ...
   // if(tools.buttons[i].text != "Table View"){
      ...
      buttonChildren.add(ListButtonMyProfileWidget(tools.buttons[i],context, account));
   // }  
    }
...
class MyProfileListingItems{
  List<ItemMyProfileModel?>? items;
  MyProfileListingItems(
      this.items,
      );

  factory MyProfileListingItems.fromJson(Map<String, dynamic> json) {
  	return MyProfileListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemMyProfileModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class MyProfileListingTools{

	String?   id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemMyProfile?>? items;
	MyProfileListingTools(
			this.id,
			this.buttons,
			this.meta,
			this.paging,
	...
	factory MyProfileListingTools.fromJson(Map<String, dynamic> json) => _$MyProfileListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$MyProfileListingToolsToJson(this);
...
class ItemMyProfileCard extends StatelessWidget {
  const ItemMyProfileCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double?   height ;
  final ItemMyProfileModel? destination;
  final String?   search;
            ...
            Container(
             // height: height,
              child: Card(
                ...
                shape: shape,
                child: ItemMyProfileContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemMyProfileContent extends StatelessWidget {
  const ItemMyProfileContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> buttonChildren = <Widget>[
	  ...
	  if (destination!.item.buttons != null) {
		...
		{
		  buttonChildren.add(itemMyProfileButton(destination,i , context));
		}
    }
...
Widget itemMyProfileButton(ItemMyProfileModel? destination, int? index, BuildContext context){
...
class ItemMyProfileWidget extends StatelessWidget {

	final ItemMyProfileModel? item;
	final String?   search;
	const  ItemMyProfileWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonMyProfileWidget(Button button, BuildContext context, bool? account){
    var cl;
	var ic;
  	if (button.color == 'green'){
  		cl = Colors.green;
