...
import 'shortlisted_bids_list_item.dart';
import 'shortlisted_bids_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'shortlisted_bids_list_base.g.dart';


class ShortlistedBidsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ShortlistedBidsListingBase(this.json){
		items = ShortlistedBidsListingItems.fromJson(this.json);
		tools = ShortlistedBidsListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemShortlistedBidsModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemShortlistedBidsCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemShortlistedBidsModel item,ItemShortlistedBidsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonShortlistedBidsWidget(tools.buttons[i],context));
    }
...
class ShortlistedBidsListingItems{
  List<ItemShortlistedBidsModel?>? items;
  ShortlistedBidsListingItems(
      this.items,
      );

  factory ShortlistedBidsListingItems.fromJson(Map<String, dynamic> json) {
  	return ShortlistedBidsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemShortlistedBidsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ShortlistedBidsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemShortlistedBids?>? items;
	...
	factory ShortlistedBidsListingTools.fromJson(Map<String, dynamic> json) => _$ShortlistedBidsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ShortlistedBidsListingToolsToJson(this);
...
class ItemShortlistedBidsCard extends StatelessWidget {
  const ItemShortlistedBidsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemShortlistedBidsModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemShortlistedBidsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemShortlistedBidsContent extends StatelessWidget {
  const ItemShortlistedBidsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemShortlistedBidsButton(destination!,i , context));
		}
    }
    ...
    final List<Widget> children = [
	  ...
	  */
	   ...
	   children.add(destination!.viewMatchingSkills(context)); 
...
Widget itemShortlistedBidsButton(ItemShortlistedBidsModel destination, int? index, BuildContext context){
...
class ItemShortlistedBidsWidget extends StatelessWidget {

	final ItemShortlistedBidsModel? item;
	final String? search;
	const  ItemShortlistedBidsWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonShortlistedBidsWidget(Button button, BuildContext context){
