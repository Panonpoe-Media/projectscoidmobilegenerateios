...
part 'show_thread_cancel_arbitration_base.g.dart';



@JsonSerializable()

class ModelActionCancelArbitrationShowThread {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? post_id;
  ...
  bool?  read_by_worker;
	  ...
	  ModelActionCancelArbitrationShowThread(
            this.id,
			this.post_id,
			this.user_id,
			this.user_str,
    ...
    factory ModelActionCancelArbitrationShowThread.fromJson(Map<String, dynamic> json) => _$ModelActionCancelArbitrationShowThreadFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionCancelArbitrationShowThreadToJson(this);

}

@JsonSerializable()
class CancelArbitrationShowThreadSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionCancelArbitrationShowThread? model;
  CancelArbitrationShowThreadSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory CancelArbitrationShowThreadSuperBase.fromJson(Map<String, dynamic> json) => _$CancelArbitrationShowThreadSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$CancelArbitrationShowThreadSuperBaseToJson(this);
  
 
}

class CancelArbitrationShowThreadBase{
	var model;
	Map<String, dynamic> json;
            var attachmentslast ;   
	CancelArbitrationShowThreadBase(this.json){
		model = CancelArbitrationShowThreadSuperBase.fromJson(this.json);
 ...
 var postShowThreadResult, State state, String? sendPath, String? id,  String? title){
  ...
  var postCancelArbitrationResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postCancelArbitrationResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postCancelArbitrationResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postCancelArbitrationResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,show_thread, postCancelArbitrationResult, state, sendPath, id,  title )
  ...
  var postCancelArbitrationResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,show_thread, postCancelArbitrationResult, state, sendPath, id,  title )
	    )
	 );
   } 
