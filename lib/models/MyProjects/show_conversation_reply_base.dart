...
part 'show_conversation_reply_base.g.dart';



@JsonSerializable()

class ModelActionReplyShowConversation {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? post_id;
  ...
  bool?  read_by_worker;
	  ...
	  ModelActionReplyShowConversation(
            this.id,
			this.post_id,
			this.user_id,
			this.user_str,
    ...
    factory ModelActionReplyShowConversation.fromJson(Map<String, dynamic> json) => _$ModelActionReplyShowConversationFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionReplyShowConversationToJson(this);

}

@JsonSerializable()
class ReplyShowConversationSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionReplyShowConversation? model;
  ReplyShowConversationSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ReplyShowConversationSuperBase.fromJson(Map<String, dynamic> json) => _$ReplyShowConversationSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ReplyShowConversationSuperBaseToJson(this);
  
 
}

class ReplyShowConversationBase{
	var model;
	Map<String, dynamic> json;
            var attachmentslast ;   
	ReplyShowConversationBase(this.json){
		model = ReplyShowConversationSuperBase.fromJson(this.json);
...
SpeedDialChild ButtonActionShowConversationWidget(Button button, BuildContext context,var formKey, ScrollController controller, SubModelController show_conversation,
 var postShowConversationResult, State state, String? sendPath, String? id,  String? title){
  ...
  var postReplyResult, State state, String? sendPath, String? id,  String? title){
    final List<Widget> buttonChildren = <Widget>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(RButtonActionShowConversationWidget(model.buttons[i], context,formKey, controller,show_conversation, postReplyResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postReplyResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionShowConversationWidget(model.buttons[i], context,formKey, controller,show_conversation, postReplyResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,show_conversation, postReplyResult, state, sendPath, id,  title )
  ...
  var postReplyResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,show_conversation, postReplyResult, state, sendPath, id,  title )
	    )
	 );
   } 
