...
import '../photo.dart';
import '../paging.dart';
part 'show_conversation_list_base.g.dart';


class ShowConversationListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ShowConversationListingBase(this.json){
		items = ShowConversationListingItems.fromJson(this.json);
		tools = ShowConversationListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemShowConversationModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemShowConversationCard(destination :item, search : search, shape : shape, height: height)
    );


	}
	
  Widget viewItem1 (ItemShowConversationModel item,ItemShowConversationModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	    return Visibility (
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonShowConversationWidget(tools.buttons[i],context));
    }
...
class ShowConversationListingItems{
  List<ItemShowConversationModel?>? items;
  ShowConversationListingItems(
      this.items,
      );

  factory ShowConversationListingItems.fromJson(Map<String, dynamic> json) {
  	return ShowConversationListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemShowConversationModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ShowConversationListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemShowConversation?>? items;
	ShowConversationListingTools(
			this.id,
			this.buttons,
			this.meta,
			this.paging,
	...
	factory ShowConversationListingTools.fromJson(Map<String, dynamic> json) => _$ShowConversationListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ShowConversationListingToolsToJson(this);
...
class ItemShowConversationCard extends StatelessWidget {
  const ItemShowConversationCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemShowConversationModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemShowConversationContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemShowConversationContent extends StatelessWidget {
  const ItemShowConversationContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemShowConversationButton(destination!,i , context));
		}
    }
...
Widget itemShowConversationButton(ItemShowConversationModel destination, int? index, BuildContext context){
...
class ItemShowConversationWidget extends StatelessWidget {

	final ItemShowConversationModel? item;
	final String? search;
	const  ItemShowConversationWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonShowConversationWidget(Button button, BuildContext context){
