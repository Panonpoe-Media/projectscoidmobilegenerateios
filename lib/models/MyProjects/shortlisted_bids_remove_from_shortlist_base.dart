...
part 'shortlisted_bids_remove_from_shortlist_base.g.dart';



@JsonSerializable()

class ModelActionRemoveFromShortlistShortlistedBids {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? bid_id;
	  ...
	  ModelActionRemoveFromShortlistShortlistedBids(
            this.id,
			this.bid_id,
				this.worker_id,
				this.worker_str,
    ...
    factory ModelActionRemoveFromShortlistShortlistedBids.fromJson(Map<String, dynamic> json) => _$ModelActionRemoveFromShortlistShortlistedBidsFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionRemoveFromShortlistShortlistedBidsToJson(this);

}

@JsonSerializable()
class RemoveFromShortlistShortlistedBidsSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionRemoveFromShortlistShortlistedBids? model;
  RemoveFromShortlistShortlistedBidsSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory RemoveFromShortlistShortlistedBidsSuperBase.fromJson(Map<String, dynamic> json) => _$RemoveFromShortlistShortlistedBidsSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$RemoveFromShortlistShortlistedBidsSuperBaseToJson(this);
  
 
}

class RemoveFromShortlistShortlistedBidsBase{
	var model;
	Map<String, dynamic> json;
            var attachmentslast ;   
	RemoveFromShortlistShortlistedBidsBase(this.json){
		model = RemoveFromShortlistShortlistedBidsSuperBase.fromJson(this.json);
...
Widget RButtonActionShortlistedBidsWidget(Button button, BuildContext context,var formKey, ScrollController controller,  SubModelController shortlisted_bids,
             ...
             // color : Color(0xFF037f51),
              onPressed: () async{
                                   controller.animateTo(controller.position.minScrollExtent,
									...
									shortlisted_bids = new  SubModelController(AppProvider.getApplication(context),
									sendPath,
									formData);
...
SpeedDialChild ButtonActionShortlistedBidsWidget(Button button, BuildContext context,var formKey, ScrollController controller, SubModelController shortlisted_bids,
 var postShortlistedBidsResult, State state, String? sendPath, String? id,  String? title){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
  ...
  String? action =   'RemoveFromShortlist';
  ...
  String? attachments_last = '';
   if(isEdit && attachmentslast != null){
	...
	if (model.model.attachments!= null) {
     ...
     String? tgr =   'remove_from_shortlist';
    ...
    return( formData);
	  ...
	  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, SubModelController shortlisted_bids,
  var postRemoveFromShortlistResult, State state, String? sendPath, String? id,  String? title){
    final List<Widget> buttonChildren = <Widget>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(RButtonActionShortlistedBidsWidget(model.buttons[i], context,formKey, controller,shortlisted_bids, postRemoveFromShortlistResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postRemoveFromShortlistResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionShortlistedBidsWidget(model.buttons[i], context,formKey, controller,shortlisted_bids, postRemoveFromShortlistResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    Widget	 RButtons(BuildContext context, bool?  visible, var formKey, ScrollController controller, SubModelController shortlisted_bids,
  var postRemoveFromShortlistResult, State state, String? sendPath, String? id,  String? title ){
     // final size =MediaQuery.of(context).size;
    double? width = 400;
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,shortlisted_bids, postRemoveFromShortlistResult, state, sendPath, id,  title )
  ...
  var postRemoveFromShortlistResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,shortlisted_bids, postRemoveFromShortlistResult, state, sendPath, id,  title )
	    )
	 );
   } 
