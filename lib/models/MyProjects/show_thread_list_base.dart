...
import '../photo.dart';
import '../paging.dart';
part 'show_thread_list_base.g.dart';


class ShowThreadListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ShowThreadListingBase(this.json){
		items = ShowThreadListingItems.fromJson(this.json);
		tools = ShowThreadListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemShowThreadModel item,String? search, bool? account, String? id) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemShowThreadCard(destination :item, search : search, shape : shape, height: height)
    );
  ...
  Widget viewItem1 (ItemShowThreadModel item,ItemShowThreadModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonShowThreadWidget(tools.buttons[i],context));
    }
...
class ShowThreadListingItems{
  List<ItemShowThreadModel?>? items;
  ShowThreadListingItems(
      this.items,
      );

  factory ShowThreadListingItems.fromJson(Map<String, dynamic> json) {
  	return ShowThreadListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemShowThreadModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ShowThreadListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemShowThread?>? items;
	...
	factory ShowThreadListingTools.fromJson(Map<String, dynamic> json) => _$ShowThreadListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ShowThreadListingToolsToJson(this);
...
class ItemShowThreadCard extends StatelessWidget {
  const ItemShowThreadCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemShowThreadModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemShowThreadContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemShowThreadContent extends StatelessWidget {
  const ItemShowThreadContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemShowThreadButton(ItemShowThreadModel destination, int? index, BuildContext context){
...
class ItemShowThreadWidget extends StatelessWidget {

	final ItemShowThreadModel? item;
	final String? search;
	const  ItemShowThreadWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonShowThreadWidget(Button button, BuildContext context){
