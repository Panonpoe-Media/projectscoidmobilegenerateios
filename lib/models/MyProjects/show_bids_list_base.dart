...
import '../photo.dart';
import '../paging.dart';
part 'show_bids_list_base.g.dart';


class ShowBidsListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	ShowBidsListingBase(this.json){
		items = ShowBidsListingItems.fromJson(this.json);
		tools = ShowBidsListingTools.fromJson(this.json);
	}
  ...
  Widget viewItem (ItemShowBidsModel item,String? search, bool? account, String? id) {
	...
	totalHeight = totalHeight + 15 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemShowBidsCard(destination :item, search : search, shape : shape, height: height)
    );
  ...
  Widget viewItem1 (ItemShowBidsModel item,ItemShowBidsModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonShowBidsWidget(tools.buttons[i],context));
    }
...
class ShowBidsListingItems{
  List<ItemShowBidsModel?>? items;
  ShowBidsListingItems(
      this.items,
      );

  factory ShowBidsListingItems.fromJson(Map<String, dynamic> json) {
  	return ShowBidsListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemShowBidsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class ShowBidsListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemShowBids?>? items;
	...
	factory ShowBidsListingTools.fromJson(Map<String, dynamic> json) => _$ShowBidsListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$ShowBidsListingToolsToJson(this);
...
class ItemShowBidsCard extends StatelessWidget {
  const ItemShowBidsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemShowBidsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemShowBidsContent extends StatelessWidget {
  const ItemShowBidsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemShowBidsButton(ItemShowBidsModel destination, int? index, BuildContext context){
...
class ItemShowBidsWidget extends StatelessWidget {

	final ItemShowBidsModel? item;
	final String? search;
	const  ItemShowBidsWidget({Key? key, @required this.item, this.search}) : super(key: key);
