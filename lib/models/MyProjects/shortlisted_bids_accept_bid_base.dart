...
part 'shortlisted_bids_accept_bid_base.g.dart';



@JsonSerializable()

class ModelActionAcceptBidShortlistedBids {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? bid_id;
    ...
    factory ModelActionAcceptBidShortlistedBids.fromJson(Map<String, dynamic> json) => _$ModelActionAcceptBidShortlistedBidsFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionAcceptBidShortlistedBidsToJson(this);

}

@JsonSerializable()
class AcceptBidShortlistedBidsSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionAcceptBidShortlistedBids? model;
  ...
  factory AcceptBidShortlistedBidsSuperBase.fromJson(Map<String, dynamic> json) => _$AcceptBidShortlistedBidsSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$AcceptBidShortlistedBidsSuperBaseToJson(this);
  
 
}

class AcceptBidShortlistedBidsBase{
	var model;
	Map<String, dynamic> json;
            var attachmentslast ;   
	AcceptBidShortlistedBidsBase(this.json){
		model = AcceptBidShortlistedBidsSuperBase.fromJson(this.json);
...
SpeedDialChild ButtonActionShortlistedBidsWidget(Button button, BuildContext context,var formKey, ScrollController controller, SubModelController shortlisted_bids,
 var postShortlistedBidsResult, State state, String? sendPath, String? id,  String? title){
  ...
  var postAcceptBidResult, State state, String? sendPath, String? id,  String? title){
    final List<Widget> buttonChildren = <Widget>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren.add(RButtonActionShortlistedBidsWidget(model.buttons[i], context,formKey, controller,shortlisted_bids, postAcceptBidResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postAcceptBidResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionShortlistedBidsWidget(model.buttons[i], context,formKey, controller,shortlisted_bids, postAcceptBidResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,shortlisted_bids, postAcceptBidResult, state, sendPath, id,  title )
  ...
  var postAcceptBidResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,shortlisted_bids, postAcceptBidResult, state, sendPath, id,  title )
	    )
	 );
   } 
