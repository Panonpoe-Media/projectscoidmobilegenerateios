...
part 'show_thread_reply_base.g.dart';



@JsonSerializable()

class ModelActionReplyShowThread {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? post_id;
  ...
  bool?  read_by_worker;
    ...
    factory ModelActionReplyShowThread.fromJson(Map<String, dynamic> json) => _$ModelActionReplyShowThreadFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionReplyShowThreadToJson(this);

}

@JsonSerializable()
class ReplyShowThreadSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionReplyShowThread? model;
  ReplyShowThreadSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ReplyShowThreadSuperBase.fromJson(Map<String, dynamic> json) => _$ReplyShowThreadSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ReplyShowThreadSuperBaseToJson(this);
  
 
}

class ReplyShowThreadBase{
	var model;
	Map<String, dynamic> json;
            var attachmentslast ;   
	ReplyShowThreadBase(this.json){
		model = ReplyShowThreadSuperBase.fromJson(this.json);
 ...
 var postShowThreadResult, State state, String? sendPath, String? id,  String? title){
  ...
  var postReplyResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postReplyResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postReplyResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postReplyResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,show_thread, postReplyResult, state, sendPath, id,  title )
  ...
  var postReplyResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,show_thread, postReplyResult, state, sendPath, id,  title )
	    )
	 );
   } 
