...
part 'show_thread_rate_worker_base.g.dart';



@JsonSerializable()

class ModelActionRateWorkerShowThread {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? post_id;
  ...
  bool?  read_by_worker;
	  ...
	  ModelActionRateWorkerShowThread(
            this.id,
			this.post_id,
			this.user_id,
			this.user_str,
    ...
    factory ModelActionRateWorkerShowThread.fromJson(Map<String, dynamic> json) => _$ModelActionRateWorkerShowThreadFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionRateWorkerShowThreadToJson(this);

}

@JsonSerializable()
class RateWorkerShowThreadSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionRateWorkerShowThread? model;
  RateWorkerShowThreadSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory RateWorkerShowThreadSuperBase.fromJson(Map<String, dynamic> json) => _$RateWorkerShowThreadSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$RateWorkerShowThreadSuperBaseToJson(this);
  
 
}

class RateWorkerShowThreadBase{
	var model;
	Map<String, dynamic> json;
            var attachmentslast ;   
	RateWorkerShowThreadBase(this.json){
		model = RateWorkerShowThreadSuperBase.fromJson(this.json);
 ...
 var postShowThreadResult, State state, String? sendPath, String? id,  String? title){
  ...
  var postRateWorkerResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postRateWorkerResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postRateWorkerResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postRateWorkerResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,show_thread, postRateWorkerResult, state, sendPath, id,  title )
  ...
  var postRateWorkerResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,show_thread, postRateWorkerResult, state, sendPath, id,  title )
	    )
	 );
   } 
