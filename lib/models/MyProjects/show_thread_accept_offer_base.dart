...
part 'show_thread_accept_offer_base.g.dart';



@JsonSerializable()

class ModelActionAcceptOfferShowThread {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? post_id;
  ...
  bool?  read_by_worker;
	  ...
	  ModelActionAcceptOfferShowThread(
            this.id,
			this.post_id,
			this.user_id,
			this.user_str,
    ...
    factory ModelActionAcceptOfferShowThread.fromJson(Map<String, dynamic> json) => _$ModelActionAcceptOfferShowThreadFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionAcceptOfferShowThreadToJson(this);

}

@JsonSerializable()
class AcceptOfferShowThreadSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionAcceptOfferShowThread? model;
  AcceptOfferShowThreadSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory AcceptOfferShowThreadSuperBase.fromJson(Map<String, dynamic> json) => _$AcceptOfferShowThreadSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$AcceptOfferShowThreadSuperBaseToJson(this);
  
 
}

class AcceptOfferShowThreadBase{
	var model;
	Map<String, dynamic> json;
            var attachmentslast ;   
	AcceptOfferShowThreadBase(this.json){
		model = AcceptOfferShowThreadSuperBase.fromJson(this.json);
 ...
 var postShowThreadResult, State state, String? sendPath, String? id,  String? title){
  ...
  var postAcceptOfferResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postAcceptOfferResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postAcceptOfferResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postAcceptOfferResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,show_thread, postAcceptOfferResult, state, sendPath, id,  title )
  ...
  var postAcceptOfferResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,show_thread, postAcceptOfferResult, state, sendPath, id,  title )
	    )
	 );
   } 
