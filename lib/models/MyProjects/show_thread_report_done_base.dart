...
part 'show_thread_report_done_base.g.dart';



@JsonSerializable()

class ModelActionReportDoneShowThread {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? post_id;
  ...
  bool?  read_by_worker;
	  ...
	  ModelActionReportDoneShowThread(
            this.id,
			this.post_id,
			this.user_id,
			this.user_str,
    ...
    factory ModelActionReportDoneShowThread.fromJson(Map<String, dynamic> json) => _$ModelActionReportDoneShowThreadFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionReportDoneShowThreadToJson(this);

}

@JsonSerializable()
class ReportDoneShowThreadSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionReportDoneShowThread? model;
  ReportDoneShowThreadSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory ReportDoneShowThreadSuperBase.fromJson(Map<String, dynamic> json) => _$ReportDoneShowThreadSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$ReportDoneShowThreadSuperBaseToJson(this);
  
 
}

class ReportDoneShowThreadBase{
	var model;
	Map<String, dynamic> json;
            var attachmentslast ;   
	ReportDoneShowThreadBase(this.json){
		model = ReportDoneShowThreadSuperBase.fromJson(this.json);
 ...
 var postShowThreadResult, State state, String? sendPath, String? id,  String? title){
  ...
  var postReportDoneResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postReportDoneResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postReportDoneResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postReportDoneResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,show_thread, postReportDoneResult, state, sendPath, id,  title )
  ...
  var postReportDoneResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,show_thread, postReportDoneResult, state, sendPath, id,  title )
	    )
	 );
   } 
