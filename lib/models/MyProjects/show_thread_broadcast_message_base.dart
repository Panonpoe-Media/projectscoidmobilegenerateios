...
part 'show_thread_broadcast_message_base.g.dart';



@JsonSerializable()

class ModelActionBroadcastMessageShowThread {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? post_id;
  ...
  bool?  read_by_worker;
	  ...
	  ModelActionBroadcastMessageShowThread(
            this.id,
			this.post_id,
			this.user_id,
			this.user_str,
    ...
    factory ModelActionBroadcastMessageShowThread.fromJson(Map<String, dynamic> json) => _$ModelActionBroadcastMessageShowThreadFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionBroadcastMessageShowThreadToJson(this);

}

@JsonSerializable()
class BroadcastMessageShowThreadSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionBroadcastMessageShowThread? model;
  BroadcastMessageShowThreadSuperBase(
	  this.id,
	  this.buttons,
	  this.meta,
	  this.model,
  ...
  factory BroadcastMessageShowThreadSuperBase.fromJson(Map<String, dynamic> json) => _$BroadcastMessageShowThreadSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$BroadcastMessageShowThreadSuperBaseToJson(this);
  
 
}

class BroadcastMessageShowThreadBase{
	var model;
	Map<String, dynamic> json;
            var attachmentslast ;   
	BroadcastMessageShowThreadBase(this.json){
		model = BroadcastMessageShowThreadSuperBase.fromJson(this.json);
 ...
 var postShowThreadResult, State state, String? sendPath, String? id,  String? title){
  ...
  var postBroadcastMessageResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postBroadcastMessageResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postBroadcastMessageResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionShowThreadWidget(model.buttons[i], context,formKey, controller,show_thread, postBroadcastMessageResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,show_thread, postBroadcastMessageResult, state, sendPath, id,  title )
  ...
  var postBroadcastMessageResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,show_thread, postBroadcastMessageResult, state, sendPath, id,  title )
	    )
	 );
   } 
