...
class EditModelCreditHistory {
  ...
  String? adjustment_amount_str;
    ...
    factory EditModelCreditHistory.fromJson(Map<String, dynamic> json) => _$EditModelCreditHistoryFromJson(json);

  Map<String, dynamic> toJson() => _$EditModelCreditHistoryToJson(this);
...
class EditCreditHistoryBase{
	var model;
	Map<String, dynamic> json;
	EditCreditHistoryBase(this.json){
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, SubModelController credit_history,
  ...
  var formData = convertFormDataEdit(model);  
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionCreditHistoryWidget(model.buttons[i], context,formKey, controller,credit_history, postCreditHistoryResult, state, sendPath, id,  title, formData));
      }
    }
       return(
	        buttonChildren 
  ...
  var postCreditHistoryResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonEditCreditHistoryWidget(model.buttons[i], context,formKey, controller,credit_history, postCreditHistoryResult, state, sendPath, id,  title, formData ));
       ...
       Widget	 RButtons(BuildContext context, bool? visible, var formKey, ScrollController controller, SubModelController credit_history,
  var postCreditHistoryResult, State state, String? sendPath, String? id,  String? title ){
    //  final size =MediaQuery.of(context).size;
    double? width = 400;
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,credit_history, postCreditHistoryResult, state, sendPath, id,  title )
