...
import 'credit_history_list_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'credit_history_list_base.g.dart';


class CreditHistoryListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	CreditHistoryListingBase(this.json){
		items = CreditHistoryListingItems.fromJson(this.json);
		tools = CreditHistoryListingTools.fromJson(this.json);
	}
  ...
  Widget viewItem (ItemCreditHistoryModel item,String? search, bool? account, String? id) {
	...
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemCreditHistoryCard(destination :item, search : search, shape : shape, height: height)
    );
  ...
  Widget viewItem1 (ItemCreditHistoryModel item,ItemCreditHistoryModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonCreditHistoryWidget(tools.buttons[i],context));
    }
...
class CreditHistoryListingItems{
  List<ItemCreditHistoryModel?>? items;
  CreditHistoryListingItems(
      this.items,
      );

  factory CreditHistoryListingItems.fromJson(Map<String, dynamic> json) {
  	return CreditHistoryListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemCreditHistoryModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class CreditHistoryListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemCreditHistory?>? items;
	...
	factory CreditHistoryListingTools.fromJson(Map<String, dynamic> json) => _$CreditHistoryListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$CreditHistoryListingToolsToJson(this);
...
class ItemCreditHistoryCard extends StatelessWidget {
  const ItemCreditHistoryCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemCreditHistoryContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemCreditHistoryContent extends StatelessWidget {
  const ItemCreditHistoryContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemCreditHistoryButton(ItemCreditHistoryModel destination, int? index, BuildContext context){
...
class ItemCreditHistoryWidget extends StatelessWidget {

	final ItemCreditHistoryModel? item;
	final String? search;
	const  ItemCreditHistoryWidget({Key? key, @required this.item, this.search}) : super(key: key);
