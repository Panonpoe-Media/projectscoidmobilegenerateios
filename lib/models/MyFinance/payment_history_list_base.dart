...
import '../photo.dart';
import '../paging.dart';
part 'payment_history_list_base.g.dart';


class PaymentHistoryListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	PaymentHistoryListingBase(this.json){
		items = PaymentHistoryListingItems.fromJson(this.json);
		tools = PaymentHistoryListingTools.fromJson(this.json);
	}
  ...
  Widget viewItem (ItemPaymentHistoryModel item,String? search, bool? account, String? id) {
	...
	totalHeight = totalHeight + 8 ;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemPaymentHistoryCard(destination :item, search : search, shape : shape, height: height)
    );
  ...
  Widget viewItem1 (ItemPaymentHistoryModel item,ItemPaymentHistoryModel item1, String search, BuildContext context,bool account, String id , bool isMax, int max, int index) {
   ...
   List<SpeedDialChild>	 listButton(BuildContext context, String id){
    ...
    {
      buttonChildren.add(ListButtonPaymentHistoryWidget(tools.buttons[i],context));
    }
...
class PaymentHistoryListingItems{
  List<ItemPaymentHistoryModel?>? items;
  PaymentHistoryListingItems(
      this.items,
      );

  factory PaymentHistoryListingItems.fromJson(Map<String, dynamic> json) {
  	return PaymentHistoryListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemPaymentHistoryModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class PaymentHistoryListingTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemPaymentHistory?>? items;
	...
	factory PaymentHistoryListingTools.fromJson(Map<String, dynamic> json) => _$PaymentHistoryListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$PaymentHistoryListingToolsToJson(this);
...
class ItemPaymentHistoryCard extends StatelessWidget {
  const ItemPaymentHistoryCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemPaymentHistoryContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemPaymentHistoryContent extends StatelessWidget {
  const ItemPaymentHistoryContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemPaymentHistoryButton(destination!,i , context));
		}
    }
    ...
    final List<Widget> children = [
	  ...
	  */
	   ...
	   children.add(destination!.viewOurAccount(context)); 
	   children.add(destination!.viewDirection(context)); 
	   ...
	   children.add(destination!.viewAccountNo(context)); 
	   children.add(destination!.viewAccountName(context)); 
	   children.add(destination!.viewTransferDate(context)); 
	   children.add(destination!.viewAmount(context)); 
	   children.add(destination!.viewTransferFee(context)); 
	   children.add(destination!.viewMessage(context)); 
	   children.add(destination!.viewAttachment(context)); 
	   children.add(destination!.viewType(context)); 
	   children.add(destination!.viewRefId(context)); 
	   children.add(destination!.viewApprovedDate(context)); 
	   children.add(destination!.viewApprovedBy(context)); 
	   children.add(destination!.viewApprovedFromIp(context)); 
	   children.add(destination!.viewApproverNote(context)); 
        /*  children.add(Padding(
                            padding: const EdgeInsets.fromLTRB(
                                8.0, 14.0, 8.0, 2.0),
...
Widget itemPaymentHistoryButton(ItemPaymentHistoryModel destination, int? index, BuildContext context){
...
class ItemPaymentHistoryWidget extends StatelessWidget {

	final ItemPaymentHistoryModel? item;
	final String? search;
	const  ItemPaymentHistoryWidget({Key? key, @required this.item, this.search}) : super(key: key);
