...
import 'blog_item_base.dart';
import 'photo.dart';
import 'paging.dart';
import 'package:horizontal_data_table/horizontal_data_table.dart';
import 'package:flutter_widget_from_html_core/flutter_widget_from_html_core.dart';
import 'package:url_launcher/url_launcher.dart';
import 'package:projectscoid/models/Blog/action.dart';
import 'package:google_mobile_ads/google_mobile_ads.dart';
import 'package:projectscoid/views/route.dart' as rt;

part 'blog_base.g.dart';
...
class ModelBlog {
  ...
  List<FileField?>? files;
    ...
    factory ModelBlog.fromJson(Map<String, dynamic> json) => _$ModelBlogFromJson(json);
...
class BlogBase{
	...
	BlogBase(this.json){
		model = BlogSuperBase.fromJson(this.json);
   ...
   Map<String, dynamic> convertFormDataEdit(var data){
	...
	if (model.model.files!= null) {
      ...
      var formData = {
      "blog[_trigger_]":"",
   "blog[article_id]": "${model.model.article_id }",
   "blog[category_id]": "${model.model.category_id}",
  "blog[author_id]": "${model.model.author_id }",
  "blog[title]": "${model.model.title }",
  "blog[published]": "${model.model.published ? 1 : 0}",
  "blog[published_date]": "${DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.published_date)}",
   "blog[teaser]": "${model.model.teaser }",
  "blog[content]": "${model.model.content }",
   "blog[keywords]": "${model.model.keywords }",
  "blog[image]": '$image',
  "blog[image_lastval]": '{"name":"${model.model.image.name}","dir":"${model.model.image.dir}","file":"${model.model.image.file}","thumb":"${model.model.image.thumb}"}',
  "blog[files]": '$files',
  "blog[files_lastval]": '[{"name":"${model.model.files[0].name}","size":${model.model.files[0].size.toString()},"created":${model.model.files[0].date.toString()},"modified":${model.model.files[0].date.toString()},"temp":"${model.model.files[0].temp}","remote":"","dir":"temp"}]',
  ...
  List<SpeedDialChild> listButton(BuildContext context,var formKey, ScrollController controller, BlogController blog,
  var postBlogResult, State state, String? sendPath, String? id,  String? title){
    final List<SpeedDialChild> buttonChildren = <SpeedDialChild>[
    ];
	for(var i = 0; i < model.buttons.length; i++)
    {
      if(model.buttons[i].text != "Table View"){
      buttonChildren!.add(ButtonEditBlogWidget(model.buttons[i], context,formKey, controller,blog, postBlogResult, state, sendPath, id,  title ));
      }  
    }
       return(
	        buttonChildren 
    ...
    SpeedDial	 Buttons(BuildContext context, bool?visible, var formKey, ScrollController controller, BlogController blog,
  var postBlogResult, State state, String? sendPath, String? id,  String? title ){
     return(
	 SpeedDial(
				//marginRight: 18,
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,blog, postBlogResult, state, sendPath!, id!,  title! )
	    )
	 );
   } 

SpeedDialChild ButtonEditBlogWidget(Button button, BuildContext context,var formKey, ScrollController controller, BlogController blog,
 var postBlogResult, State state, String? sendPath, String? id,  String? title){
  if(button.type == 'custom_filter'){
	...
	return(	SpeedDialChild(
		  ...
		  onTap: ()async{
		  {
                        ...
                        formData,
						false);
                        final future = blog.postBlog();
                        future.then((value) {
                        state.setState(() {
                        postBlogResult = value;
...
class BlogSuperBase {
  ...
  factory BlogSuperBase.fromJson(Map<String, dynamic> json) => _$BlogSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$BlogSuperBaseToJson(this);
...
class ViewModelBlog {
  ...
  String? files_name;
   ...
   factory ViewModelBlog.fromJson(Map<String, dynamic> json) => _$ViewModelBlogFromJson(json);

   Map<String, dynamic> toJson() => _$ViewModelBlogToJson(this); 
...
class BlogViewBase{
	...
	BlogViewBase(this.json);
	
	
   List<SpeedDialChild>	 listButton(BuildContext context, bool?visible, var formKey, ScrollController controller, BlogController blog,
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren!.add(ButtonViewBlogWidget(model.buttons[i],context,  formKey, controller, blog,
 ...
 SpeedDialChild ButtonViewBlogWidget(Button button, BuildContext context,var formKey, ScrollController controller, BlogController blog,
  State state, String? sendPath, String? id,  String? title, bool?account){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
	...
	return(	SpeedDialChild(
		  ...
		  onTap: ()async{
		  {
                        ...
                        null,
						false);
                        final future = blog.downloadFile();
                        future.then((value) {
                        state.setState(() {
                       // postBlogResult = value;
						//AppProvider.getRouter(context)!.pop(context);	
    ...
    State state, String? sendPath, String? id,  String? title, bool? account){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, visible!, formKey, controller, blog,
                                    state, sendPath, id!,  title!, account! )
	    )
	 );
   } 	
...
class BlogViewSuperBase {
  ...
  BlogViewSuperBase(
      ...
      factory BlogViewSuperBase.fromJson(Map<String, dynamic> json) => _$BlogViewSuperBaseFromJson(json);
      Map<String, dynamic> toJson() => _$BlogViewSuperBaseToJson(this);
...
class BlogListingBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	BlogListingBase(this.json){
		items = BlogListingItems.fromJson(this.json);
		tools = BlogListingTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemBlogModel item,String? search, bool?account) {
	ShapeBorder? shape;
    double? height = 0;
	...
	totalHeight = totalHeight + 8 ;
	  height = totalHeight * 50;
        ...
        child:  ItemBlogCard(destination :item, search : search!, shape : shape!, height : height!)
    );
  ...
  Widget viewItemId(ItemBlogModel item,String? search, int? index, bool?account, String? id) {
	ShapeBorder? shape;
    double? height = 0;
	double? totalHeight = 0;
	   totalHeight = totalHeight + 2 ;
...
Widget viewItem1 (ItemBlogModel item,ItemBlogModel item1, String? search, BuildContext context,bool?account, String? id , bool?isMax, int? max, int? index) {
	ShapeBorder? shape;
    double? height = 0;
	double? totalHeight = 0;
	    return Visibility (
	...
	Widget viewItemState (ItemBlogModel item,String? search, int? index, BuildContext context, bool?account, State st, VoidCallback onrefresh) {
	ShapeBorder? shape;
    double? height = 0;
	double? totalHeight = 0;
	   totalHeight = totalHeight + 2 ;
   ...
   // if(tools.buttons[i].text != "Table View"){
      ...
      buttonChildren!.add(ListButtonBlogWidget(tools.buttons[i],context, account));
   // }  
    }
...
class BlogListingItems{
  List<ItemBlogModel?>? items;
  BlogListingItems(
      this.items,
      );

  factory BlogListingItems.fromJson(Map<String, dynamic> json) {
  	return BlogListingItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemBlogModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
class BlogListingTools{
	...
	factory BlogListingTools.fromJson(Map<String, dynamic> json) => _$BlogListingToolsFromJson(json);

	Map<String, dynamic> toJson() => _$BlogListingToolsToJson(this);
...
class ItemBlogCard extends StatelessWidget {
  const ItemBlogCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
            ...
            Container(
             // height: height,
              child: Card(
                ...
                shape: shape,
                child: ItemBlogContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemBlogContent extends StatelessWidget {
  const ItemBlogContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget>buttonChildren = <Widget>[
	  ...
	  if (destination!.item.buttons != null) {
		...
		{
		  buttonChildren!.add(itemBlogButton(destination,i , context));
		}
    }
...
Widget itemBlogButton(ItemBlogModel? destination, int? index, BuildContext context){
  BlogController blog; 
...
class ItemBlogWidget extends StatelessWidget {
	...
	final ItemBlogModel?  item;
	final String? search;
	const  ItemBlogWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild ListButtonBlogWidget(Button button, BuildContext context, bool?account){
    var cl;
	var ic;
  	if (button.color == 'green'){
  		cl = Colors.green;
