...
import 'comments_index_item.dart';
import 'comments_index_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'comments_index_base.g.dart';


class CommentsIndexBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	CommentsIndexBase(this.json){
		items = CommentsIndexItems.fromJson(this.json);
		tools = CommentsIndexTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemIndexCommentsModel item,String? search) {
	ShapeBorder? shape;
    double height = 0;
	...
	totalHeight = totalHeight + 15 ;
	  height = totalHeight * 50;
        ...
        child:  ItemIndexCommentsCard(destination :item, search : search, shape : shape, height : height)
    );
   ...
   List<SpeedDialChild>	 listButton(BuildContext context){
    ...
    {
      buttonChildren.add(IndexButtonCommentsWidget(tools.buttons[i],context));
    }
...
class CommentsIndexItems{
  List<ItemIndexCommentsModel?>? items;
  CommentsIndexItems(
      this.items,
      );

  factory CommentsIndexItems.fromJson(Map<String, dynamic> json) {
  	return CommentsIndexItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemIndexCommentsModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class CommentsIndexTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemIndexComments?>? items;
	CommentsIndexTools(
			this.id,
			this.buttons,
			this.meta,
	...
	factory CommentsIndexTools.fromJson(Map<String, dynamic> json) => _$CommentsIndexToolsFromJson(json);

	Map<String, dynamic> toJson() => _$CommentsIndexToolsToJson(this);
...
class ItemIndexCommentsCard extends StatelessWidget {
  const ItemIndexCommentsCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemIndexCommentsModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            SizedBox(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemIndexCommentsContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemIndexCommentsContent extends StatelessWidget {
  const ItemIndexCommentsContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
...
Widget itemCommentsButton(ItemIndexCommentsModel destination, int index, BuildContext context){
...
class ItemIndexCommentsWidget extends StatelessWidget {

	final ItemIndexCommentsModel?  item;
	final String? search;
	const  ItemIndexCommentsWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild IndexButtonCommentsWidget(Button button, BuildContext context){
