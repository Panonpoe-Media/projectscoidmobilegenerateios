...
part 'sales_edit_base.g.dart';


Widget RButtonActionSalesWidget(Button button, BuildContext context,var formKey, ScrollController controller,  SubModelController sales,
...
class EditModelSales {
  ...
  String? testimony;
    ...
    factory EditModelSales.fromJson(Map<String, dynamic> json) => _$EditModelSalesFromJson(json);

  Map<String, dynamic> toJson() => _$EditModelSalesToJson(this);
...
class EditSalesBase{
	...
	EditSalesBase(this.json){
		model = SalesSuperBase.fromJson(this.json);
	}
	
   Map<String, dynamic> convertFormDataEdit(var data){
      var formData = {
      "product_sales[_trigger_]":"",
  "product_sales[buyer_id]": "${model.model.buyer_id }",
  "product_sales[order_id]": "${model.model.order_id }",
  "product_sales[title]": "${model.model.title }",
   "product_sales[type_id]": "${model.model.type_id}",
  "product_sales[product_id]": "${model.model.product_id }",
  "product_sales[project_id]": "${model.model.project_id }",
  "product_sales[service_id]": "${model.model.service_id }",
  "product_sales[bid_id]": "${model.model.bid_id }",
  "product_sales[iklan_id]": "${model.model.iklan_id }",
  "product_sales[date]": "${model.model.date}",
  "product_sales[price]": "${model.model.price}",
   "product_sales[status_id]": "${model.model.status_id}",
  "product_sales[seller_id]": "${model.model.seller_id }",
  "product_sales[buyer_payment_id]": "${model.model.buyer_payment_id }",
  "product_sales[seller_credit_id]": "${model.model.seller_credit_id }",
  "product_sales[rating]": "${model.model.rating }",
  "product_sales[testimony]": "${model.model.testimony }",
  ...
  List<Widget> RlistButton(BuildContext context,var formKey, ScrollController controller, SubModelController sales,
  ...
  var formData = convertFormDataEdit(model);  
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionSalesWidget(model.buttons[i], context,formKey, controller,sales, postSalesResult, state, sendPath, id,  title, formData));
      }
    }
       return(
	        buttonChildren 
  ...
  var postSalesResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonEditSalesWidget(model.buttons[i], context,formKey, controller,sales, postSalesResult, state, sendPath, id,  title, formData ));
       ...
       Widget	 RButtons(BuildContext context, bool? visible, var formKey, ScrollController controller, SubModelController sales,
  var postSalesResult, State state, String? sendPath, String? id,  String? title ){
    //  final size =MediaQuery.of(context).size;
    double? width = 400;
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,sales, postSalesResult, state, sendPath, id,  title )
