...
import 'sales_index_item.dart';
import 'sales_index_item_base.dart';
import '../photo.dart';
import '../paging.dart';
part 'sales_index_base.g.dart';


class SalesIndexBase{
  var items;
  var tools;
	Map<String, dynamic> json;
	SalesIndexBase(this.json){
		items = SalesIndexItems.fromJson(this.json);
		tools = SalesIndexTools.fromJson(this.json);
	}
	
  Widget viewItem (ItemIndexSalesModel item,String? search) {
	ShapeBorder? shape;
    double height = 0;
	double totalHeight = 0;
	  ...
	  height = totalHeight * 50;
        ...
        child:  ItemIndexSalesCard(destination :item, search : search, shape : shape, height : height)
    );
   ...
   List<SpeedDialChild>	 listButton(BuildContext context){
    ...
    {
      buttonChildren.add(IndexButtonSalesWidget(tools.buttons[i],context));
    }
...
class SalesIndexItems{
  List<ItemIndexSalesModel?>? items;
  SalesIndexItems(
      this.items,
      );

  factory SalesIndexItems.fromJson(Map<String, dynamic> json) {
  	return SalesIndexItems(
				(json['items'] as List)
						?.map((e) => e == null
						? null
						: ItemIndexSalesModel(e as Map<String, dynamic>))
						?.toList());
	}
}
...
@JsonSerializable()
class SalesIndexTools{

	String? id;
	Meta? meta;
	List<Button?>? buttons;
	Paging? paging;
	List<ItemIndexSales?>? items;
	SalesIndexTools(
			this.id,
			this.buttons,
			this.meta,
	...
	factory SalesIndexTools.fromJson(Map<String, dynamic> json) => _$SalesIndexToolsFromJson(json);

	Map<String, dynamic> toJson() => _$SalesIndexToolsToJson(this);
...
class ItemIndexSalesCard extends StatelessWidget {
  const ItemIndexSalesCard({ Key? key, @required this.destination, this.search, this.shape, this.height})
      : assert(destination != null),
        super(key: key);

  // This height will allow for all the Card's content to fit comfortably within the card.
  final double? height;
  final ItemIndexSalesModel? destination;
  final String? search;
          ...
          children: <Widget>[
            ...
            SizedBox(
              ...
              child: Card(
                ...
                shape: shape,
                child: ItemIndexSalesContent(destination: destination),
              ),
            ),
          ],
        ),
...
class ItemIndexSalesContent extends StatelessWidget {
  const ItemIndexSalesContent({ Key? key, @required this.destination })
      : assert(destination != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
	...
	if (destination!.item!.buttons != null) {
		...
		{
		  buttonChildren.add(itemSalesButton(destination!,i , context));
		}
    }
...
Widget itemSalesButton(ItemIndexSalesModel destination, int index, BuildContext context){
...
class ItemIndexSalesWidget extends StatelessWidget {

	final ItemIndexSalesModel?  item;
	final String? search;
	const  ItemIndexSalesWidget({Key? key, @required this.item, this.search}) : super(key: key);
...
SpeedDialChild IndexButtonSalesWidget(Button button, BuildContext context){
