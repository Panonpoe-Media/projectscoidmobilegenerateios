...
part 'comments_new_reply_base.g.dart';



@JsonSerializable()

class ModelActionNewReplyComments {
  int?    age;
  int?    cnt;
  int?    page;
  String? id;
...
String? product_post_id;
  ...
  String? captcha;
    ...
    factory ModelActionNewReplyComments.fromJson(Map<String, dynamic> json) => _$ModelActionNewReplyCommentsFromJson(json);

  Map<String, dynamic> toJson() => _$ModelActionNewReplyCommentsToJson(this);

}

@JsonSerializable()
class NewReplyCommentsSuperBase {

  String? id;
  Meta? meta;
  List<Button?>? buttons;
  ModelActionNewReplyComments? model;
  ...
  factory NewReplyCommentsSuperBase.fromJson(Map<String, dynamic> json) => _$NewReplyCommentsSuperBaseFromJson(json);

  Map<String, dynamic> toJson() => _$NewReplyCommentsSuperBaseToJson(this);
  
 
}

class NewReplyCommentsBase{
	var model;
	Map<String, dynamic> json;
	NewReplyCommentsBase(this.json){
		model = NewReplyCommentsSuperBase.fromJson(this.json);
	}
             ...
             // color : Color(0xFF037f51),
              onPressed: () async{
                                   controller.animateTo(controller.position.minScrollExtent,
									...
									comments = new  SubModelController(AppProvider.getApplication(context),
									sendPath,
									formData);
...
SpeedDialChild ButtonActionCommentsWidget(Button button, BuildContext context,var formKey, ScrollController controller, SubModelController comments,
 var postCommentsResult, State state, String? sendPath, String? id,  String? title){
  if(button.type == 'custom_filter'){
    return (
	       SpeedDialChild(
  ...
  var postNewReplyResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      ...
      buttonChildren.add(RButtonActionCommentsWidget(model.buttons[i], context,formKey, controller,comments, postNewReplyResult, state, sendPath, id,  title));
      }
    }
       return(
	        buttonChildren 
  ...
  var postNewReplyResult, State state, String? sendPath, String? id,  String? title){
	...
	for(var i = 0; i < model.buttons.length; i++)
    {
      buttonChildren.add(ButtonActionCommentsWidget(model.buttons[i], context,formKey, controller,comments, postNewReplyResult, state, sendPath, id,  title ));
    }
       return(
	        buttonChildren 
	   );
    ...
    return(
        ButtonBarTheme(
            ...
            child: ButtonBar(
                ...
                buttonMinWidth: 0.9 * width,
                children:
           RlistButton(context, formKey,controller,comments, postNewReplyResult, state, sendPath, id,  title )
  ...
  var postNewReplyResult, State state, String? sendPath, String? id,  String? title ){
	 ...
	 SpeedDial(
				...
				elevation: 8.0,
				shape: CircleBorder(),
				children:listButton(context, formKey,controller,comments, postNewReplyResult, state, sendPath, id,  title )
	    )
	 );
   } 
