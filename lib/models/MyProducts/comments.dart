  ...
  formData.addAll({"product_post[user_id]": "${model.model.user_id }"});
  formData.addAll({"product_post[product_id]": "${model.model.product_id }"});
  formData.addAll({"product_post[thread_id]": "${model.model.thread_id }"});
   formData.addAll({"product_post[post_type_id]": "${model.model.post_type_id }"});
  formData.addAll({"product_post[post_date]": "${model.model.post_date == null? '':DateFormat('dd/MM/yyyy HH:mm:ss').format(model.model.post_date)}"});
  formData.addAll({"product_post[message]": "${model.model.message }"});
  formData.addAll({"product_post[attachments]": '$attachments'});
  formData.addAll({"product_post[attachments_lastval]": '$attachments_last'});
  formData.addAll({"product_post[read_by_seller]": "${model.model.read_by_seller ? 1 : 0}"});
  formData.addAll({"product_post[read_by_buyer]": "${model.model.read_by_buyer ? 1 : 0}"});
