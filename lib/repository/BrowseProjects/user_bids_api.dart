part of 'package:projectscoid/repository/APIRepository.dart';
 ...
 Future<bool> isOldUserBidsList(DBRepository _dbRepository)async{

    int ageDB = await _dbRepository.listUserBidsAge1();
  ...
  Future<bool> isEmptyUserBidsListDB(DBRepository _dbRepository) async {
    var user_bids;
    user_bids = await _dbRepository.loadUserBidsList1('');
   ...
   Future<UserBidsListingModel?> getUserBidsList(String url, int page, APIProvider _apiProvider, DBRepository _dbRepository)async{
...
Future<UserBidsListingModel> getUserBidsListSearch(String url, int page, APIProvider _apiProvider, DBRepository _dbRepository)async{
  UserBidsListingModel user_bids ;   
  user_bids = await _apiProvider.getListUserBids(url, page)
		  .then((responseUserBidsList) {
...
Future<UserBidsListingModel> _loadAndSaveUserBidsList(UserBidsListingModel list, String searchKey, int page, DBRepository _dbRepository) async{
    ...
    for(var i = 0; i < list.items.items.length ; i++){
      ...
      await _dbRepository.saveOrUpdateUserBidsList1(user_bids);
    }
	appList.items.items.clear();
    appList.items.items.addAll(await _dbRepository.loadUserBidsList1(searchKey)) ;
    return appList;
  }

Future<void> _loadAndSaveUserBidsList1(UserBidsListingModel list, String searchKey, int page, DBRepository _dbRepository) async{
    UserBidsListingModel appList = list;
    int age = DateTime.now().toUtc().millisecondsSinceEpoch;
	await _dbRepository.saveOrUpdateUserBidsListInfo1(list);
  ...
  Future<UserBidsListingModel> loadUserBidsList( String searchKey, DBRepository _dbRepository) async{
    UserBidsListingModel appList = await _dbRepository.loadUserBidsListInfo1('');
	appList.items.items.clear();
    appList.items.items.addAll(await _dbRepository.loadUserBidsList1(searchKey)) ;
    return appList;
  }
