...
enum CollapseBarMode {
  ...
  none,
  /// The background widget will scroll in a parallax fashion and right translate.
  translate,
}
...
/// `zoomBackground`. The app bar also makes use of [CollapseBarMode.parallax] by
/// default.
///
/// ```dart imports
/// import 'package:flutter/material.dart';
...
///               stretchModes: <StretchBarMode>[
///                 StretchBarMode.zoomBackground,
///                 StretchBarMode.blurBackground,
///                 StretchBarMode.fadeTitle,
///               ],
///               centerTitle: true,
...
class ExpandSpaceBar extends StatefulWidget {
  /// Creates a flexible space bar.
  ///
  /// Most commonly used in the [AppBar.flexibleSpace] field.
  const ExpandSpaceBar({
    Key? key,
    required this.title,
    required this.background,
    required this.centerTitle,
    required this.titlePadding,
    this.collapseMode = CollapseBarMode.parallax,
    this.stretchModes = const <StretchBarMode>[StretchBarMode.zoomBackground],
  ...
  ///
  /// Defaults to [CollapseBarMode.parallax].
  final CollapseBarMode collapseMode;

  /// Stretch effect while over-scrolling,
  ///
  /// Defaults to include [StretchBarMode.zoomBackground].
  final List<StretchBarMode> stretchModes;
  ...
  @override
  _ExpandSpaceBarState createState() => _ExpandSpaceBarState();
}

class _ExpandSpaceBarState extends State<ExpandSpaceBar> {
  ...
  double _getCollapsePadding(double t, FlexibleSpaceBarSettings settings) {
    switch (widget.collapseMode) {
      case CollapseBarMode.pin:
        return -(settings.maxExtent - settings.currentExtent);
      case CollapseBarMode.none:
        return 0.0;
      case CollapseBarMode.parallax:
        final double deltaExtent = settings.maxExtent - settings.minExtent;
        return -Tween<double>(begin: 0.0, end: deltaExtent / 4.0).transform(t);
      case CollapseBarMode.translate:
        final double deltaExtent = settings.maxExtent - settings.minExtent;
        return Tween<double>(begin: 0.0, end: (deltaExtent / 4.0)*20).transform(t);
  ...
  double _getCollapsePaddingRight(double t, FlexibleSpaceBarSettings settings) {
    switch (widget.collapseMode) {
      case CollapseBarMode.pin:
        return -(settings.maxExtent - settings.currentExtent);
      case CollapseBarMode.none:
         ...
         assert(
          ...
          if (widget.background != null) {
            ...
            if (opacity > 0.0) {
              ...
              // StretchBarMode.zoomBackground
              if (widget.stretchModes.contains(StretchBarMode.zoomBackground) &&
              ...
              children.add(Positioned(
                ...
                left: 0.0,
                right:_getCollapsePaddingRight(t, settings),
                height: height,
                child: Opacity(
                  opacity: opacity,
              ...
              // StretchBarMode.blurBackground
              if (widget.stretchModes.contains(StretchBarMode.blurBackground) &&
          ...
          if (widget.title != null) {
            ...
            // StretchBarMode.fadeTitle
            if (widget.stretchModes.contains(StretchBarMode.fadeTitle) &&
                constraints.maxHeight > settings.maxExtent) {
            ...
            if (opacity > 0.0) {
              TextStyle? titleStyle = theme.primaryTextTheme.caption;
...
class ExpandSpaceBarSettings extends InheritedWidget {
  ...
  @override
  bool updateShouldNotify(ExpandSpaceBarSettings oldWidget) {
