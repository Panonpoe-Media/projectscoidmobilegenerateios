...
class _MenuTabBar extends State<MenuTabBar> with TickerProviderStateMixin {
  ...
  VoidCallback ? _listenerDown;
  VoidCallback ? _listenerUp;
  ...
  List<Widget> _buildMenuIcons(){
    ...
    for(var i = 0; i < widget.iconButtons!.length; i++){
      if(i == widget.iconButtons!.length/2) {
        icons.add(
            SizedBox(width: MediaQuery.of(context).size.width/(widget.iconButtons!.length+1), height: 0)
        );
      }
      icons.add(SizedBox(width: MediaQuery.of(context).size.width/(widget.iconButtons!.length+1), child: widget.iconButtons![i]));
    }
  ...
  void _updateButtonPosition(double dy){
    ...
    _animationUp = Tween<double>(begin: position, end: MediaQuery.of(context).size.height*0.7).animate(CurvedAnimation(parent: _animationControllerUp!, curve: Curves.ease));
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(children: <Widget>[
      Stack(children: <Widget>[
        ...
        StreamBuilder(stream: _isActivated!.stream, builder: (context, AsyncSnapshot<int> snapshot){
          return snapshot.data == -1 ? SizedBox(height: 0, width: 0) :
          StreamBuilder(initialData: 0.0, stream: _opacity!.stream, builder: (context, AsyncSnapshot<double> snapshot) {
            return Opacity(opacity: snapshot.data!, child:
            StreamBuilder(initialData: 0.0, stream: _positionButton!.stream, builder: (context, AsyncSnapshot<double> snapshot) {
              ...
              return ClipPath(clipper: ContainerClipper(positon!), child: Container(width: double.infinity, height: MediaQuery.of(context).size.height, color: widget.background));
            })
            );
          });
        }),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.center, children: <Widget>[
          Listener(
              ...
              child: StreamBuilder(stream: _positionButton!.stream, initialData: 10.0, builder: (context, AsyncSnapshot snapshot){
                return Padding(padding: EdgeInsets.only(bottom: snapshot.data), child:
                StreamBuilder(stream: _isActivated!.stream, builder: (context, AsyncSnapshot snapshot){
                  return FloatingActionButton(elevation: 0, onPressed: (){
                  ...
                  Transform.rotate(angle: _animationRotate!.value, child:
                  Icon(Icons.circle, size: 30,  color: snapshot.data == -1 ? widget.colorMenuIconDefault : widget.colorMenuIconActivated)),
        ...
        StreamBuilder(stream: _isActivated!.stream, builder: (context, AsyncSnapshot<int> snapshot) {
          return snapshot.data == 1 ? Padding(padding: const EdgeInsets.only(top: 0), child: widget.child) : const SizedBox(width: 0, height: 0);
        }),
        )
      ])
    ]);
...
class MenuTabBarItem extends StatelessWidget {
  ...
  const MenuTabBarItem({this.label, this.onTap}): assert(label != null);
