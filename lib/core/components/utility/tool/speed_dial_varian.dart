...
class SpeedDialVarian extends StatefulWidget {
  /// Children buttons, from the lowest to the highest.
  final List<SpeedDialVarianChild>? children;
  ...
  @override
  _SpeedDialVarianState createState() => _SpeedDialVarianState();
}

class _SpeedDialVarianState extends State<SpeedDialVarian> with SingleTickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(SpeedDialVarian oldWidget) {
  ...
  List<Widget> _getChildrenList() {
    ...
    return widget.children!
        .map((SpeedDialVarianChild? child) {
          int index = widget.children!.indexOf(child!);

          var childAnimation = Tween(begin: 0.0, end: 48.0).animate(
            CurvedAnimation(
              parent: _controller!,
          ...
          return AnimatedChildVar(
            ...
            shape: child.shape,
            heroTag: widget.heroTag != null ? '${widget.heroTag}-child-$index' : null, key: null,
          );
        })
        .toList()
        .reversed
  ...
  Widget _renderButton() {
    ...
    var animatedFloatingButton = AnimatedFloatingButtonVar(
