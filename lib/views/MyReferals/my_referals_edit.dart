...
//123
class UserMyReferalsEdit extends StatefulWidget {
  static const String PATH = '/user/my_referals/edit/:id/:title';
  final String? id ;
  final String? title;
  UserMyReferalsEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  UserMyReferalsEditState createState() => UserMyReferalsEditState();
}

class UserMyReferalsEditState extends State<UserMyReferalsEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/user/my_referals/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/user/my_referals/add';
  String sendPath = Env.value!.baseUrl! + '/user/my_referals/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/user/my_referals/edit/' +widget.id! +'/' + widget.title!;
     ...
     fetchData(my_referals);
      ...
      return Scaffold(
          ...
          body:isLoading?
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,my_referals, postMyReferalsResult, this, sendPath, widget.id, widget.title)
