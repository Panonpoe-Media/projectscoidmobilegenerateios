...
//123
class UserMyPointsEdit extends StatefulWidget {
  static const String PATH = '/user/my_points/edit/:id/:title';
  final String? id ;
  final String? title;
  UserMyPointsEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  UserMyPointsEditState createState() => UserMyPointsEditState();
}

class UserMyPointsEditState extends State<UserMyPointsEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/user/my_points/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/user/my_points/add';
  String sendPath = Env.value!.baseUrl! + '/user/my_points/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/user/my_points/edit/' +widget.id! +'/' + widget.title!;
    controller = ScrollController();
    validation.add(true);
  }

 fetchData(MyPointsController? my_points)async {
    if (this.model == null) {
      final future = my_points!.editMyPoints();
      future.then((value) {
        setState(() {
          isLoading = false;
        });
     ...
     fetchData(my_points);
      ...
      return Scaffold(
          ...
          body:isLoading?
                  ...
                  child: Column(
					  ...
					  children: isLoading? [] : [
							  Padding(
							   ...
							   this.model.editPointsPesta(this),  
							   this.model.editPointsWorker(this),  
							   this.model.editPointsOwner(this),  
							   this.model.editPointsSeller(this),  
							   this.model.editPointsBuyer(this),  
							   this.model.editPointsAffiliate(this),  
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,my_points, postMyPointsResult, this, sendPath, widget.id, widget.title)
