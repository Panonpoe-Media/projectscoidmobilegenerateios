...
//123
class PublicExistingUserEdit extends StatefulWidget {
  static const String PATH = '/public/existing_user/edit/:id/:title';
  final String? id ;
  final String? title;
  PublicExistingUserEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  PublicExistingUserEditState createState() => PublicExistingUserEditState();
}

class PublicExistingUserEditState extends State<PublicExistingUserEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/public/existing_user/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/public/existing_user/add';
  String sendPath = Env.value!.baseUrl! + '/public/existing_user/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/public/existing_user/edit/' +widget.id! +'/' + widget.title!;
    controller = ScrollController();
    validation.add(true);
  }

 fetchData(ExistingUserController? existing_user)async {
    if (this.model == null) {
      final future = existing_user!.editExistingUser();
      future.then((value) {
        setState(() {
          isLoading = false;
        });
  ...
  Widget build(BuildContext context) {
    ...
    existing_user = new  ExistingUserController(AppProvider.getApplication(context),
        getPath,
        AppAction.edit,
        widget.id,
        widget.title,
     ...
     fetchData(existing_user);
      ...
      return Scaffold(
          ...
          body:isLoading?
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,existing_user, postExistingUserResult, this, sendPath, widget.id, widget.title)
