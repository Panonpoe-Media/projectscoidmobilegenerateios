...
class ServiceReviewsListBase extends StatefulWidget {
  final String? id ;
  final String? title;
  String? url;
  ServiceReviewsListBase({Key? key, this.id, this.title, this.url}) : super(key: key);
  @override
  ServiceReviewsListBaseState createState() => ServiceReviewsListBaseState();
}

class  ServiceReviewsListBaseState extends State<ServiceReviewsListBase>{
  String title = 'Service Reviews'; 
...
String filterShortcodes(String input,
  ...
  Widget build(BuildContext context) {
   ...
   final themeManager =  Provider.of<ThemeManager>(context);
	...
	listing =      ServiceReviewsListing(AppProvider.getApplication(context), getPath, false, ServiceReviewsListingUninitialized());
	 ...
	 return BlocBuilder<ServiceReviewsListing, ServiceReviewsState>(
      ...
      themeManager.themeMode == ThemeMode.dark? darkMode = true : darkMode = false;
        ...
        if (state is ServiceReviewsListingLoaded) {
		  ...
		  if (state.service_reviews!.listButton(context, widget.id!).length == 0){ 
            return
              Scaffold(
                  body:
                  Center(
              ...
              Scaffold(
                 ...
                 floatingActionButton: state.service_reviews!.Buttons(context, _dialVisible, widget.id!)
              );
			  }
          }else{
					  ...
					  slivers: <Widget>[
													  ...
													  title: InkWell(
														  ...
														  child :      Column(
															  ...
															  children: <Widget>[
																...
																//Text('Service Reviews by ${filterShortcodes(readText(widget.title!.replaceAll('&amp;','&'), 40))}', style: TextStyle(fontSize: 20.0, color: darkMode ? CurrentTheme.BackgroundColor : Colors.white)),
						...
						SliverList(
						  ...
						  delegate: SliverChildBuilderDelegate(
							...
							(context, index){  return index >= state.service_reviews!.items.items.length? 
													state.service_reviews!.tools.paging.total_pages == state.service_reviews!.tools.paging.current_page?
													Container(height: 0.0, width: 0.0,):	
													UserServiceReviewsListBottomLoader()
															  : index == 0 ?
											   state.service_reviews!.viewItem1 (state.service_reviews!.items.items[index] , state.service_reviews!.items.items[index] , searchText ,context, account, widget.id!,  state.hasReachedMax!, state.service_reviews!.items.items.length, index)
											  :state.service_reviews!.viewItem1 (state.service_reviews!.items.items[index] , state.service_reviews!.items.items[index-1] , searchText ,context,  account, widget.id!, state.hasReachedMax!, state.service_reviews!.items.items.length-1, index);											

												// : state.service_reviews!.viewItem (state.service_reviews!.items.items[index] , searchText, account, widget.id!, );
												 },
					...
					/*
					  ...
					  children: [
						...
						Positioned.fill(
							...
							Padding(
							  padding: EdgeInsets.only(left: 25, top: 20),
							  child: Text('Service Reviews', style: TextStyle(fontSize: 20, color: Colors.white),),
							)
											          ...
											          ListView.builder(
																itemBuilder: (BuildContext context, int index) {
																  ...
																  return index >= state.service_reviews!.items.items.length? 
																		...
																		UserServiceReviewsListBottomLoader()
																				  : index == 0 ?
																  state.service_reviews!.viewItem1 (state.service_reviews!.items.items[index] , state.service_reviews!.items.items[index] , searchText ,context, account, widget.title!,  state.hasReachedMax!, state.service_reviews!.items.items.length, index)
																  :state.service_reviews!.viewItem1 (state.service_reviews!.items.items[index] , state.service_reviews!.items.items[index-1] , searchText ,context,  account, widget.title!, state.hasReachedMax!, state.service_reviews!.items.items.length-1, index);
																	 // : state.service_reviews!.viewItem (state.service_reviews!.items.items[index] , searchText, account, widget.id!, );
																},
...
class ServiceReviewsListingLoaded extends ServiceReviewsState {
  final ServiceReviewsListingModel? service_reviews;
  ...
  */
   ...
   if (!hasReachedMax(currentState)) {
      try {
        if (currentState is ServiceReviewsListingUninitialized){
			  if(isSearch){
			     ServiceReviewsListingModel? service_reviews = await listingSearchServiceReviews(1);
				  int deltaPage = service_reviews!.items.items.length ~/(service_reviews!.tools.paging.total_rows/service_reviews!.tools.paging.total_pages).round();
				  if (deltaPage < 1){
					deltaPage = 1;
				  } else{
				  deltaPage = 1;
				  }
				  return emit ( service_reviews!.items.items.isEmpty ? (currentState as ServiceReviewsListingLoaded).copyWith(hasReachedMax: true,
					  page: deltaPage)                           
				   ...
				   ServiceReviewsListingModel? service_reviews = await listingServiceReviews(1);
				   int deltaPage;
				   if(service_reviews!.tools.paging.total_pages != 0){
				      deltaPage = service_reviews!.items.items.length ~/(service_reviews!.tools.paging.total_rows/service_reviews!.tools.paging.total_pages).round();
					  if (deltaPage < 1){
						deltaPage = 1;
					  } else{
						deltaPage = 1;
        ...
        if (currentState is ServiceReviewsListingLoaded) {
          //page++;
		  final oldpage = (currentState as ServiceReviewsListingLoaded).page;
		  	if(isSearch){
			  	  final page = (currentState as ServiceReviewsListingLoaded).page! + 1;
				  ServiceReviewsListingModel? service_reviews = await listingSearchServiceReviews(page);
				 if ((currentState as ServiceReviewsListingLoaded).service_reviews!.tools.paging.total_pages == oldpage) {
					return emit ((currentState as ServiceReviewsListingLoaded).copyWith(hasReachedMax: true,
						page: page));
                  }	else{   
				  ...
				  final page = (currentState as ServiceReviewsListingLoaded).page! + 1;
				   if ((currentState as ServiceReviewsListingLoaded).service_reviews!.tools.paging.total_pages == oldpage) {
					return emit ( (currentState as ServiceReviewsListingLoaded).copyWith(hasReachedMax: true,
					  ...
					  ServiceReviewsListingModel? service_reviews = await listingServiceReviews(page);
					 return emit (service_reviews!.items.items.isEmpty
						  ? (currentState as ServiceReviewsListingLoaded).copyWith(hasReachedMax: true,
						  page: page)
						  : ServiceReviewsListingLoaded(  service_reviews:  service_reviews ,
  ...
  final currentState = state;
   try {
        ...
        if (currentState is ServiceReviewsListingLoaded) {
		  if(isSearch){
		     ServiceReviewsListingModel? service_reviews = await listingSearchRefreshServiceReviews();
              return emit ( service_reviews!.items.items.isEmpty
		      ...
		      ServiceReviewsListingModel? service_reviews = await listingRefreshServiceReviews();
			   int deltaPage = service_reviews!.items.items.length ~/ (service_reviews!.tools.paging.total_rows/service_reviews!.tools.paging.total_pages).round();
				  if (deltaPage < 1){
					deltaPage = 1;
				  }else{
        ...
        if (currentState is ServiceReviewsListingUninitialized){
			  if(isSearch){
			     ServiceReviewsListingModel? service_reviews = await listingSearchServiceReviews(1);
				  ...
				  yield service_reviews!.items.items.isEmpty ? (currentState as ServiceReviewsListingLoaded).copyWith(hasReachedMax: true,
					  page: deltaPage)                           
					  :ServiceReviewsListingLoaded(service_reviews: service_reviews,
					  hasReachedMax: false,
					  page: deltaPage);
        ...
        if (currentState is ServiceReviewsListingLoaded) {
		  ...
		  final oldpage = (currentState as ServiceReviewsListingLoaded).page;
		  	if(isSearch){
				  ...
				  ServiceReviewsListingModel? service_reviews = await listingSearchServiceReviews(page);
				 if ((currentState as ServiceReviewsListingLoaded).service_reviews!.tools.paging.total_pages == oldpage) {
					yield (currentState as ServiceReviewsListingLoaded).copyWith(hasReachedMax: true,
						page: page);
                  }	else{   
					  if(!service_reviews!.items.items.isEmpty){
						 (currentState as ServiceReviewsListingLoaded).service_reviews!.items.items.addAll(service_reviews!.items.items);
  ...
  Future<ServiceReviewsListingModel?> listingServiceReviews(int page) async {
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	return await apiRepProvider!.getServiceReviewsListAPI(url, page);
	/*
	var dt  = ServiceReviewsListingModel(await apiRepProvider!.getData(sprintf(url, [page])));
	final data = await _loadAndSaveServiceReviewsListSearch(dt ,'',page);
	return data;
	*/
  }

  Future<ServiceReviewsListingModel?> listingRefreshServiceReviews() async {
  
  
   ServiceReviewsListingModel? service_reviews = null;
    APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllServiceReviewsList1();
    service_reviews  = await apiRepProvider!.getServiceReviewsListAPI(url, 1);
  ...
  Future<ServiceReviewsListingModel?> listingSearchServiceReviews(int page) async {
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	return await apiRepProvider!.getServiceReviewsListSearchAPI(url, page);
  ...
  Future<ServiceReviewsListingModel?> listingSearchRefreshServiceReviews() async {
   ServiceReviewsListingModel? service_reviews = null;
     APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllServiceReviewsList1();
    service_reviews  = await apiRepProvider!.getServiceReviewsListSearchAPI(url, 1);
    return service_reviews;
  /*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
