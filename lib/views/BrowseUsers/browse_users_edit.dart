...
//123
class PublicBrowseUsersEdit extends StatefulWidget {
  static const String PATH = '/public/browse_users/edit/:id/:title';
  final String? id ;
  final String? title;
  PublicBrowseUsersEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  PublicBrowseUsersEditState createState() => PublicBrowseUsersEditState();
}

class PublicBrowseUsersEditState extends State<PublicBrowseUsersEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/public/browse_users/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/public/browse_users/add';
  String sendPath = Env.value!.baseUrl! + '/public/browse_users/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/public/browse_users/edit/' +widget.id! +'/' + widget.title!;
    controller = ScrollController();
    validation.add(true);
  }

 fetchData(BrowseUsersController? browse_users)async {
    if (this.model == null) {
      final future = browse_users!.editBrowseUsers();
      future.then((value) {
        setState(() {
          isLoading = false;
        });
     ...
     fetchData(browse_users);
      ...
      return Scaffold(
          ...
          body:isLoading?
                  ...
                  child: Column(
					  ...
					  children: isLoading? [] : [
							  Padding(
							   ...
							   this.model.editPortofolio(this),  
							   this.model.editAsProjectWorker(this),  
							   this.model.editAsProjectOwner(this),  
							   this.model.editServices(this),  
							   this.model.editProducts(this),  
							   this.model.editUserArbitrations(this),  
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,browse_users, postBrowseUsersResult, this, sendPath, widget.id, widget.title)
