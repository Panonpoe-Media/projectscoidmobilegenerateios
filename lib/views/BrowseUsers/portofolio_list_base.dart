													  ...
													  title: InkWell(
														  ...
														  child :      Column(
															  ...
															  children: <Widget>[
																...
																//Text('Portofolio by ${filterShortcodes(readText(widget.title!.replaceAll('&amp;','&'), 40))}', style: TextStyle(fontSize: 20.0, color: darkMode ? CurrentTheme.BackgroundColor : Colors.white)),
						...
						SliverList(
						  ...
						  delegate: SliverChildBuilderDelegate(
							...
							(context, index){  return index >= state.portofolio!.items.items.length? 
													...
													PublicPortofolioListBottomLoader()
															  : index == 0 ?
...
class PortofolioListing extends Bloc<PortofolioEvent, PortofolioState> {
  ...
  */
        ...
        if (currentState is PortofolioListingUninitialized){
			  if(isSearch){
			     PortofolioListingModel? portofolio = await listingSearchPortofolio(1);
				  ...
				  return emit ( portofolio!.items.items.isEmpty ? (currentState as PortofolioListingLoaded).copyWith(hasReachedMax: true,
					  page: deltaPage)                           
		  ...
		  final oldpage = (currentState as PortofolioListingLoaded).page;
		  	if(isSearch){
				 ...
				 if ((currentState as PortofolioListingLoaded).portofolio!.tools.paging.total_pages == oldpage) {
					return emit ((currentState as PortofolioListingLoaded).copyWith(hasReachedMax: true,
						page: page));
                  }	else{   
  ...
  Future<PortofolioListingModel?> listingPortofolio(int page) async {
	...
	return await apiRepProvider!.getPortofolioListAPI(url, page);
	/*
  ...
  Future<PortofolioListingModel?> listingRefreshPortofolio() async {
   ...
   PortofolioListingModel? portofolio = null;
    ...
    portofolio  = await apiRepProvider!.getPortofolioListAPI(url, 1);
    return portofolio;
	/*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
  ...
  Future<PortofolioListingModel?> listingSearchPortofolio(int page) async {
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	return await apiRepProvider!.getPortofolioListSearchAPI(url, page);
  ...
  Future<PortofolioListingModel?> listingSearchRefreshPortofolio() async {
   PortofolioListingModel? portofolio = null;
     APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllPortofolioList1();
    portofolio  = await apiRepProvider!.getPortofolioListSearchAPI(url, 1);
    return portofolio;
  /*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
...
class  PublicPortofolioListBottomLoader extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(
