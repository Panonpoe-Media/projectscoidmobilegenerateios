...
class AsProjectWorkerListBase extends StatefulWidget {
  final String? id ;
  final String? title;
  String? url;
  AsProjectWorkerListBase({Key? key, this.id, this.title, this.url}) : super(key: key);
  @override
  AsProjectWorkerListBaseState createState() => AsProjectWorkerListBaseState();
}

class  AsProjectWorkerListBaseState extends State<AsProjectWorkerListBase>{
  String title = 'As Project Worker'; 
...
String filterShortcodes(String input,
  ...
  Widget build(BuildContext context) {
   ...
   final themeManager =  Provider.of<ThemeManager>(context);
	...
	listing =      AsProjectWorkerListing(AppProvider.getApplication(context), getPath, false, AsProjectWorkerListingUninitialized());
	 ...
	 return BlocBuilder<AsProjectWorkerListing, AsProjectWorkerState>(
      ...
      themeManager.themeMode == ThemeMode.dark? darkMode = true : darkMode = false;
        ...
        if (state is AsProjectWorkerListingLoaded) {
		  ...
		  if (state.as_project_worker!.listButton(context, widget.id!).length == 0){ 
            return
              Scaffold(
                  body:
                  Center(
              ...
              Scaffold(
                 ...
                 floatingActionButton: state.as_project_worker!.Buttons(context, _dialVisible, widget.id!)
              );
			  }
          }else{
					  ...
					  slivers: <Widget>[
													  ...
													  title: InkWell(
														  ...
														  child :      Column(
															  ...
															  children: <Widget>[
																...
																//Text('As Project Worker by ${filterShortcodes(readText(widget.title!.replaceAll('&amp;','&'), 40))}', style: TextStyle(fontSize: 20.0, color: darkMode ? CurrentTheme.BackgroundColor : Colors.white)),
						...
						SliverList(
						  ...
						  delegate: SliverChildBuilderDelegate(
							...
							(context, index){  return index >= state.as_project_worker!.items.items.length? 
													state.as_project_worker!.tools.paging.total_pages == state.as_project_worker!.tools.paging.current_page?
													Container(height: 0.0, width: 0.0,):	
													PublicAsProjectWorkerListBottomLoader()
															  : index == 0 ?
											   state.as_project_worker!.viewItem1 (state.as_project_worker!.items.items[index] , state.as_project_worker!.items.items[index] , searchText ,context, account, widget.id!,  state.hasReachedMax!, state.as_project_worker!.items.items.length, index)
											  :state.as_project_worker!.viewItem1 (state.as_project_worker!.items.items[index] , state.as_project_worker!.items.items[index-1] , searchText ,context,  account, widget.id!, state.hasReachedMax!, state.as_project_worker!.items.items.length-1, index);											

												// : state.as_project_worker!.viewItem (state.as_project_worker!.items.items[index] , searchText, account, widget.id!, );
												 },
					...
					/*
					  ...
					  children: [
						...
						Positioned.fill(
							...
							Padding(
							  padding: EdgeInsets.only(left: 25, top: 20),
							  child: Text('As Project Worker', style: TextStyle(fontSize: 20, color: Colors.white),),
							)
											          ...
											          ListView.builder(
																itemBuilder: (BuildContext context, int index) {
																  ...
																  return index >= state.as_project_worker!.items.items.length? 
																		...
																		PublicAsProjectWorkerListBottomLoader()
																				  : index == 0 ?
																  state.as_project_worker!.viewItem1 (state.as_project_worker!.items.items[index] , state.as_project_worker!.items.items[index] , searchText ,context, account, widget.title!,  state.hasReachedMax!, state.as_project_worker!.items.items.length, index)
																  :state.as_project_worker!.viewItem1 (state.as_project_worker!.items.items[index] , state.as_project_worker!.items.items[index-1] , searchText ,context,  account, widget.title!, state.hasReachedMax!, state.as_project_worker!.items.items.length-1, index);
																	 // : state.as_project_worker!.viewItem (state.as_project_worker!.items.items[index] , searchText, account, widget.id!, );
																},
...
class AsProjectWorkerListingLoaded extends AsProjectWorkerState {
  final AsProjectWorkerListingModel? as_project_worker;
  ...
  */
   ...
   if (!hasReachedMax(currentState)) {
      try {
        if (currentState is AsProjectWorkerListingUninitialized){
			  if(isSearch){
			     AsProjectWorkerListingModel? as_project_worker = await listingSearchAsProjectWorker(1);
				  int deltaPage = as_project_worker!.items.items.length ~/(as_project_worker!.tools.paging.total_rows/as_project_worker!.tools.paging.total_pages).round();
				  if (deltaPage < 1){
					deltaPage = 1;
				  } else{
				  deltaPage = 1;
				  }
				  return emit ( as_project_worker!.items.items.isEmpty ? (currentState as AsProjectWorkerListingLoaded).copyWith(hasReachedMax: true,
					  page: deltaPage)                           
				   ...
				   AsProjectWorkerListingModel? as_project_worker = await listingAsProjectWorker(1);
				   int deltaPage;
				   if(as_project_worker!.tools.paging.total_pages != 0){
				      deltaPage = as_project_worker!.items.items.length ~/(as_project_worker!.tools.paging.total_rows/as_project_worker!.tools.paging.total_pages).round();
					  if (deltaPage < 1){
						deltaPage = 1;
					  } else{
						deltaPage = 1;
        ...
        if (currentState is AsProjectWorkerListingLoaded) {
          //page++;
		  final oldpage = (currentState as AsProjectWorkerListingLoaded).page;
		  	if(isSearch){
			  	  final page = (currentState as AsProjectWorkerListingLoaded).page! + 1;
				  AsProjectWorkerListingModel? as_project_worker = await listingSearchAsProjectWorker(page);
				 if ((currentState as AsProjectWorkerListingLoaded).as_project_worker!.tools.paging.total_pages == oldpage) {
					return emit ((currentState as AsProjectWorkerListingLoaded).copyWith(hasReachedMax: true,
						page: page));
                  }	else{   
				  ...
				  final page = (currentState as AsProjectWorkerListingLoaded).page! + 1;
				   if ((currentState as AsProjectWorkerListingLoaded).as_project_worker!.tools.paging.total_pages == oldpage) {
					return emit ( (currentState as AsProjectWorkerListingLoaded).copyWith(hasReachedMax: true,
					  ...
					  AsProjectWorkerListingModel? as_project_worker = await listingAsProjectWorker(page);
					 return emit (as_project_worker!.items.items.isEmpty
						  ? (currentState as AsProjectWorkerListingLoaded).copyWith(hasReachedMax: true,
						  page: page)
						  : AsProjectWorkerListingLoaded(  as_project_worker:  as_project_worker ,
  ...
  final currentState = state;
   try {
        ...
        if (currentState is AsProjectWorkerListingLoaded) {
		  if(isSearch){
		     AsProjectWorkerListingModel? as_project_worker = await listingSearchRefreshAsProjectWorker();
              return emit ( as_project_worker!.items.items.isEmpty
		      ...
		      AsProjectWorkerListingModel? as_project_worker = await listingRefreshAsProjectWorker();
			   int deltaPage = as_project_worker!.items.items.length ~/ (as_project_worker!.tools.paging.total_rows/as_project_worker!.tools.paging.total_pages).round();
				  if (deltaPage < 1){
					deltaPage = 1;
				  }else{
        ...
        if (currentState is AsProjectWorkerListingUninitialized){
			  if(isSearch){
			     AsProjectWorkerListingModel? as_project_worker = await listingSearchAsProjectWorker(1);
				  ...
				  yield as_project_worker!.items.items.isEmpty ? (currentState as AsProjectWorkerListingLoaded).copyWith(hasReachedMax: true,
					  page: deltaPage)                           
					  :AsProjectWorkerListingLoaded(as_project_worker: as_project_worker,
					  hasReachedMax: false,
					  page: deltaPage);
        ...
        if (currentState is AsProjectWorkerListingLoaded) {
		  ...
		  final oldpage = (currentState as AsProjectWorkerListingLoaded).page;
		  	if(isSearch){
				  ...
				  AsProjectWorkerListingModel? as_project_worker = await listingSearchAsProjectWorker(page);
				 if ((currentState as AsProjectWorkerListingLoaded).as_project_worker!.tools.paging.total_pages == oldpage) {
					yield (currentState as AsProjectWorkerListingLoaded).copyWith(hasReachedMax: true,
						page: page);
                  }	else{   
					  if(!as_project_worker!.items.items.isEmpty){
						 (currentState as AsProjectWorkerListingLoaded).as_project_worker!.items.items.addAll(as_project_worker!.items.items);
  ...
  Future<AsProjectWorkerListingModel?> listingAsProjectWorker(int page) async {
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	return await apiRepProvider!.getAsProjectWorkerListAPI(url, page);
	/*
	var dt  = AsProjectWorkerListingModel(await apiRepProvider!.getData(sprintf(url, [page])));
	final data = await _loadAndSaveAsProjectWorkerListSearch(dt ,'',page);
	return data;
	*/
  }

  Future<AsProjectWorkerListingModel?> listingRefreshAsProjectWorker() async {
  
  
   AsProjectWorkerListingModel? as_project_worker = null;
    APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllAsProjectWorkerList1();
    as_project_worker  = await apiRepProvider!.getAsProjectWorkerListAPI(url, 1);
  ...
  Future<AsProjectWorkerListingModel?> listingSearchAsProjectWorker(int page) async {
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	return await apiRepProvider!.getAsProjectWorkerListSearchAPI(url, page);
  ...
  Future<AsProjectWorkerListingModel?> listingSearchRefreshAsProjectWorker() async {
   AsProjectWorkerListingModel? as_project_worker = null;
     APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllAsProjectWorkerList1();
    as_project_worker  = await apiRepProvider!.getAsProjectWorkerListSearchAPI(url, 1);
    return as_project_worker;
  /*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
