...
class RemoveMyBuddies extends StatefulWidget {

  final String? id ;
  final String? title;
  static const String PATH = '/user/my_buddies/remove/:id/:title';
  RemoveMyBuddies({Key? key, this.id, this.title}) : super(key: key);

  @override
  RemoveMyBuddiesState createState() => RemoveMyBuddiesState();
}

class RemoveMyBuddiesState extends State<RemoveMyBuddies> with RestorationMixin{
  String? getPath = Env.value!.baseUrl! + '/user/my_buddies/remove/%s/%s';
 // String? sendPath = Env.value!.baseUrl! + '/user/my_buddies/add';
  String? sendPath = Env.value!.baseUrl! + '/user/my_buddies/remove/%s/%s';

  MyBuddiesController? remove;
  ...
  var model;
  var formData;
  var postRemoveResult;
  var isLoading = true;
  var isError = false;
  var errmsg= 'Unauthorized  :'+'Remove';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  @override
  String? get restorationId => 'Remove';
 ...
 fetchData(MyBuddiesController? remove , BuildContext context)async {
    if (this.model == null) {
       final future = remove!.getRemoveMyBuddies('');
       future.then((value) {
        setState(() {
          isLoading = false;
        });
         ...
         _onWidgetDidBuild(() {
           ScaffoldMessenger.of(context).showSnackBar(
             SnackBar(
               content: Text('Unauthorized  :'+ sprintf('Remove', [widget.id!,widget.title!])),
               backgroundColor: Colors.red,
             ),
           );
         });
							...
							FlatButton(
								...
								if(this.model != null){
								    if([ 'PlaceNewBid'].contains('Remove')){
										remove!.deleteAllRemoveMyBuddies(this.model.model.model.project_title!);
										}else if(['AskOwner'].contains('Remove')){
										remove!.deleteAllRemoveMyBuddies(this.model.model.model.project_id.toString());
										}else if(['HireMe'].contains('Remove')){
										remove!.deleteAllRemoveMyBuddies(this.model.model.model.private_worker_id.toString());
										
										}else if(['InviteToBid'].contains('Remove')){
										remove!.deleteAllRemoveMyBuddies(this.model.model.model.user_id);
										}else{
										remove!.deleteAllRemoveMyBuddies('');
										}
								}
							 ...
							 FlatButton(
								...
								if(this.model != null){
											  ...
											  if (formKey.currentState!.validate()) {
												...
												if([ 'PlaceNewBid'].contains('Remove')){
													remove!.saveRemoveMyBuddies(this.model,this.model.model.model.project_title!);
													}else if(['AskOwner'].contains('Remove')){
													remove!.saveRemoveMyBuddies(this.model,this.model.model.model.project_id.toString());
													}else if(['HireMe'].contains('Remove')){
													remove!.saveRemoveMyBuddies(this.model,this.model.model.model.private_worker_id.toString());
													
													}else if(['InviteToBid'].contains('Remove')){
													remove!.saveRemoveMyBuddies(this.model,this.model.model.model.user_id);
													}else{
													remove!.saveRemoveMyBuddies(this.model,'');
													}
											  }else{
													if([ 'PlaceNewBid'].contains('Remove')){
													remove!.saveRemoveMyBuddies(this.model,this.model.model.model.project_title!);
  ...
  Widget build(BuildContext context) {
   ...
   final themeManager =  Provider.of<ThemeManager>(context);
    ...
    remove =    MyBuddiesController(AppProvider.getApplication(context),
        getPath,
        AppAction.edit,
        widget.id!,
        widget.title!,
	 ...
	 fetchData(remove, context);
      ...
      Scaffold(
                ...
                appBar: 
                            ...
                            */  
                        	...
                        	Container(
                              height: 30,
							   ...
							   isLoading!? [] : isError? null: this.model.RButtons(context, _dialVisible, formKey, controller,remove, postRemoveResult, this, sendPath, widget.id!, widget.title!),
                  ...
                  //  floatingActionButton: isLoading!? [] : isError? null: this.model.Buttons(context, _dialVisible, formKey, controller,remove, postRemoveResult, this, sendPath, widget.id!, widget.title!),
