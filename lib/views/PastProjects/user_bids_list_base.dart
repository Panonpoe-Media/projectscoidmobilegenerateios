...
class UserBidsListing extends Bloc<UserBidsEvent, UserBidsState> {
  ...
  final String url;
  final bool isSearch;
  UserBidsListing(this.application, this.url, this.isSearch, UserBidsState initialState):super(initialState);
  @override
  void dispose() {
   // super.dispose();
  }
 ...
 Stream<Transition<UserBidsEvent, UserBidsState>> transformEvents(
  ...
  Future<UserBidsListingModel?> listingUserBids(int page) async {
	...
	return await apiRepProvider!.getUserBidsListAPI1(url, page);
	/*
  ...
  Future<UserBidsListingModel?> listingRefreshUserBids() async {
   ...
   UserBidsListingModel? user_bids;
    APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository?.deleteAllUserBidsList2();
    user_bids  = await apiRepProvider!.getUserBidsListAPI1(url, 1);
    return user_bids;
	/*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
  ...
  Future<UserBidsListingModel> listingSearchUserBids(int page) async {
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	return await apiRepProvider!.getUserBidsListSearchAPI1(url, page);
  ...
  Future<UserBidsListingModel?> listingSearchRefreshUserBids() async {
   UserBidsListingModel? user_bids;
     APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository?.deleteAllUserBidsList2();
    user_bids  = await apiRepProvider!.getUserBidsListSearchAPI1(url, 1);
    return user_bids;
  /*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
