...
//123
class UserCheckoutEdit extends StatefulWidget {
  static const String PATH = '/user/checkout/edit/:id/:title';
  final String? id ;
  final String? title;
  UserCheckoutEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  UserCheckoutEditState createState() => UserCheckoutEditState();
}

class UserCheckoutEditState extends State<UserCheckoutEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/user/checkout/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/user/checkout/add';
  String sendPath = Env.value!.baseUrl! + '/user/checkout/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/user/checkout/edit/' +widget.id! +'/' + widget.title!;
    controller = ScrollController();
    validation.add(true);
  }

 fetchData(CheckoutController? checkout)async {
    if (this.model == null) {
      final future = checkout!.editCheckout();
      future.then((value) {
        setState(() {
          isLoading = false;
        });
     ...
     fetchData(checkout);
      ...
      return Scaffold(
          ...
          body:isLoading?
                  ...
                  child: Column(
					  ...
					  children: isLoading? [] : [
							  Padding(
							   ...
							   this.model.editCheckoutItems(this),  
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,checkout, postCheckoutResult, this, sendPath, widget.id, widget.title)
