...
//123
class UserMyProfileEdit extends StatefulWidget {
  static const String PATH = '/user/my_profile/edit/:id/:title';
  final String? id ;
  final String? title;
  UserMyProfileEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  UserMyProfileEditState createState() => UserMyProfileEditState();
}

class UserMyProfileEditState extends State<UserMyProfileEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/user/my_profile/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/user/my_profile/add';
  String sendPath = Env.value!.baseUrl! + '/user/my_profile/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/user/my_profile/edit/' +widget.id! +'/' + widget.title!;
    controller = ScrollController();
    validation.add(true);
  }

 fetchData(MyProfileController? my_profile)async {
    if (this.model == null) {
      final future = my_profile!.editMyProfile();
      future.then((value) {
        setState(() {
          isLoading = false;
        });
  ...
  Widget build(BuildContext context) {
    ...
    my_profile = new  MyProfileController(AppProvider.getApplication(context),
        getPath,
        AppAction.edit,
        widget.id,
        widget.title,
     ...
     fetchData(my_profile);
      ...
      return Scaffold(
          ...
          body:isLoading?
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,my_profile, postMyProfileResult, this, sendPath, widget.id, widget.title)
