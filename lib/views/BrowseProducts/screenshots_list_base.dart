...
String filterShortcodes(String input,
  ...
  Widget build(BuildContext context) {
   ...
   final themeManager =  Provider.of<ThemeManager>(context);
	...
	listing =      ScreenshotsListing(AppProvider.getApplication(context), getPath, false, ScreenshotsListingUninitialized());
...
class ScreenshotsListing extends Bloc<ScreenshotsEvent, ScreenshotsState> {
  ...
  */
        ...
        if (currentState is ScreenshotsListingUninitialized){
			  if(isSearch){
			     ScreenshotsListingModel? screenshots = await listingSearchScreenshots(1);
				  ...
				  return emit ( screenshots!.items.items.isEmpty ? (currentState as ScreenshotsListingLoaded).copyWith(hasReachedMax: true,
					  page: deltaPage)                           
		  ...
		  final oldpage = (currentState as ScreenshotsListingLoaded).page;
		  	if(isSearch){
				 ...
				 if ((currentState as ScreenshotsListingLoaded).screenshots!.tools.paging.total_pages == oldpage) {
					return emit ((currentState as ScreenshotsListingLoaded).copyWith(hasReachedMax: true,
						page: page));
                  }	else{   
  ...
  Future<ScreenshotsListingModel?> listingScreenshots(int page) async {
	...
	return await apiRepProvider!.getScreenshotsListAPI(url, page);
	/*
  ...
  Future<ScreenshotsListingModel?> listingRefreshScreenshots() async {
   ...
   ScreenshotsListingModel? screenshots = null;
    APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllScreenshotsList1();
    screenshots  = await apiRepProvider!.getScreenshotsListAPI(url, 1);
    return screenshots;
	/*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
  ...
  Future<ScreenshotsListingModel?> listingSearchScreenshots(int page) async {
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	return await apiRepProvider!.getScreenshotsListSearchAPI(url, page);
  ...
  Future<ScreenshotsListingModel?> listingSearchRefreshScreenshots() async {
   ScreenshotsListingModel? screenshots = null;
     APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllScreenshotsList1();
    screenshots  = await apiRepProvider!.getScreenshotsListSearchAPI(url, 1);
    return screenshots;
  /*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
