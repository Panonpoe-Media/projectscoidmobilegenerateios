...
import 'show_conversation_add_to_shortlist.dart';
import 'show_conversation_accept_bid.dart';
import 'show_conversation_show_bids.dart';
...
class ShowConversationListing extends Bloc<ShowConversationEvent, ShowConversationState> {
  ...
  final String url;
  final bool isSearch;
  ShowConversationListing(this.application, this.url, this.isSearch, ShowConversationState initialState):super(initialState);
  @override
  void dispose() {
   // super.dispose();
  }
 ...
 Stream<Transition<ShowConversationEvent, ShowConversationState>> transformEvents(
  ...
  Future<ShowConversationListingModel?> listingShowConversation(int page) async {
	...
	return await apiRepProvider!.getShowConversationListAPI1(url, page);
	/*
  ...
  Future<ShowConversationListingModel?> listingRefreshShowConversation() async {
   ...
   ShowConversationListingModel? show_conversation = null;
    APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllShowConversationList2();
    show_conversation  = await apiRepProvider!.getShowConversationListAPI1(url, 1);
  ...
  Future<ShowConversationListingModel?> listingSearchShowConversation(int page) async {
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	return await apiRepProvider!.getShowConversationListSearchAPI1(url, page);
  ...
  Future<ShowConversationListingModel?> listingSearchRefreshShowConversation() async {
   ShowConversationListingModel? show_conversation = null;
     APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllShowConversationList2();
    show_conversation  = await apiRepProvider!.getShowConversationListSearchAPI1(url, 1);
    return show_conversation;
  /*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
