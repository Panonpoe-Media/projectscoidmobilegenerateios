...
//123
class UserMyProjectsEdit extends StatefulWidget {
  static const String PATH = '/user/my_projects/edit/:id/:title';
  final String? id ;
  final String? title;
  UserMyProjectsEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  UserMyProjectsEditState createState() => UserMyProjectsEditState();
}

class UserMyProjectsEditState extends State<UserMyProjectsEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/user/my_projects/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/user/my_projects/add';
  String sendPath = Env.value!.baseUrl! + '/user/my_projects/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/user/my_projects/edit/' +widget.id! +'/' + widget.title!;
     ...
     fetchData(my_projects);
      ...
      return Scaffold(
          ...
          body:isLoading?
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,my_projects, postMyProjectsResult, this, sendPath, widget.id, widget.title)
