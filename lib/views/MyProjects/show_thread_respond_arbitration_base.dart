...
class ShowThreadRespondArbitrationBase extends StatefulWidget {
  final String? id ;
  final String? title;
  String? url;
  ShowThreadRespondArbitrationBase({Key? key, this.id, this.title, this.url}) : super(key: key);
  @override
  ShowThreadRespondArbitrationBaseState createState() => ShowThreadRespondArbitrationBaseState();
}

class ShowThreadRespondArbitrationBaseState extends State<ShowThreadRespondArbitrationBase>  with RestorationMixin{
  String getPath = '';
  String sendPath = '';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  @override
  String get restorationId => 'ShowThreadRespondArbitration';
    ...
    if (this.model == null) {
       ...
       future.then((value) {
        setState(() {
		  ...
		  try{
		   this.model = RespondArbitrationShowThreadModel(value);
		  }catch(e){
		   errmsg = value;
		   isError = true;
		  }
  ...
  Widget build(BuildContext context) {
    ...
    show_thread = new SubModelController(AppProvider.getApplication(context),
	 ...
	 fetchData(show_thread,  context );
      ...
      return Scaffold(
          appBar: AppBar(
		    title: Text('Respond Arbitration'),
          ),
          body:isLoading?
             // Center(
