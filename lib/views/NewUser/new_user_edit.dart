...
//123
class PublicNewUserEdit extends StatefulWidget {
  static const String PATH = '/public/new_user/edit/:id/:title';
  final String? id ;
  final String? title;
  PublicNewUserEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  PublicNewUserEditState createState() => PublicNewUserEditState();
}

class PublicNewUserEditState extends State<PublicNewUserEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/public/new_user/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/public/new_user/add';
  String sendPath = Env.value!.baseUrl! + '/public/new_user/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/public/new_user/edit/' +widget.id! +'/' + widget.title!;
    controller = ScrollController();
    validation.add(true);
  }

 fetchData(NewUserController? new_user)async {
    if (this.model == null) {
      final future = new_user!.editNewUser();
      future.then((value) {
        setState(() {
          isLoading = false;
        });
  ...
  Widget build(BuildContext context) {
    ...
    new_user = new  NewUserController(AppProvider.getApplication(context),
        getPath,
        AppAction.edit,
        widget.id,
        widget.title,
     ...
     fetchData(new_user);
      ...
      return Scaffold(
          ...
          body:isLoading?
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,new_user, postNewUserResult, this, sendPath, widget.id, widget.title)
