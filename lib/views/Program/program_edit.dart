...
//123
class PublicProgramEdit extends StatefulWidget {
  static const String PATH = '/public/program/edit/:id/:title';
  final String? id ;
  final String? title;
  PublicProgramEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  PublicProgramEditState createState() => PublicProgramEditState();
}

class PublicProgramEditState extends State<PublicProgramEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/public/program/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/public/program/add';
  String sendPath = Env.value!.baseUrl! + '/public/program/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/public/program/edit/' +widget.id! +'/' + widget.title!;
    controller = ScrollController();
    validation.add(true);
  }

 fetchData(ProgramController? program)async {
    if (this.model == null) {
      final future = program!.editProgram();
      future.then((value) {
        setState(() {
          isLoading = false;
        });
     ...
     fetchData(program);
      ...
      return Scaffold(
          ...
          body:isLoading?
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,program, postProgramResult, this, sendPath, widget.id, widget.title)
