...
class PromoProgram extends StatefulWidget {

  static const String PATH = '/public/program/promo/:id/:title';
  final String? id ;
  final String? title;
  PromoProgram({Key? key, this.id, this.title}) : super(key: key);

  @override
  PromoProgramState createState() => PromoProgramState();
}

class PromoProgramState extends State<PromoProgram> with RestorationMixin{
  String? getPath = Env.value!.baseUrl! + '/public/program/promo/%s/%s';
 // String? sendPath = Env.value!.baseUrl! + '/public/program/add';
  String? sendPath = Env.value!.baseUrl! + '/public/program/promo/%s/%s';

  ProgramController? promo;
...
final RestorableInt _counter = RestorableInt(0);
  ...
  @override
  String? get restorationId => 'Promo';
 ...
 fetchData(ProgramController? promo , BuildContext context)async {
    if (this.model == null) {
       final future = promo!.getPromoProgram('');
       future.then((value) {
        setState(() {
          isLoading = false;
        });
         ...
         _onWidgetDidBuild(() {
           ScaffoldMessenger.of(context).showSnackBar(
             SnackBar(
               content: Text('Unauthorized  :'+ sprintf('Promo', [widget.id!,widget.title!])),
               backgroundColor: Colors.red,
             ),
           );
         });
							...
							FlatButton(
								...
								if(this.model != null){
								    if([ 'PlaceNewBid'].contains('Promo')){
										promo!.deleteAllPromoProgram(this.model.model.model.project_title!);
										}else if(['AskOwner'].contains('Promo')){
										promo!.deleteAllPromoProgram(this.model.model.model.project_id.toString());
										}else if(['HireMe'].contains('Promo')){
										promo!.deleteAllPromoProgram(this.model.model.model.private_worker_id.toString());
										
										}else if(['InviteToBid'].contains('Promo')){
										promo!.deleteAllPromoProgram(this.model.model.model.user_id);
										}else{
										promo!.deleteAllPromoProgram('');
										}
								}
							 ...
							 FlatButton(
								...
								if(this.model != null){
											  ...
											  if (formKey.currentState!.validate()) {
												...
												if([ 'PlaceNewBid'].contains('Promo')){
													promo!.savePromoProgram(this.model,this.model.model.model.project_title!);
													}else if(['AskOwner'].contains('Promo')){
													promo!.savePromoProgram(this.model,this.model.model.model.project_id.toString());
													}else if(['HireMe'].contains('Promo')){
													promo!.savePromoProgram(this.model,this.model.model.model.private_worker_id.toString());
													
													}else if(['InviteToBid'].contains('Promo')){
													promo!.savePromoProgram(this.model,this.model.model.model.user_id);
													}else{
													promo!.savePromoProgram(this.model,'');
													}
											  }else{
													if([ 'PlaceNewBid'].contains('Promo')){
													promo!.savePromoProgram(this.model,this.model.model.model.project_title!);
  ...
  Widget build(BuildContext context) {
   ...
   final themeManager =  Provider.of<ThemeManager>(context);
    ...
    promo =    ProgramController(AppProvider.getApplication(context),
        getPath,
        AppAction.edit,
        widget.id!,
        widget.title!,
	 ...
	 fetchData(promo, context);
      ...
      Scaffold(
                ...
                appBar: 
                            ...
                            */  
                        	...
                        	Container(
                              height: 30,
							   ...
							   isLoading!? [] : isError? null: this.model.RButtons(context, _dialVisible, formKey, controller,promo, postPromoResult, this, sendPath, widget.id!, widget.title!),
                  ...
                  //  floatingActionButton: isLoading!? [] : isError? null: this.model.Buttons(context, _dialVisible, formKey, controller,promo, postPromoResult, this, sendPath, widget.id!, widget.title!),
