...
class  PublicTestListing extends StatefulWidget {
  static const String PATH = '/public/test/listing/:id';
    final String? id ;
   PublicTestListing({Key? key, this.id}) : super(key: key);
  @override
   PublicTestListingState createState() =>  PublicTestListingState();
}

class  PublicTestListingState extends State< PublicTestListing> with RestorationMixin{
  String title = 'Test'; 
...
final RestorableDouble cs = RestorableDouble(0);
  ...
  Widget build(BuildContext context) {
      ...
      getPath = Env.value!.baseUrl! + '/public/test/listing?page=%d' + ids;
    }
              ...
              actions: <Widget>[
                IconButton(
                    ...
                    onPressed: () async {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => SearchTestListing(id : '', title : '')),
                      );
					/*
                      if (!selected) {
                        setState(() {
      ...
      builder: (BuildContext context, TestState state) {
        ...
        if (state is TestListingLoaded) {
		  ...
		  if (state.test!.tools.buttons.length == 0){ 
            return
              ...
              Scaffold(
                 ...
                 floatingActionButton: state.test!.Buttons(context, _dialVisible, account, open)
				//floatingActionButton: isLoading? null :  state.test!.Buttons(context, _dialVisible, controller,test,  this, Env.value!.baseUrl!, '', title)
              );
			  }
          }
			if (state.test!.tools.buttons.length == 0){ 
					...
					body:
							...
							child:ListView.builder(
								itemBuilder: (BuildContext context, int index) {
									  ...
									  state.test!.tools.paging.total_pages == state.test!.tools.paging.current_page?
                                      Container(height: 0.0, width: 0.0,):
									  PublicTestBottomLoader()
									   : state.test!.viewItem (state.test!.items.items[index] , searchText, account );
								},
		  ...
		  Scaffold(
					...
					floatingActionButton: state.test!.Buttons(context, _dialVisible, account, open)
					//floatingActionButton: isLoading? null :  state.test!.Buttons(context, _dialVisible, controller,test,  this, Env.value!.baseUrl!, '', title, account)		
...
class  SearchTestListing extends StatefulWidget {
  static const String PATH = '/public/test/listing/:id';
    final String? id ;
	final String? title;
   SearchTestListing({Key? key, this.id, this.title}) : super(key: key);
  @override
   SearchTestListingState createState() =>  SearchTestListingState();
}

class  SearchTestListingState extends State< SearchTestListing>  with RestorationMixin{
  String title = 'Test'; 
  bool _dialVisible = true;
  bool search = false;
  String getPath = Env.value!.baseUrl! + '/public/test/listing?page=%d';
...
final RestorableDouble cs = RestorableDouble(0);
  ...
  Widget build(BuildContext context) {
    search = true;
    if(widget.id == ''){
      getPath = Env.value!.baseUrl! + '/public/test/listing?page=%d&' + 'search=' + searchText;
    } else{
      getPath = Env.value!.baseUrl! + '/public/test/listing/' + widget.id!  + '?page=%d';
    }
	accountController =AccountController(AppProvider.getApplication(context),
	AppAction.view   );
    List<Map> listAccount = [];
...
////////////////////////////////
class  PublicTestBottomLoader extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(
