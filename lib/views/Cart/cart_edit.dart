...
//123
class UserCartEdit extends StatefulWidget {
  static const String PATH = '/user/cart/edit/:id/:title';
  final String? id ;
  final String? title;
  UserCartEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  UserCartEditState createState() => UserCartEditState();
}

class UserCartEditState extends State<UserCartEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/user/cart/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/user/cart/add';
  String sendPath = Env.value!.baseUrl! + '/user/cart/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/user/cart/edit/' +widget.id! +'/' + widget.title!;
    controller = ScrollController();
    validation.add(true);
  }

 fetchData(CartController? cart)async {
    if (this.model == null) {
      final future = cart!.editCart();
      future.then((value) {
        setState(() {
          isLoading = false;
        });
     ...
     fetchData(cart);
      ...
      return Scaffold(
          ...
          body:isLoading?
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,cart, postCartResult, this, sendPath, widget.id, widget.title)
