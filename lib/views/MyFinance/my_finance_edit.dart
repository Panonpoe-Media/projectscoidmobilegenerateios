...
//123
class UserMyFinanceEdit extends StatefulWidget {
  static const String PATH = '/user/my_finance/edit/:id/:title';
  final String? id ;
  final String? title;
  UserMyFinanceEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  UserMyFinanceEditState createState() => UserMyFinanceEditState();
}

class UserMyFinanceEditState extends State<UserMyFinanceEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/user/my_finance/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/user/my_finance/add';
  String sendPath = Env.value!.baseUrl! + '/user/my_finance/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/user/my_finance/edit/' +widget.id! +'/' + widget.title!;
    controller = ScrollController();
    validation.add(true);
  }

 fetchData(MyFinanceController? my_finance)async {
    if (this.model == null) {
      final future = my_finance!.editMyFinance();
      future.then((value) {
        setState(() {
          isLoading = false;
        });
     ...
     fetchData(my_finance);
      ...
      return Scaffold(
          ...
          body:isLoading?
                  ...
                  child: Column(
					  ...
					  children: isLoading? [] : [
							  Padding(
							   ...
							   this.model.editPaymentHistory(this),  
							   this.model.editCreditHistory(this),  
							   this.model.editDataPajak(this),  
							   this.model.editSecurity(this),  
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,my_finance, postMyFinanceResult, this, sendPath, widget.id, widget.title)
