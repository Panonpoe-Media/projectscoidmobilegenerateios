...
class SalesListing extends Bloc<SalesEvent, SalesState> {
  ...
  */
        ...
        if (currentState is SalesListingUninitialized){
			  if(isSearch){
			     SalesListingModel? sales = await listingSearchSales(1);
				  ...
				  return emit ( sales!.items.items.isEmpty ? (currentState as SalesListingLoaded).copyWith(hasReachedMax: true,
					  page: deltaPage)                           
		  ...
		  final oldpage = (currentState as SalesListingLoaded).page;
		  	if(isSearch){
				 ...
				 if ((currentState as SalesListingLoaded).sales!.tools.paging.total_pages == oldpage) {
					return emit ((currentState as SalesListingLoaded).copyWith(hasReachedMax: true,
						page: page));
                  }	else{   
  ...
  Future<SalesListingModel?> listingSales(int page) async {
	...
	return await apiRepProvider!.getSalesListAPI(url, page);
	/*
  ...
  Future<SalesListingModel?> listingRefreshSales() async {
   ...
   SalesListingModel? sales = null;
    APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllSalesList1();
    sales  = await apiRepProvider!.getSalesListAPI(url, 1);
    return sales;
	/*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
  ...
  Future<SalesListingModel?> listingSearchSales(int page) async {
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	return await apiRepProvider!.getSalesListSearchAPI(url, page);
  ...
  Future<SalesListingModel?> listingSearchRefreshSales() async {
   SalesListingModel? sales = null;
     APIRepository? apiRepProvider = application.projectsAPIRepository;
    await application.projectsDBRepository!.deleteAllSalesList1();
    sales  = await apiRepProvider!.getSalesListSearchAPI(url, 1);
    return sales;
  /*
    APIRepository? apiRepProvider = application.projectsAPIRepository;
	int page = 0;
