...
//123
class UserMyOrdersEdit extends StatefulWidget {
  static const String PATH = '/user/my_orders/edit/:id/:title';
  final String? id ;
  final String? title;
  UserMyOrdersEdit({Key? key, this.id, this.title}) : super(key: key);
  @override
  UserMyOrdersEditState createState() => UserMyOrdersEditState();
}

class UserMyOrdersEditState extends State<UserMyOrdersEdit>  with RestorationMixin{
  String getPath = Env.value!.baseUrl! + '/user/my_orders/edit/%s/%s';
 // String sendPath = Env.value!.baseUrl! + '/user/my_orders/add';
  String sendPath = Env.value!.baseUrl! + '/user/my_orders/edit/%s/%s';
...
final RestorableInt _counter = RestorableInt(0);
  ...
  initState(){
    super.initState();
	sendPath = Env.value!.baseUrl! + '/user/my_orders/edit/' +widget.id! +'/' + widget.title!;
     ...
     fetchData(my_orders);
      ...
      return Scaffold(
          ...
          body:isLoading?
            ...
            floatingActionButton:  isLoading? null : this.model.Buttons(context, _dialVisible, formKey, controller,my_orders, postMyOrdersResult, this, sendPath, widget.id, widget.title)
